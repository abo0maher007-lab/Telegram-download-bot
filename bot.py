...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v33.0 Engine (x16 Parallel Download)
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v33_0")
...
app = Client("UniversalDownloaderBot_v33_0", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
# ----------------------------------------------------
# 🖼️ أدوات الثمبنيل والمدة وأبعاد الفيديو v33.0
# ----------------------------------------------------
...
def get_video_dimensions(file_path: str) -> Tuple[int, int]:
    """استخراج أبعاد الفيديو (العرض والارتفاع) لإصلاح زر تكبير الشاشة في المشغل"""
    try:
        cmd = [
            "ffprobe", "-v", "error",
            "-select_streams", "v:0",
...
def generate_ffmpeg_thumbnail(video_path: str, task_id: str, suffix: str = "") -> Optional[str]:
    """مولد الثمبنيل المطور المضمون لإصلاح عدم ظهور الصورة المصغرة"""
...
def get_valid_thumbnail(video_path: str, task_id: str, existing_thumb: Optional[str] = None, suffix: str = "") -> Optional[str]:
    """التحقق من وجود صورة مصغرة صالحة وتوليد واحدة إن لم تكن متوفرة"""
...
def split_video_file(file_path: str, task_id: str, target_size_bytes: int = 1900 * 1024 * 1024) -> List[str]:
    """تقسيم الفيديو إذا تجاوز الحجم المسموح به إلى أجزاء تناسب تليجرام"""
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v33.0 (x16 Parallel Download Engine)
# ----------------------------------------------------
class UniversalEngineV33:
    def __init__(self):
        self.user_agents = [
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        """استخراج معلومات المقطع والفيديو قبل التنزيل"""
        ydl_opts = {
            'quiet': True,
            'no_warnings': True,
        ...
        elif ("dailymotion.com" in url or "dai.ly" in url) and DM_COOKIE_PATH and os.path.exists(DM_COOKIE_PATH):
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        def ytdl_hook(d):
            ...
            if d.get("status") in ["downloading", "finished"]:
                ...
                label = "استخراج الصوت MP3" if target_option == "mp3" else f"جاري التحميل المتوازي x16 ({target_option}p)"
        ...
        ydl_opts = {
            ...
            'retries': 50,
            'fragment_retries': 50,
            'concurrent_fragment_downloads': 16,  # 🚀 تفعيل التنزيل المتوازي بـ 16 خيط
        ...
        if "dailymotion.com" in url.lower() or "dai.ly" in url.lower():
...
engine = UniversalEngineV33()

# ----------------------------------------------------
# 🛠️ مدير الواجهة والتقدم v33.0
# ----------------------------------------------------
    ...
    while task_id not in CANCELLED_TASKS:
        try:
            ...
            if now - last_update_time >= 2.5:
                ...
                text = (
                    f"⚡ **[v33.0 Universal Engine - x16 Parallel]**\n"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v33.0 Universal Engine**\n\n"
        "✨ **الجديد في v33.0:**\n"
        "• دعم كامل للتنزيل من منصة **Dailymotion** لجميع الفيديوهات والجودات.\n"
        "• دعم التحميل المتوازي بـ 16 خيطاً (x16 Parallel Concurrent Downloads) للسرعة القصوى.\n"
        "• حل جذر ومؤكد لمشكلة عدم ظهور الصورة المصغرة (Thumbnail) للفيديو.\n"
        "• إنتاج الصورة المصغرة واستخراج أبعاد الفيديو تلقائياً لدعم العرض والملء الكامل للشاشة.\n"
        "• المحافظة الكاملة على دعم التنزيل، التقصي، التقسيم وقص الفيديوهات `/trim`.\n\n"
        "💡 **مثال لاستخدام القص:**\n"
...
async def trim_url_command(client: Client, message: Message):
    ...
    status_msg = await message.reply_text("✂️ **جاري البدء في عملية تحميل وقص المقطع (x16 Parallel)...**", quote=True)
...
# ----------------------------------------------------
# 📩 معالجة الفيديو الموجه والملفات (Stateless Forward Engine v33)
# ----------------------------------------------------
...
async def process_video_conversion(client: Client, task_id: str, target_msg: Message, mode: str, status_msg: Message, trim_times: Optional[tuple] = None):
    ...
    try:
        ...
        parts = await loop.run_in_executor(None, split_video_file, final_file, task_id)
        ...
        for idx, part_file in enumerate(parts):
            ...
            def upload_progress(current, total):
                ...
                q = PROGRESS_QUEUES.get(task_id)
                if q:
                    label = f"رفع المستند ({idx+1}/{len(parts)})" if is_doc else (f"رفع الصوت MP3 ({idx+1}/{len(parts)})" if is_audio else f"رفع الفيديو ({idx+1}/{len(parts)})")
                    loop.call_soon_threadsafe(q.put_nowait, (label, current, total, upload_start))

            duration = get_media_duration(part_file)
            width, height = get_video_dimensions(part_file)
            file_title = os.path.basename(part_file)
            ...
            if is_doc:
                await client.send_document(
                    chat_id=status_msg.chat.id,
                    document=part_file,
                    caption=f"📁 **{file_title}**{part_suffix}\n🛡️ **Engine:** `v33.0 x16 Parallel & Thumb Fix`",
                    progress=upload_progress
                )
            elif is_audio:
                ...
                thumb_path = get_valid_thumbnail(part_file, task_id, suffix=f"_{idx}")
                await client.send_video(
                    chat_id=status_msg.chat.id,
                    video=part_file,
                    width=width if width > 0 else None,
...
# ----------------------------------------------------
# 📩 معالجة الرسائل والروابط ومدخلات القص v33.0
# ----------------------------------------------------
...
async def handle_message(client: Client, message: Message):
    ...
    try:
        ...
        info_text = (
            ...
            f"👇 **اختر الجودة أو الصيغة المطلوبة للبدء:**"
        )

        buttons = []
        if 1080 in res_list or not res_list:
            buttons.append([InlineKeyboardButton("🎬 1080p", callback_data=f"q_1080_{req_id}"), InlineKeyboardButton("🎬 720p", callback_data=f"q_720_{req_id}")])
        else:
            row = []
            if len(res_list) >= 1: row.append(InlineKeyboardButton(f"🎬 {res_list[0]}p", callback_data=f"q_{res_list[0]}_{req_id}"))
            if len(res_list) >= 2: row.append(InlineKeyboardButton(f"🎬 {res_list[1]}p", callback_data=f"q_{res_list[1]}_{req_id}"))
            if row: buttons.append(row)

        if 480 in res_list or 360 in res_list or not res_list:
            buttons.append([InlineKeyboardButton("🎬 480p", callback_data=f"q_480_{req_id}"), InlineKeyboardButton("🎬 360p", callback_data=f"q_360_{req_id}")])

        buttons.append([InlineKeyboardButton("✨ أفضل جودة متاحة (Auto)", callback_data=f"q_best_{req_id}")])
        buttons.append([InlineKeyboardButton("🎵 تحميل صوت MP3 (320kbps)", callback_data=f"q_mp3_{req_id}")])
...
async def option_callback_handler(client: Client, callback: CallbackQuery):
    try:
        ...
        msg_text = "📁 **جاري جلب وتحميل الملف...**" if option in ["doc", "vid"] else ("🎵 **جاري استخراج الصوت MP3...**" if option == "mp3" else f"🔎 **جاري التنزيل المتوازي x16 ({option}p)...**")
        await callback.answer()
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v33.0 Universal Engine (مع دعم Dailymotion والتحميل المتوازي x16)...")
    app.run()
