...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v63 Engine (Cloudflare Core Bypass Layer)
# ----------------------------------------------------
...
class UniversalEngineV63:
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """دالة مخصصة لتحميل الفيديوهات المباشرة وتفادي خطأ Cloudflare & 429 عبر Chrome Impersonation"""
        out_dir = "downloads"
        os.makedirs(out_dir, exist_ok=True)
        ...
        def progress_callback(blocknum, blocksize, totalsize):
            ...
            q = PROGRESS_QUEUES.get(task_id)
            if q:
                loop.call_soon_threadsafe(q.put_nowait, ("تحميل الفيديو (Cloudflare Direct Pass)", downloaded, totalsize, start_time, "bytes"))
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v63 Engine - Cloudflare Bypass]**\n"
                    ...
                    text = (
                        f"⚡ **[v63 Engine - Cloudflare Core Bypass Layer]**\n"
                        f"📌 **العملية:** {action_title}\n\n"
                        f"{bar}"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v63 Universal Downloader (Cloudflare Core Bypass Layer)**\n\n"
        "✨ **تحديثات الإصدار v63 الجديدة:**\n"
        "• ⚡ **[Cloudflare Core Bypass Layer]**: تخطي فحص `yt-dlp` بالكامل للروابط المباشرة مثل `b2.shahidtv.net` والروابط المنتهية بـ `.mp4` والتحميل الفوري بمحاكاة Chrome الفولاذية.\n"
        "• 🛡️ **حل مشكلة HTTP Error 429 & 403:** تدوير مخصص للرؤوس (Header Rotation) وانتحال صفة متصفحات الكروم الحديثة لتجاوز حظر المنصات.\n"
...
# ----------------------------------------------------
# 📩 معالجة الرسائل والروابط v63 (Cloudflare Core Bypass Layer)
# ----------------------------------------------------
...
async def handle_message(client: Client, message: Message):
    ...
    # 🚀 خطوة الحماية الفولاذية: إذا كان الرابط مباشراً وينتهي بامتداد فيديو، نتخطى فحص yt-dlp تماماً
    direct_extensions = ('.mp4', '.mkv', '.avi', '.mov', '.flv', '.webm', '.m4v', '.3gp')
    
    # تنظيف الرابط من أي مسافات أو رموز زائدة في النهاية للتحقق الدقيق
    clean_check_url = raw_url.rstrip()
    
    if any(clean_check_url.lower().endswith(ext) for ext in direct_extensions):
        req_id = f"dir_{message.from_user.id}_{int(time.time())}"
        PROGRESS_QUEUES[req_id] = asyncio.Queue()
        
        # حفظ الرابط في قائمة الانتظار للمطابقة
        PENDING_URLS[req_id] = raw_url
        
        status_msg = await message.reply_text(
            "⚡ **[v63 Engine - Direct Path Mode]**\n"
            "🔍 تم كشف رابط فيديو مباشر! جاري تخطي فحص الحماية والتحميل الفوري بمحاكاة الكروم الفولاذية...",
            quote=True
        )
        
        # توجيه الطلب مباشرة إلى معالج التحميل الآمن دون المرور بـ yt-dlp
        task = asyncio.get_running_loop().create_task(process_task(client, req_id, raw_url, "best", status_msg))
        ACTIVE_TASKS[req_id] = task
        return

    # النمط العادي للروابط الأخرى (تيك توك، فيسبوك، إلخ) التي لا تنتهي بـ .mp4 مباشرة
...
if __name__ == "__main__":
    logger.info("🚀 تشغيل البوت v63 Engine (Cloudflare Core Bypass)...")
    app.run()
