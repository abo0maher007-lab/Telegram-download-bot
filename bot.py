...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v58 Engine (curl_cffi Anti-403 Anti-Cloudflare Engine)
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v58_CurlCffi")
...
def compress_video_ffmpeg(input_path: str, target_quality: str, output_path: str, task_id: Optional[str] = None, loop: Optional[asyncio.AbstractEventLoop] = None) -> bool:
    try:
        ...
        logger.error(f"❌ استثناء غير متوقع أثناء ضغط الفيديو: {e}")
        return False
...
def is_direct_media_link(url: str) -> bool:
    url_lower = url.lower()
    direct_extensions = ('.mp4', '.mkv', '.avi', '.mov', '.flv', '.webm', '.m4v', '.3gp', '.mp3', '.m4a')
    parsed = urlparse(url_lower)
    return any(parsed.path.endswith(ext) for ext in direct_extensions)
...
def download_mega_file(url: str, task_id: str) -> Dict[str, Any]:
    try:
        ...
        raise Exception("يرجى تثبيت مكتبة mega.py")
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v58 Engine (مع curl_cffi وتجاوز Cloudflare)
# ----------------------------------------------------
class UniversalEngineV58:
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        ...
        ydl_opts = {
            ...
            'nocheckcertificate': True,
            'skip_download': True,
            'impersonate': 'chrome',  # استخدام محرك curl_cffi الذكي لتجاوز Cloudflare والبصمات
            'user_agent': ua,
            'geo_bypass': True,
            'check_formats': False,
            'verbose': True
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """دالة مخصصة مع إجبار تحويل الطلبات وتخطي Cloudflare عبر curl_cffi"""
        auto_disk_guard()
        url_lower = url.lower()
        ...
        if target_option == "mp3":
            ...
            is_audio = False
            postprocessors = [{'key': 'FFmpegThumbnailsConvertor', 'format': 'jpg'}]
            if target_option in ["best", "direct"]:
        ...
        # إعدادات yt-dlp المعززة بدعم --impersonate chrome وتفعيل curl_cffi لتجاوز 403 / Cloudflare
        ydl_opts = {
            ...
            'no_warnings': True,
            'nocheckcertificate': True,
            'impersonate': 'chrome',  # توجيه طلبات yt-dlp عبر محرك curl_cffi وبصمة Chrome
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[curl_cffi Bypass Engine]**\n"
                            f"📌 **العملية:** {action_title}\n\n"
                            f"{bar}"
                            f"⏱️ **المنقضي:** `{format_time(current)}` / `{format_time(total)}`\n"
                            f"🚀 **سرعة الضغط:** `{speed_factor:.2f}x` | ⏳ **المتبقي:** `{format_time(eta)}`\n\n"
                            f"📤 **الخطوة التالية:** سيتم رفع الفيديو فور الانتهاء."
                        )
                    else:
                    ...
                    if total > 0:
                        ...
                        bar = "🔄 `جاري تدفق البيانات مع تجاوز Cloudflare...`\n"
                        eta_str = ""

                    text = (
                        f"⚡ **[curl_cffi Protected Engine]**\n"
                        f"📌 **العملية:** {action_title}\n\n"
                        f"{bar}"
...
# ----------------------------------------------------
# ⚙️ تنفيذ المهام الشاملة والرفع لجميع المنصات والروابط المباشرة
# ----------------------------------------------------
async def process_task(client: Client, task_id: str, url: str, option: str, status_msg: Message):
    ...
    try:
        ...
        raw_desc = media_info.get("description", "")
        ...
        for idx, part_file in enumerate(parts):
            ...
            if media_info.get("is_audio") or option == "mp3":
            ...
            elif media_info.get("is_document") or option == "doc":
        ...
        # التحقق من خيارات التقاط 9 لقطات
        chat_id = status_msg.chat.id
        user_cfg = get_user_settings(chat_id)
        if not media_info.get("is_audio") and user_cfg["snapshots_direct"]:
...
# ----------------------------------------------------
# 🗜️ معالجة الفيديوهات المضغوطة المرفوعة مسبقاً
# ----------------------------------------------------
    ...
    try:
        ...
        for idx, part_file in enumerate(parts):
            ...
            caption = (
                ...
                f"🛡️ **Engine:** `curl_cffi Supported`"
            )
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v58 Universal Downloader & curl_cffi Protected Engine**\n\n"
        "✨ **المميزات المدمجة والتحديثات الجديدة:**\n"
        "• 🛡️ **تجاوز حظر Cloudflare و 403:** تم ربط `yt-dlp` بمحرك `curl_cffi` المطور مع وسام `--impersonate chrome` لمضاهاة بصمات المتصفح وإيقاف رسائل الحظر.\n"
        "• ⚡ **التحميل المباشر الفوري:** تم إلغاء أزرار اختيار الجودة في دالة التحميل المباشر ليبدأ التنزيل تلقائيًا دون تأخير.\n"
        "• 🗜️ **ضغط الفيديو التفاعلي:** دعم خيارات ضغط المقاطع بدقة بـ FFmpeg مع مؤشر المتابعة المباشر.\n"
        "• 🎵🎬 **نمط TikTok SS:** تنزيل الفيديو والصوت بشكل منفرد فور إضافة `ss` إلى الرابط.\n"
        "• ✂️ **قص الفيديوهات بـ `/trim`:** اقتطاع أي فيديو حسب البداية والنهاية بسهولة.\n"
        "• 🌐 **الحفاظ الكامل على المنصات:** دعم Dailymotion و MediaFire و Mega ومختلف منصات التواصل الاجتماعي.\n"
    )
...
async def settings_cmd(client: Client, message: Message):
    ...
    await message.reply_text(
        ...
        "يمكنك التعديل وتفعيل الخيارات بحسب مصدر الفيديو:",
        reply_markup=kb,
        quote=True
    )
...
async def trim_url_command(client: Client, message: Message):
    ...
    if not re.match(r'^https?://', url):
        await message.reply_text("❌ **الرابط غير صالح، يرجى كتابة رابط صحيح.**", quote=True)
        return
...
# ----------------------------------------------------
# 📩 استقبال واعتراض الرسائل والروابط
# ----------------------------------------------------
...
async def handle_message(client: Client, message: Message):
    ...
    if is_ss_mode:
        ...
        status_msg = await message.reply_text("⚡ **[TikTok SS Mode]** جاري استخراج الفيديو والصوت بشكل منفرد...", quote=True)
        PROGRESS_QUEUES[req_id] = asyncio.Queue()
        
        task = asyncio.get_running_loop().create_task(process_task(client, req_id, clean_url, "best", status_msg))
    ...
    # 1️⃣ معالجة التحميل المباشر للروابط المباشرة بدون إظهار أزرار الجودة
    if is_direct_media_link(url):
        status_msg = await message.reply_text("⚡ **جاري تنزيل الملف المباشر مع تجاوز الحظر...**", quote=True)
        PROGRESS_QUEUES[req_id] = asyncio.Queue()
        
        task = asyncio.get_running_loop().create_task(process_task(client, req_id, url, "direct", status_msg))
        ACTIVE_TASKS[req_id] = task
        return
    ...
    if "mediafire.com" in url.lower():
        try:
            ...
            if mf_info["is_video"]:
                ...
                await message.reply_text(
                    f"📁 **MediaFire File:** `{filename}`\n\n"
                    f"⚡ نوع الملف: `{mf_info['ext'].upper()}` - سيتم تنزيله كمستند.",
                    reply_markup=keyboard,
                    quote=True
                )
            return
    ...
    # 4️⃣ روابط وسائل التواصل الاجتماعي الأخرى (استخراج الجودات وإتاحة الاختيار)
    loading_msg = await message.reply_text("🔍 **جاري فحص الرابط مع محرك curl_cffi الذكي...**", quote=True)
    loop = asyncio.get_running_loop()
...
async def cancel_dailymotion_handler(client: Client, callback: CallbackQuery):
    try:
        ...
        if cancel_event:
            cancel_event.set()
            await callback.answer("🛑 جاري إلغاء التحميل...", show_alert=True)
        else:
            await callback.answer("⚠️ تعذر إيجاد العملية لإلغائها.", show_alert=True)
    except Exception as e:
        logger.error(f"Cancel Handler Error: {e}")

@app.on_callback_query(filters.regex(r"^cncl_"))
async def cancel_general_handler(client: Client, callback: CallbackQuery):
    try:
        ...
        await callback.message.edit_text("🛑 **تم إلغاء العملية بنجاح.**", reply_markup=None)
    except Exception as e:
        logger.error(f"Cancel General Error: {e}")
...
if __name__ == "__main__":
    auto_disk_guard()
    logger.info("🚀 جاري تشغيل البوت v58 مع محرك curl_cffi المطور وتخطي حمايات Cloudflare & 403...")
    app.run()
