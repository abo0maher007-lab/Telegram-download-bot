...
# --- خادم ويب خفيف لإرضاء Render ومنع خطأ Port Scanning ---
...
# ترويسات محدثة ومتطورة لتجاوز حظر 403 Forbidden
HEADERS = {
...
async def start_handler(event):
    await event.respond(
        "🚀 **مرحباً بك في بوت التنزيل والرفع المتقدم v5.2 Pro**\n\n"
...
async def stats_handler(event):
    ...
    await event.respond(
        f"📊 **إحصائيات النظام v5.2 Pro:**\n\n"
        f"⚙️ **العمليات النشطة:** `{len(ACTIVE_TASKS)}`\n"
...
async def download_gplay_apk(package_id, status_msg):
    await status_msg.edit(
        f"🔍 **جاري جلب معلومات التطبيق وتجاوز الحماية:** `{package_id}`..."
    )
    try:
...
async def process_download_job(event, url):
    ...
    status_msg = await event.respond(
        "⏳ **جاري فحص الرابط ومصادر الخدمة...**", buttons=get_buttons()
    )
    ...
    if "play.google.com" in url:
        ...
        url, file_name, icon_url = await download_gplay_apk(
            package_id, status_msg
        )
        custom_headers["Referer"] = f"https://apkpure.com/ar/{package_id}"
    ...
    try:
        async with aiohttp.ClientSession(
            timeout=timeout_config, headers=custom_headers
        ) as session:
            async with session.get(url, allow_redirects=True) as response:
                if response.status != 200:
                    await status_msg.edit(
                        f"❌ فشل الاتصال بالسيرفر. كود الاستجابة: `{response.status}`\n(تم رفض الطلب بـ 403 أو رابط منتهي الصلاحية)",
                        buttons=None,
                    )
                    return
                ...
                if total_size > MAX_FILE_SIZE:
                    await status_msg.edit(
                        "❌ يتجاوز حجم الملف الحد المسموح به (2 جيجابايت).",
                        buttons=None,
                    )
                    return
                    ...
                    async for chunk in response.content.iter_chunked(
                        ...
                        if cancel_event.is_set():
                            ...
                            if os.path.exists(file_name):
                                os.remove(file_name)
                            await status_msg.edit(
                                "🛑 تم إلغاء العملية بنجاح.", buttons=None
                            )
                            return

                        if not pause_event.is_set():
                            await status_msg.edit(
                                f"⏸️ **التنزيل موقوف مؤقتاً:** `{file_name}`",
        ...
        await status_msg.edit(
            f"📤 **جاري رفع:** `{file_name}` إلى تلغرام...",
            buttons=[
...
async def queue_handler(event):
    ...
    if qsize > 1:
        await event.respond(
            f"⏳ **تمت إضافة العملية لطابور الانتظار.**\nترتيبك: `{qsize - 1}`"
        )
...
async def main():
    ...
    print("Bot v5.2 Pro with Web Server is running...")
