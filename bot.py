...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v63 Engine (Cloudflare Bypass Edition)
# ----------------------------------------------------
...
def clean_facebook_url(url: str) -> str:
    """تنظيف وتنسيق روابط فيسبوك لحل مشاكل Cannot parse data في yt-dlp v63"""
    clean_url = url.strip()
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v63 Engine - مع حل التخفي والتجاوز الذكي لـ Cloudflare
# ----------------------------------------------------
class UniversalEngineV63:
    def __init__(self):
        self.user_agents = USER_AGENTS_POOL
        # قائمة ملفات الكوكيز المتاحة بحسب المنصة للتبديل والتنقل
        self.cookie_files = {
    ...
    def _get_base_ydl_opts(self, url: str, target_option: str, out_template: str, impersonate_target: str = "chrome") -> dict:
        """بناء خيارات yt-dlp المتقدمة لمنع الحظر مع دعم impersonate عبر curl_cffi"""
        ...
        ydl_opts = {
            ...
            'no_warnings': True,
            'nocheckcertificate': True,
            'impersonate': impersonate_target,  # تفعيل خيار impersonate باستخدام curl_cffi
        ...
        # إضافة البروكسي تلقائياً عند وجوده
        if HTTP_PROXY:
            ydl_opts['proxy'] = HTTP_PROXY
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        ...
        ydl_opts = self._get_base_ydl_opts(url, "info", "", impersonate_target="chrome")
        ydl_opts.update({
            'skip_download': True,
            'check_formats': False,
        })
        ...
        for attempt, target in enumerate(impersonate_targets):
            try:
                ...
                ydl_opts['http_headers'] = get_random_headers(is_fb)
                ...
                err_str = str(e).lower()
                if ("403" in err_str or "cloudflare" in err_str) and attempt < len(impersonate_targets) - 1:
                    logger.warning(f"⚠️ Cloudflare/403 Challenge detected during info extraction. Retrying with impersonate='{impersonate_targets[attempt+1]}'...")
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        ydl_opts = self._get_base_ydl_opts(url, "direct", file_path, impersonate_target="chrome")
        ydl_opts.update({
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """دالة التحميل الاستثنائية بأسلوب Async/Clean مع استراتيجية المحاولة والبدائل (Fallback) وتجاوز الحظر"""
        auto_disk_guard()
        url_lower = url.lower()
        ...
        # البدائل والوضعيات المختلفة للتنقل التلقائي عند مواجهة الحظر Cloudflare Challenge 403
        fallback_strategies = [
            {"impersonate": "chrome", "proxy": HTTP_PROXY, "use_cookies": True},
            {"impersonate": "chrome:120", "proxy": HTTP_PROXY, "use_cookies": True},
            {"impersonate": "safari", "proxy": HTTP_PROXY, "use_cookies": False},
            {"impersonate": "edge", "proxy": HTTP_PROXY, "use_cookies": True}
        ]
        ...
        for attempt, strategy in enumerate(fallback_strategies):
            ...
            ydl_opts = self._get_base_ydl_opts(url, target_option, out_template, impersonate_target=strategy["impersonate"])
            ...
            try:
                logger.info(f"🔄 محاولة التحميل ({attempt+1}/{len(fallback_strategies)}) باستعمال impersonate='{strategy['impersonate']}'...")
                ...
                if "403" in err_message or "cloudflare" in err_message or "challenge" in err_message:
                    logger.warning(f"⚠️ التماثُ مع Cloudflare/403 في المحاولة {attempt+1}. جاري الالتفاف والتجربة بالبديل التالي...")
                    time.sleep(2 * (attempt + 1) + random.uniform(0.5, 1.5))
                    continue
...
# ----------------------------------------------------
# 📥 استقبال المعالجات العامة والروابط مع لوحة الجودة المباشرة v63
# ----------------------------------------------------
...
async def start_command(client: Client, message: Message):
    welcome_text = (
        "👋 **أهلاً بك في بوت التحميل الشامل المحسّن (v63 Engine)!**\n\n"
        "✨ **المميزات المتاحة:**\n"
        "• 📥 تحميل الفيديوهات من كافة المنصات (تيك توك، إنستغرام، فيسبوك، دايلي موشن، تويتر).\n"
        "• 🛡️ **نظام تجاوز حظر Cloudflare 403 الجديد.**\n"
...
if __name__ == "__main__":
    logger.info("🚀 جاري تشغيل البوت مع محرك v63 الحامي من حظر Cloudflare...")
    app.run()
