...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v30.0 Engine
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v30_0")
...
app = Client("UniversalDownloaderBot_v30_0", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
# ----------------------------------------------------
# 🖼️ أدوات الثمبنيل والمدة والقص والتقسيم v30
# ----------------------------------------------------
...
class UniversalEngineV30:
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # في حالة القوائم (Playlists) نأخذ العنصر الأول
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v30.0 Universal Engine**\n\n"
        "✨ **الجديد في v30.0:**\n"
        "• إظهار معلومات ودقة الفيديو ومقاساته التلقائية في اللوحة قبل البدء بالتحميل.\n"
        "• دعم تقسيم الفيديو تلقائياً في حال تجاوز حجمه 2 جيجابايت.\n"
        "• إمكانية استخدام أمر القص المباشر `/trim [البداية] [النهاية] [الرابط]`.\n"
        "• المحافظة على جميع الخصائص والمميزات السابقة دون تغيير.\n\n"
        "💡 **مثال لاستخدام القص:**\n"
...
async def process_url_trim_task(client: Client, task_id: str, url: str, start_str: str, end_str: str, status_msg: Message):
    ...
    try:
        ...
        # فحص الحجم وتقسيم المقطع إذا تجاوز 2 جيجابايت
        ...
        for idx, part_file in enumerate(parts):
            ...
            thumb_path = generate_ffmpeg_thumbnail(part_file, task_id, f"_{idx}")

            part_caption = f"✂️ **{file_info['title']} (مقصوص)**\n⏱️ **من:** `{start_str}` **إلى:** `{end_str}`\n🛡️ **Engine:** `v30.0 Hybrid Edition`"
            if len(parts) > 1:
...
# ----------------------------------------------------
# 📩 معالجة الفيديو الموجه والملفات (Stateless Forward Engine v30)
# ----------------------------------------------------
...
async def process_video_conversion(client: Client, task_id: str, target_msg: Message, mode: str, status_msg: Message, trim_times: Optional[tuple] = None):
    ...
    try:
        ...
        # التحقق من إمكانية التقسيم إذا تجاوز 2 جيجابايت
...
# ----------------------------------------------------
# 📩 معالجة الرسائل والروابط ومدخلات القص (مع إظهار المعلومات قبل التحميل)
# ----------------------------------------------------
...
async def handle_message(client: Client, message: Message):
    ...
    # إظهار معلومات الفيديو والخيارات المتاحة قبل بدء التحميل
    ...
    try:
        ...
        # تراجع آمن في حال فشل جلب المعلومات مسبقاً
        keyboard = InlineKeyboardMarkup([
            [
...
async def option_callback_handler(client: Client, callback: CallbackQuery):
    try:
        ...
        msg_text = "📁 **جاري جلب وتحميل الملف...**" if option in ["doc", "vid"] else ("🎵 **جاري استخراج الصوت MP3...**" if option == "mp3" else f"🔎 **جاري التنزيل ({option}p)...**")
        await callback.answer()
...
async def process_task(client: Client, task_id: str, url: str, option: str, init_status_msg: Message):
    ...
    try:
        ...
        if os.path.exists(file_path):
            # فحص تقسيم الفيديو إذا تجاوز حجمه 2 جيجابايت
            ...
            for idx, part_file in enumerate(parts):
                ...
                if is_document:
                    ...
                    if part_duration <= 0: part_duration = get_media_duration(part_file)
                    part_thumb = generate_ffmpeg_thumbnail(part_file, task_id, f"_{idx}") if len(parts) > 1 else thumb_path
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v30.0 Universal Engine...")
    app.run()
