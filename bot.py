...
# ----------------------------------------------------
# 3. خطاف yt-dlp الذكي للتحميل بأعلى جودة
# ----------------------------------------------------
...
class ProgressFileReader:
    ...
    def read(self, size=-1):
        ...
        now = time.time()
        if now - self._last_update_time >= 2.0 or self._read_so_far == self._total_size:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        "مرحباً بك في بوت التنزيل والرفع المباشر v7.0 🚀\n\n"
        "✨ **المميزات:**\n"
        "• **لوحة رفع حية:** عداد تقدم حي يوضح نسبة وحجم وسرعة والوقت المتبقي أثناء الرفع.\n"
        "• **تنزيل مباشر بأقصى دقة** بمجرد تحويل أي رابط.\n"
...
async def process_download(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str, is_trim=False, start_sec=0, duration_sec=0):
    ...
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("❌ إلغاء العملية", callback_data=f"cancel_{task_id}")]])
    status_msg = await update.message.reply_text("⏳ **جاري الاتصال بالسيرفر...**", parse_mode="Markdown", reply_markup=keyboard)
...
def main():
    ...
    logging.info("==========================================")
    logging.info("تم التشغيل بنجاح! عداد الرفع التفاعلي يعمل الآن.")
    logging.info("==========================================")
