...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v65 Engine (Advanced Fallback & Cloudflare Bypass)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v65 Engine (تجاوز الأخطاء والحماية التراتبية)
# ----------------------------------------------------
class UniversalEngineV65:
    ...
    # =========================================================================
    # 🔄 دالة التحميل المحدثة جذرياً - خوارزمية السلسلة التراجعية (Fallback Chain)
    # =========================================================================
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        if target_option == "mp3":
            ...
            if target_option == "best":
                ...
                format_selector = f'best[height<={target_option}][ext=mp4]/bestvideo[height<={target_option}]+bestaudio/best'

        # إعدادات متسلسلة للتبديل في حالة الفشل الضمني (Fallback Strategies)
        fallback_strategies = [
            {'impersonate': 'chrome', 'format': format_selector, 'player_client': 'android,web'},
            {'impersonate': 'chrome:120', 'format': 'bestvideo+bestaudio/best', 'player_client': 'ios,web'},
            {'impersonate': 'safari', 'format': 'best', 'player_client': 'web'},
            {'impersonate': None, 'format': 'best', 'player_client': 'mweb'}
        ]
        ...
        for attempt, strategy in enumerate(fallback_strategies):
            ...
            ydl_opts = {
                ...
                'extractor_args': {
                    'youtube': [f"player_client={strategy['player_client']}"]
                }
            }
            ...
            if "facebook.com" in url_lower or "fb.watch" in url_lower or "fb.gg" in url_lower:
                url = clean_facebook_url(url)
                ydl_opts['user_agent'] = random.choice(FB_USER_AGENTS)
            ...
            # استخدام البروكسي عند وجود حجب أو تقييد في المحاولات المتقدمة
            if attempt >= 2:
            ...
            try:
                logger.info(f"🔄 محاولة التنزيل بالاستراتيجية رقم [{attempt + 1}/{len(fallback_strategies)}]")
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    info = ydl.extract_info(url, download=True)
                    if 'entries' in info and len(info['entries']) > 0:
                        info = info['entries'][0]
                ...
                last_exception = err
                logger.warning(f"⚠️ فشلت الاستراتيجية {attempt + 1}: {err}")
                time.sleep(1.5)
                continue

        raise last_exception if last_exception else Exception("تعذر استخراج أو تنزيل مقطع الفيديو بعد تجربة كافة الاستراتيجيات.")
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v65 Engine - Cloudflare Bypass Protected]**\n"
                    ...
                    text = (
                        f"⚡ **[v65 Engine - Direct Stream Speed]**\n"
                        f"📌 **العملية:** {action_title}\n\n"
                        f"{bar}"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v65 Universal Downloader & Direct Stream Engine**\n\n"
        "✨ **تحديثات الإصدار v65 الجديدة:**\n"
        "• 🛠️ **حل مشكلة خطأ التحميل نهائياً:** إضافة محرك المحاولات المتراتبة (Fallback Engine) لتجاوز قيود وحجب السيرفرات.\n"
        "• 🛡️ **تجاوز حظر المتصفحات:** تبديل البصمات (Chrome, Safari, iOS Client) تلقائياً عند ظهور أخطاء HTTP 403 أو 429.\n"
        "• 🔄 **تدوير واستخدام البروكسيات عند التقييد:** الاستعانة ببروكسيات سريعة للالتفاف على الحظر.\n"
...
async def process_url_trim_task(client: Client, task_id: str, url: str, start_str: str, end_str: str, status_msg: Message, user_message: Optional[Message] = None):
    ...
    try:
        ...
        for idx, part_file in enumerate(parts):
            ...
            duration = await loop.run_in_executor(None, get_media_duration, part_file)
            width, height = await loop.run_in_executor(None, get_video_dimensions, part_file)
            raw_thumb = await loop.run_in_executor(None, get_valid_thumbnail, part_file, task_id, None, f"_trm_{idx}")
            part_thumb = sanitize_thumb(raw_thumb)

            caption = f"✂️ **تم قص الفيديو بنجاح!**\n⏱️ **القص:** من `{start_str}` إلى `{end_str}`\n🛡️ **Engine:** `v65 Engine`"
...
if __name__ == "__main__":
    logger.info("🤖 جاري تشغيل البوت بمحرك v65 المتطور...")
    app.run()
