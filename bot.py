...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8932698139:AAE3xCrI18fJRoX2vgFD0DlCMORV_5HrBFo")
...
ACTIVE_TASKS = {}
QUEUE = asyncio.Queue()
PROCESSED_MESSAGES = set()  # منع تكرار استجابة البوت لنفس الرسالة
...
async def process_download_job(event, url):
    ...
    if "play.google.com" in url:
        ...
        if not package_id:
            await status_msg.edit("❌ لم يتم العثور على المعرف الخاص بالتطبيق.", buttons=None)
            return
...
async def queue_handler(event):
    ...
    # فحص معرف الرسالة لمنع التنفيذ المزدوج
...
async def main():
    ...
    print("Bot v5.1 Pro with Web Server is running...")
