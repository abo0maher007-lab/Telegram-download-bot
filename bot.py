...
def fix_faststart(input_path):
    """إعادة ترتيب أجزاء الفيديو لتدعم البث المباشر والتشغيل الفوري فوراً دون إهمال الجودة"""
    optimized_path = f"stream_{input_path}"
    try:
        cmd = [
            "ffmpeg",
            "-y",
        ...
        if os.path.exists(optimized_path) and os.path.getsize(optimized_path) > 0:
            ...
            os.rename(optimized_path, input_path)
    except Exception as e:
        print(f"Faststart optimization error: {e}")
...
async def start_handler(event):
    welcome_text = (
        ...
        "💡 أرسل رابط الفيديو المباشر ليتم رفعه مع المعاينة والتشغيل المباشر."
    )
...
async def process_download_job(event, url):
    ...
    file_name = os.path.basename(url.split("?")[0]) or "downloaded_file.mp4"
    ...
    try:
        ...
        if is_video:
            await safe_edit_message(
                status_msg,
                f"⚙️ **جاري تهيئة الفيديو لدعم البث والتشغيل المباشر...**",
                buttons=get_buttons(),
            )
            # تفعيل خاصية التشغيل المباشر البسيطة والسريعة
            fix_faststart(file_name)
        ...
        await bot.send_file(
            event.chat_id,
            file_name,
            caption=f"📱 **اسم الملف:** `{file_name}`\n⚡ **تم الرفع بنجاح (يدعم التشغيل المباشر)**",
