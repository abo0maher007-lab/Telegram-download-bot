...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v25.0 Build Fix Engine
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v25_0")
...
app = Client("UniversalDownloaderBot_v25_0", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
# ----------------------------------------------------
# ☁️ محرك Mega.nz المستقر (اعتماد yt-dlp المستقل بدون مكتبات خارجية)
# ----------------------------------------------------
def download_mega_file(url: str, task_id: str) -> Dict[str, Any]:
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            return {
                "file_path": filename,
                "title": str(info.get('title', 'Mega File')),
                "duration": 0,
                "thumb_path": None,
                "is_audio": False,
                "is_document": True
        ...
        # محاولة أخيرة عبر megatools إن وُجدت على النظام
        try:
            cmd = ["megatools", "dl", "--path", out_dir, url]
            subprocess.run(cmd, capture_output=True, text=True, check=True)
            files = [os.path.join(out_dir, f) for f in os.listdir(out_dir) if f.startswith(task_id)]
            if files:
            ...
            pass
        raise Exception(f"تعذر تحميل رابط Mega: {str(e)[:100]}")

# ----------------------------------------------------
# 🧠 المحرك الشامل v25.0
# ----------------------------------------------------
class UniversalEngineV25:
    def __init__(self):
        self.user_agents = [
        ...
        try:
            ...
            if now - last_update_time >= 2.5:
                ...
                if total > 0:
                    ...
                    bar = "🔄 `جاري تدفق البيانات واستخراج الملف...`\n"
                    eta_str = ""

                text = (
                    f"⚡ **[v25.0 Build Fix Engine]**\n"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v25.0 Build Fix Engine**\n\n"
        "• تم حل مشكلة البناء (Build Failed) نهائياً.\n"
        "• دعم جميع المواقع، بالإضافة إلى ميديافاير وميجا بأعلى كفاءة.\n"
        "• التحكم بالقرص عبر `/disk`.\n\n"
        "أرسل رابط التنزيل للبدء."
    )
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            def upload_progress(current, total):
                q = PROGRESS_QUEUES.get(task_id)
                if q:
                    label = "رفع الملف المستند" if is_document else ("رفع الملف الصوتي MP3" if is_audio else f"رفع الفيديو ({option}p)")
            ...
            if is_document:
                await client.send_document(
                    ...
                    document=file_path,
                    caption=f"📁 **{file_info['title']}**\n🛡️ **Engine:** `v25.0 Build Fix Edition`",
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v25.0 Build Fix Engine...")
    app.run()
