...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v64 Dynamic Proxy Engine
# ----------------------------------------------------
...
# الرابط الخاص بك من موقع Webshare
WEBSHARE_API_URL = "https://proxy.webshare.io/api/v2/proxy/list/download/nxgmkwgszrjlorsfpyyxgubjupcwvlljlcyemzgm/-/any/username/direct/-/?plan_id=14364793"

def get_working_proxy() -> Optional[str]:
    """جلب قائمة البروكسيات من الرابط واختيار واحد بشكل عشوائي"""
    try:
        ...
        if response.status_code == 200:
            ...
            if proxies:
                ...
                logger.info(f"🌐 [Webshare Proxy] تم اختيار البروكسي العشوائي: {selected.split('@')[-1]}")
                return selected
    except Exception as e:
        logger.error(f"❌ خطأ في جلب قائمة البروكسيات من Webshare: {e}")
...
# ----------------------------------------------------
# تحديث مكتبة yt-dlp تلقائياً عند تشغيل البوت
# ----------------------------------------------------
def auto_update_ytdlp():
    try:
        logger.info("🔄 [v64 Update] جاري فحص وتحديث مكتبة yt-dlp برمجياً لضمان العمل بكفاءة...")
        subprocess.run([os.sys.executable, "-m", "pip", "install", "--upgrade", "yt-dlp", "curl_cffi", "requests"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        logger.info("✅ [v64 Update] تم تحديث مكتبة yt-dlp والملحقات بأحدث إصدار متاحة.")
    except Exception as e:
...
# ----------------------------------------------------
# إدارة الكوكيز (Cookies Management & Auto Netscape Support)
# ----------------------------------------------------
...
async def download_dailymotion_video(event, url, quality_choice, status_msg, user_message: Optional[Message] = None):
    ...
    selected_proxy = get_working_proxy() or HTTP_PROXY
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v64 Dynamic Proxy Engine
# ----------------------------------------------------
class UniversalEngineV64:
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """دالة مخصصة لتحميل الفيديوهات المباشرة بتمرير البصمات وتفادي حظر HTTP 403 مع البروكسي الديناميكي"""
        out_dir = "downloads"
        os.makedirs(out_dir, exist_ok=True)
        ...
        def ytdl_hook(d):
            ...
            if d.get("status") in ["downloading", "finished"]:
                ...
                q = PROGRESS_QUEUES.get(task_id)
                if q:
                    loop.call_soon_threadsafe(q.put_nowait, ("تحميل مباشر عالي السرعة (محمي بالبروكسي)", downloaded, total, start_time, "bytes"))
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        # إضافة البروكسي الديناميكي فقط إذا تم جلب القائمة أو تعيينه بنجاح
        if selected_proxy:
            ydl_opts['proxy'] = selected_proxy
        ...
        for dl_attempt in range(max_dl_retries):
            try:
                # محاولة تغيير البروكسي مجدداً في حال المحاولات المكررة
                if dl_attempt > 0:
                ...
                if "403" in err_msg or "forbidden" in err_msg or "429" in err_msg:
                    if dl_attempt < max_dl_retries - 1:
                        backoff_delay = (2 ** dl_attempt) + random.uniform(1.0, 3.0)
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v64 Engine - Dynamic Webshare Proxy]**\n"
                    ...
                    text = (
                        f"⚡ **[v64 Engine - Dynamic Proxy Engine]**\n"
                        f"📌 **العملية:** {action_title}\n\n"
                        f"{bar}"
    ...
    try:
        ...
        for idx, part_file in enumerate(parts):
            ...
            caption = (
                ...
                f"🛡️ **Engine:** `v64 Dynamic Proxy Engine`"
            )
...
# ----------------------------------------------------
# 📡 الأحداث والأوامر v64
# ----------------------------------------------------
@app.on_message(filters.command(["start", "."]) | filters.regex(r"^/$") & filters.private)
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v64 Universal Downloader Engine**\n\n"
        "🌐 **تحديثات إصدار v64 (دعم البروكسي الديناميكي عبر Webshare API):**\n"
        "• 🔄 **دالة جلب البروكسي تلقائياً:** التبديل والعزل الديناميكي للعنوان واختيار بروكسي عشوائي عند كل عملية تنزيل.\n"
        "• 🛡️ **تكامل كامل مع yt-dlp:** دمج خيار البروكسي بسلاسة مع خيارات `ydl_opts['proxy']` لضمان عدم حظر IP السيرفر.\n"
        "• 🎭 **محاكاة متكاملة:** التجاوز الذكي لحظر IP وCloudflare عبر تنوع المسارات والجلسات.\n\n"
        "✨ **مع الحفاظ التام على باقي المميزات:** (الرفع التلقائي للأدمن، الضغط بـ FFmpeg، أمر /trim، واقتطاع 9 لقطات)."
    )
...
if __name__ == "__main__":
    logger.info("🚀 جاري تشغيل بوت v64 Dynamic Proxy Engine...")
    auto_disk_guard()
    
    # ضمان إنشاء Event Loop بدائي مستقر متوافق مع Python 3.10+ / Python 3.14
    try:
        loop = asyncio.get_running_loop()
    except RuntimeError:
