...
async def download_file_direct(url, output_path, cancel_event, status_msg):
    ...
    # معالجة الروابط المباشرة العادية مع دعم إعادة المحاولة لخطأ 429
...
async def start_handler(event):
    # التحقق من أن أرسل الرسالة هو مالك البوت فقط
    ...
    welcome_text = (
        ...
        "💡 **ماذا يقدم هذا البوت؟**\n"
        "يقوم البوت بأخذ الروابط المباشرة للملفات وروابط Mega، ويدعم تحميلها ثم إعادة رفعها لك مباشرة داخل تلغرام بسرعة عالية جداً دون استهلاك بياناتك!\n\n"
        "📦 **المميزات والصيغ المدعومة:**\n"
        "• 📏 الحد الأقصى: ملفات بحجم يصل إلى 2 جيجابايت (2GB).\n"
        "• 🎬 الفيديوهات والصوت: MP4, MKV, MP3, WEBM...\n"
        "• ☁️ دعم كامل لروابط Mega.nz المباشرة.\n"
...
async def process_download_job(event, url):
    ...
    try:
        ...
        # استخراج لقطات المعاينة وإرسالها كمجموعة صور منفصلة (Album)
        if is_video:
...
async def queue_handler(event):
    # تجاهل أي مستخدم آخر غير الأدمن
    if event.sender_id != ADMIN_ID:
        return
