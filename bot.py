...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v28.0 Hybrid Engine
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v28_0")
...
app = Client("UniversalDownloaderBot_v28_0", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)

ACTIVE_TASKS = {}
CANCELLED_TASKS = set()
PENDING_URLS = {}
AWAITING_TRIM_INPUT = {}  # إدارة إدخال أوقات القص
PROGRESS_QUEUES = {}
...
# ----------------------------------------------------
# 🖼️ أدوات الثمبنيل والمدة والقص v28
# ----------------------------------------------------
    ...
    while task_id not in CANCELLED_TASKS:
        try:
            ...
            if now - last_update_time >= 2.5:
                ...
                text = (
                    f"⚡ **[v28.0 Universal Hybrid Engine]**\n"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v28.0 Universal Hybrid Engine**\n\n"
        "✨ **الجديد في v28.0:** حل مشكلة انتهت صلاحية الطلب بشكل نهائي ودائم عبر Stateless Callbacks!\n\n"
        "• تحويل وقص الفيديوهات المحولة للبوت (فيديو / MP3 / مستند).\n"
        "• قص الفيديو بدقة بتحديد وقت البداية والنهاية.\n"
        "• دعم ملفات تصل لـ **2 جيجابايت**.\n"
        "• التعرف الذكي على روابط **MediaFire**، **Mega.nz**، ومواقع التواصل.\n"
        "• لوحة التحكم بالقرص `/disk` وإلغاء العمليات الفوري.\n\n"
...
# ----------------------------------------------------
# 📩 معالجة الفيديو الموجه والملفات (Stateless Forward Engine v28)
# ----------------------------------------------------
...
async def handle_video_or_document(client: Client, message: Message):
    ...
    # التضمين المباشر لـ chat_id و message_id داخل أزرار Callback لتفادي انتهاء الصلاحية
...
async def conversion_callback_handler(client: Client, callback: CallbackQuery):
    try:
        ...
        # جلب الرسالة المرجعية الأصلية مباشرة عبر التليجرام
        try:
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v28.0 Hybrid Engine...")
    app.run()
