...
logger = logging.getLogger("DownloaderBot-v4")

# ----------------------------------------------------
# 🚂 الإعداد والمتغيرات - v4
# ----------------------------------------------------
...
app = Client("SmartDownloaderBot_v4", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
# ----------------------------------------------------
# 📊 أدوات v4 المتقدمة (التنظيف، المعالجة، والقياس)
# ----------------------------------------------------

def cleanup_temp_files(task_id: str):
    """تنظيف مؤكد وشامل لجميع الملفات وبقايا الدمج والتحويل"""
    patterns = [
        f"downloads/{task_id}*",
...
def scrape_direct_video_link_v4(url: str) -> str:
    """محرك v4 للقشط الخارق لتجاوز جدران الحماية"""
    headers = {
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # 2. وسوم الميتا og/twitter/video
            for meta in soup.find_all('meta'):
                prop = meta.get('property', '') or meta.get('name', '')
                if prop in ['og:video', 'og:video:secure_url', 'twitter:player:stream', 'twitter:player']:
                    if meta.get('content') and meta['content'].startswith("http"):
                        return meta['content']

            # 3. Regex للروابط المباشرة (mp4/m3u8/webm)
            matches = re.findall(r'https?://[^\s\'"]+\.(?:m3u8|mp4|webm|mov)', resp.text)
            if matches:
                return matches[0]
    except Exception as e:
        logger.warning(f"v4 Scraper failed: {e}")
    return None
...
def get_cancel_keyboard(task_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[
        InlineKeyboardButton("❌ إلغاء العملية (v4 Extreme)", callback_data=f"cancel_{task_id}")
    ]])
...
def make_ytdl_opts(base_opts: dict, status_msg: Message, loop: asyncio.AbstractEventLoop, task_id: str, action_name: str):
    ...
    opts["concurrent_fragment_downloads"] = 10  # تسريع مضاعف للـ HLS
    opts["socket_timeout"] = 15
    opts["retries"] = 3
    return opts

async def smart_extractor_v4(url: str, status_msg: Message, task_id: str):
    ...
    for idx, (st_opts, st_name) in enumerate(strategies, 1):
        if task_id in CANCELLED_TASKS:
            raise ProcessCancelledException("CANCELLED_BY_USER")
        try:
            await update_status_ui(status_msg, f"فحص v4 ({idx}/18)...", 0, 100, time.time(), task_id)
    ...
    # 🚀 خطة التعزيز v4 Extreme
    try:
        await update_status_ui(status_msg, "⚡ تشغيل Extreme Scraper v4...", 0, 100, time.time(), task_id)
        scraped_url = await loop.run_in_executor(None, scrape_direct_video_link_v4, url)
        if scraped_url:
            opts = make_ytdl_opts(strategies[8][0], status_msg, loop, task_id, "تحميل Extreme v4")
            file_path, title, duration = await loop.run_in_executor(None, run_yt, scraped_url)
            if file_path and os.path.exists(file_path):
                return file_path, title, duration, "⚡ Extreme Direct Scraper v4"
    except Exception as e:
        logger.error(f"فشلت خطة التعزيز v4: {e}")
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text("🔥 **أهلاً بك في الإصدار v4 Extreme**\n\nأرسل أي رابط فيديو لبدء الاستخراج الفائق وتجاوز الحظر.")
...
async def process_download(client: Client, message: Message, task_id: str, url: str):
    status_msg = await message.reply_text(
        "⏳ **[v4 Extreme] جاري بدء المحرك وفحص الرابط...**",
    ...
    try:
        file_path, title, duration, used_method = await smart_extractor_v4(url, status_msg, task_id)
        ...
        if file_path and os.path.exists(file_path):
            ...
            await status_msg.edit_text("❌ **فشلت جميع محاولات v4 Extreme في استخراج هذا الفيديو.**")
...
if __name__ == "__main__":
    ...
    logger.info("🚀 تم تشغيل محرك v4 Extreme بنجاح...")
    app.run()