...
# ----------------------------------------------------
# 📡 الأحداث والأوامر (تشمل أمر /trim الجديد في v29)
# ----------------------------------------------------
...
def build_disk_keyboard():
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("📤 إرسال وتصنيف المستندات والوسائط", callback_data="disk_auto_send"),
        ],
        [
...
async def disk_command(client: Client, message: Message):
    ...
    info = get_disk_info()
    text = (
        "💾 **لوحة التحكم المحدثة في قرص التحميلات /disk:**\n\n"
...
async def auto_send_and_classify_files(client: Client, chat_id: int):
        ...
        if os.path.isfile(file_path):
            ...
            try:
                if ext in video_exts:
                    ...
                    thumb_path = generate_ffmpeg_thumbnail(file_path, f"auto_{int(time.time())}")
                    await client.send_video(
                        chat_id=chat_id,
                        video=file_path,
                        thumb=thumb_path if (thumb_path and os.path.exists(thumb_path)) else None,
                        duration=duration,
                        caption=f"🎬 **تصنيف تلقائي (فيديو):** `{filename}`\n🛡️ **Engine:** `v29.0 Hybrid`"
                    )
                    ...
                    await client.send_audio(
                        ...
                        audio=file_path,
                        duration=duration,
                        caption=f"🎵 **تصنيف تلقائي (صوت):** `{filename}`\n🛡️ **Engine:** `v29.0 Hybrid`"
                    )
                elif ext in photo_exts:
                    await client.send_photo(
                        chat_id=chat_id,
                        photo=file_path,
                        caption=f"🖼️ **تصنيف تلقائي (صورة):** `{filename}`\n🛡️ **Engine:** `v29.0 Hybrid`"
                    )
                else:
                    await client.send_document(
                        chat_id=chat_id,
                        document=file_path,
                        caption=f"📁 **تصنيف تلقائي (مستند):** `{filename}`\n🛡️ **Engine:** `v29.0 Hybrid`"
                    )
                sent_count += 1
                try:
                    os.remove(file_path)
                ...
                logger.error(f"Failed to auto send {file_path}: {e}")
                failed_count += 1
...
async def disk_callback_handler(client: Client, callback: CallbackQuery):
    ...
    if action == "disk_auto_send":
        await callback.answer("⏳ جاري فحص، تصنيف وإرسال جميع وسائط ومستندات القرص...")
        sent, failed = await auto_send_and_classify_files(client, callback.message.chat.id)
        await callback.answer(f"✅ تم تصنيف وإرسال {sent} ملف بنجاح! (فشل: {failed})", show_alert=True)
