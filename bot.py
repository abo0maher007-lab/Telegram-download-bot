...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v60 Engine (Cloudflare Anti-Bot Bypass)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 🌐 v60: توسيع حوض متصفحات الـ User-Agents للموبايل والويب
# ----------------------------------------------------
USER_AGENTS_POOL = [
...
# ----------------------------------------------------
# ⚙️ v60: محاكي متصفح متقدم وضبط رؤوس الطلبات لكسر Cloudflare
# ----------------------------------------------------
def get_random_headers(referer: Optional[str] = None) -> dict:
    ...
    elif "Android" in ua or "Linux" in ua:
        platform = '"Android"' if "Android" in ua else '"Linux"'
    else:
        platform = '"Windows"'

    site_mode = random.choice(['same-origin', 'cross-site', 'none'])
...
# ----------------------------------------------------
# 📌 تنزيل ورفع فيديوهات Dailymotion v60
# ----------------------------------------------------
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    # v60: تفعيل خيارات كسر حماية Cloudflare وتأطير Generic Impersonate Chrome
    ydl_opts = {
        'format': format_str,
        ...
        def run_dl():
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                try:
                    ...
                    if "403" in str(ex_dm) or "Cloudflare" in str(ex_dm):
                        ydl_opts['headers'] = get_random_headers('https://www.google.com/')
...
def inspect_mediafire_link(url: str) -> Dict[str, Any]:
    ...
    headers = get_random_headers(referer=origin_referer)
...
def download_mediafire_file(url: str, target_option: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
    ...
    start_time = time.time()
    parsed = urlparse(url)
    headers = get_random_headers(referer=f"{parsed.scheme}://{parsed.netloc}/")
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            if e.code in [429, 403, 500, 502, 503, 504] and attempt < max_attempts - 1:
                jitter = random.uniform(0.5, 2.5)
                sleep_time = min(30, (base_delay ** attempt) + jitter)
                logger.warning(f"⚠️ HTTP {e.code} on MediaFire. Jittered backoff sleeping {sleep_time:.2f}s...")
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v60 Engine (Cloudflare Anti-Bot Bypass)
# ----------------------------------------------------
class UniversalEngineV60:
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        """v60: استخراج معلومات الفيديو مع كسر جدران حماية Cloudflare وتخفي Generic Impersonate"""
        parsed = urlparse(url)
        origin_ref = f"{parsed.scheme}://{parsed.netloc}/" if parsed.scheme and parsed.netloc else "https://www.google.com/"
        headers = get_random_headers(referer=origin_ref)
        url_lower = url.lower()
        ...
        # v60: Dynamic Handler for Cloudflare Challenges & 403 Errors
        max_retries = 4
        base_delay = 2
        for attempt in range(max_retries):
            try:
                ...
                err_msg = str(e).lower()
                if ("429" in err_msg or "403" in err_msg or "cloudflare" in err_msg or "http error" in err_msg) and attempt < max_retries - 1:
                    jitter = random.uniform(0.5, 2.5)
                    sleep_time = min(30, (base_delay ** attempt) + jitter)
                    logger.warning(f"🛡️ v60 Cloudflare/Anti-Bot Challenge Detected (Attempt {attempt + 1}). Retrying silently in {sleep_time:.2f}s...")
                    time.sleep(sleep_time)
                    
                    # تعديل صامت ورؤوس معززة ومحاكي متصفح متطور لـ Cloudflare
                    new_headers = get_random_headers(referer="https://www.google.com/")
                    ydl_opts['http_headers'] = new_headers
                    ydl_opts['user_agent'] = new_headers['User-Agent']
                    ydl_opts['impersonate'] = random.choice(['chrome', 'chrome-120', 'edge-120'])
                    ydl_opts['extractor_args']['generic'] = {'impersonate': ['chrome']}
                    continue
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """v60: دالة التنزيل المباشر مع محاكي طلبات حقيقي وحامي 403 الديناميكي التلقائي"""
        out_dir = "downloads"
        os.makedirs(out_dir, exist_ok=True)
        ...
        if not filename or '.' not in filename or len(filename) > 60:
            filename = f"direct_video_{task_id}.mp4"
        ...
        for attempt in range(max_attempts):
            ...
            try:
                        ...
                        while True:
                            ...
                            if q:
                                loop.call_soon_threadsafe(
                                    q.put_nowait, 
                                    ("تحميل مباشر بـ (Cloudflare Impersonate Emulator)", downloaded, totalsize, start_time, "bytes")
                                )
                break
            except urllib.error.HTTPError as e:
                # v60 Dynamic Handler for 403 / Cloudflare Challenges
                if e.code in [403, 429, 500, 502, 503, 504] and attempt < max_attempts - 1:
                    jitter = random.uniform(0.5, 2.5)
                    sleep_time = min(30, (base_delay ** attempt) + jitter)
                    logger.warning(f"🛡️ v60 Direct Downloader Auto-Fixing HTTP {e.code} / Cloudflare block. Retrying silently in {sleep_time:.2f}s...")
                    time.sleep(sleep_time)
                    # إرسال طلب جديد عبر حارس الطلبات المباشرة المحسّن
                    headers = get_random_headers(referer="https://www.google.com/")
                    continue
                raise e
            except Exception as ex:
                if attempt < max_attempts - 1:
                    jitter = random.uniform(0.5, 2.5)
                    sleep_time = min(30, (base_delay ** attempt) + jitter)
                    logger.warning(f"⚠️ Connection error on attempt {attempt + 1}: {ex}. Retrying in {sleep_time:.2f}s...")
                    time.sleep(sleep_time)
                    headers = get_random_headers()
                    continue
                raise ex
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        # v60: تضمين محاكي الهوية المتطور (generic:impersonate) وتخطي Cloudflare
        ydl_opts = {
        ...
        # v60 Dynamic Error Handler & Anti-Bot Challenge Recovery
        max_dl_retries = 4
        base_delay = 2
        for dl_attempt in range(max_dl_retries):
            try:
                ...
                if ("429" in err_str or "403" in err_str or "cloudflare" in err_str or "http error" in err_str) and dl_attempt < max_dl_retries - 1:
                    jitter = random.uniform(0.5, 2.5)
                    sleep_time = min(30, (base_delay ** dl_attempt) + jitter)
                    logger.warning(f"🛡️ v60 Auto-Bypassing Cloudflare Error ({err}). Sleep {sleep_time:.2f}s before attempt {dl_attempt + 2}...")
                    time.sleep(sleep_time)
                    
                    # تعديل رؤوس الطلب والترميز ومحاكي المتصفح دون إظهار الخطأ للمستخدم
                    new_hdrs = get_random_headers(referer="https://www.google.com/")
                    ydl_opts['http_headers'] = new_hdrs
                    ydl_opts['user_agent'] = new_hdrs['User-Agent']
                    ydl_opts['impersonate'] = random.choice(['chrome', 'chrome-120', 'edge-120'])
                    ydl_opts['extractor_args']['generic'] = {'impersonate': ['chrome']}
                    continue
                raise err
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v60 Engine - Cloudflare Anti-Bot Bypass]**\n"
                    ...
                    text = (
                        f"⚡ **[v60 Engine - Cloudflare Anti-Bot Bypass]**\n"
                        f"📌 **العملية:** {action_title}\n\n"
                        f"{bar}"
    ...
    try:
        ...
        for idx, part_file in enumerate(parts):
            ...
            caption = (
                ...
                f"🛡️ **Engine:** `v60 Engine (Cloudflare Anti-Bot Bypass)`"
            )
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت Universal Downloader & v60 Engine (Cloudflare Anti-Bot Bypass)**\n\n"
        "✨ **الحلول التقنية المضافة في هذا التحديث (v60):**\n"
        "• 🕵️‍♂️ **تضمين محاكي الهوية المتطور (generic:impersonate):** تم تمرير معاملات extractor_args المخصصة للـ generic لتخبر أداة التحميل بالتخفي الكلي ومحاكاة متصفح Chrome أو متصفحات الويب الحديثة وتخطي الجدران الحماية لـ Cloudflare تلقائياً.\n"
        "• ⚙️ **ضبط نظام الاتصال عبر الـ Cookies الصامتة ورؤوس المتصفح الحقيقية:** قمنا بتعزيز محاكي الطلبات المباشرة ليمرر تفاصيل إضافية مثل `Sec-Fetch-Site` و `Sec-Fetch-Mode` و `Accept-Language` لتخطي نظام كشف الـ Bots الذكي والترميزات المحمية بـ Cloudflare.\n"
        "• 🛡️ **تطبيق آلية حماية معالج الأخطاء الديناميكي المباشر:** عند اكتشاف البوت لـ خطأ 403 أو تحدي حظر البوتات، سيقوم تلقائياً بتجربة ترميز طلب معزز ومعدل مع إضافة وسيط فك محاكاة للمتصفح دون التسبب في إظهار الخطأ للمستخدم.\n"
        "• 📜 **تحديث السجل والتعريف البرمجي:** تم ترحيل الكود رسمياً إلى النسخة `v60 Engine (Cloudflare Anti-Bot Bypass)` مع الحفاظ الكامل على كافة الخصائص السابقة واللقطات وخيارات الضغط وأوامر الـ Trim دون المساس بها.\n"
    )
...
async def process_url_trim_task(client: Client, task_id: str, url: str, start_str: str, end_str: str, status_msg: Message):
    ...
    try:
        ...
        if task_id not in CANCELLED_TASKS:
            ...
            try:
                if "429" in str(e):
                    await status_msg.edit_text("⚠️ **تنبيه (HTTP 429):** كثرة طلبات معالجة. يرجى الانتظار دقيقة قبل إعادة المحاولة.")
                else:
...
async def handle_message(client: Client, message: Message):
    ...
    try:
        ...
        if "429" in str(e):
            await loading_msg.edit_text("⚠️ **تنبيه:** قامت المنصة بتحديد الطلبات مؤقتاً (HTTP 429). يمكنك اختيار الجودة للبدء بنمط الحماية المباشر:", reply_markup=keyboard)
        else:
...
async def process_task(client: Client, task_id: str, url: str, option: str, init_status_msg: Message):
    ...
    try:
        ...
        if task_id not in CANCELLED_TASKS:
            ...
            try:
                if "429" in str(e):
                    await init_status_msg.edit_text("⚠️ **تنبيه (HTTP Error 429):** قامت المنصة بتحديد طلبات المعالجة. تم حفظ الطلب، جرب مجدداً بعد قليل.")
                else:
                    await init_status_msg.edit_text(f"❌ **حدث خطأ أثناء التنزيل/الرفع:**\n`{str(e)[:150]}`")
            except Exception:
                pass
    finally:
        worker_task.cancel()
...
if __name__ == "__main__":
    logger.info("🚀 جاري تشغيل بوت v60 Engine (Cloudflare Anti-Bot Bypass)...")
    app.run()
