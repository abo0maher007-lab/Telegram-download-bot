...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v34.0 Engine (Dailymotion Fix + x16 Parallel)
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v34_0")
...
app = Client("UniversalDownloaderBot_v34_0", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
# ----------------------------------------------------
# 🖼️ أدوات الثمبنيل والمدة وأبعاد الفيديو v34.0
# ----------------------------------------------------
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v34.0 (Dailymotion Fix Engine)
# ----------------------------------------------------
class UniversalEngineV34:
    def __init__(self):
        self.user_agents = [
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        """استخراج معلومات المقطع والفيديو قبل التنزيل مع دعم Dailymotion الإصلاحي"""
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        def ytdl_hook(d):
            ...
            if d.get("status") in ["downloading", "finished"]:
                ...
                label = "استخراج الصوت MP3" if target_option == "mp3" else f"جاري التحميل المتوازي v34 ({target_option}p)"
        ...
        if target_option == "mp3":
            ...
            if target_option == "best":
                ...
                # تم إضافة خيارات احتياطية في حال تخصيص أبعاد غير مدمجة في Dailymotion
        ...
        # 🔧 إصلاح Dailymotion المخصص لفك المشكلة
        if "dailymotion.com" in url.lower() or "dai.ly" in url.lower():
...
engine = UniversalEngineV34()

# ----------------------------------------------------
# 🛠️ مدير الواجهة والتقدم v34.0
# ----------------------------------------------------
    ...
    while task_id not in CANCELLED_TASKS:
        try:
            ...
            if now - last_update_time >= 2.5:
                ...
                text = (
                    f"⚡ **[v34.0 Engine - Dailymotion Fixed]**\n"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v34.0 Universal Engine**\n\n"
        "✨ **تحديثات v34.0:**\n"
        "• 🛠️ **حل مشكلة Dailymotion بشكل كامل** وتجاوز خطأ `No video formats found` عبر محرك استخراج HLS/M3U8 المطور.\n"
        "• ⚡ دعم التحميل المتوازي x16 المتقدم لسرعة تحميل قصوى.\n"
        "• 🖼️ إصلاح عدم ظهور الصور المصغرة (Thumbnails) وأبعاد شاشة العرض.\n"
        "• ✂️ دعم قص الفيديوهات عبر أحدث أوامر `/trim`.\n\n"
        "💡 **مثال لاستخدام القص:**\n"
...
async def trim_url_command(client: Client, message: Message):
    ...
    if len(args) < 4:
        await message.reply_text(
            ...
            "💡 **مثال:**\n"
            "`/trim 00:10 01:30 https://example.com/video.mp4`\n"
            "`/trim 00:00:10 00:01:20 https://dailymotion.com/video/xxxx`",
            quote=True
        )
        return
...
# ----------------------------------------------------
# 📩 معالجة الفيديو الموجه والملفات (Stateless Forward Engine)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 📩 معالجة الرسائل والروابط ومدخلات القص v34.0
# ----------------------------------------------------
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v34.0 (تم حل مشكلة Dailymotion وتهيئة المحرك)...")
    app.run()
