...
# --- الإعدادات ---
VERSION = "v76.0-Railway-Font-Settings-Edition"
API_ID = int(os.environ.get("API_ID", 0))
API_HASH = os.environ.get("API_HASH", "")
BOT_TOKEN = os.environ.get("BOT_TOKEN", "")
PORT = int(os.environ.get("PORT", 8080))

# تخزين إعدادات المستخدمين في الذاكرة (مع القيم الافتراضية)
USER_SETTINGS = {}
PENDING_TASKS = {}
DIRECT_TASKS = {}
...
FONT_SIZE_MAP = {
    "small": 0.18,    # كبير نسبياً
    "medium": 0.25,   # واضح جداً
    "large": 0.35,    # ضخم
    "xlarge": 0.45    # عملاق
}
...
def get_user_config(chat_id):
    if chat_id not in USER_SETTINGS:
        USER_SETTINGS[chat_id] = {
            "snapshots": True,     # أخذ اللقطات التسع (مُفعل افتراضياً)
...
# --- استخراج 9 لقطات ألبوم من الفيديو مع تفعيل حجم الخط الخاص بالمستخدم ---
...
def build_settings_buttons(chat_id):
    ...
    buttons = [
        [
            Button.inline(f"📸 ألبوم اللقطات: {snap_status}", data="toggle_snaps")
        ],
        [
...
async def start_handler(event):
    changelog_text = (
        ...
        "📜 **سجل التحديثات والإضافات الجديد:**\n"
        "1️⃣ **لوحة إعدادات شاملة:** خيارات للتحكم في (تفعيل/تعطيل ألبوم اللقطات، خيارات الجودة، وحجم الخط) عبر `/settings`.\n"
        "2️⃣ **إلغاء العمليات:** إمكانية إيقاف وإلغاء أي عملية تنزيل أو رفع فوراً عبر زر `❌ إلغاء العملية`.\n"
        "3️⃣ **لقطات الفيديو:** استخراج 9 لقطات مقسمة على كامل أوقات الفيديو تلقائياً وإرسالها كألبوم.\n"
        "4️⃣ **توقيت اللقطات:** كتابة الوقت بوضوح أبيض شفاف في منتصف اللقطات.\n"
        "5️⃣ **شريط التقدم المفصل:** استعراض الحجم، السرعة، الوقت المنقضي والمتبقي أثناء الرفع والتنزيل.\n\n"
        "⚙️ لتعديل الخيارات والإعدادات، أرسل الأمر: `/settings`"
    )
    await event.respond(changelog_text)
...
def main():
    ...
    print("🤖 البوت يعمل بأعلى كفاءة وجاهز مع خيارات التحكم والشريط التفصيلي!")
