...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v17 Engine
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v17")
...
app = Client("UniversalDownloaderBot_v17", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v17 (Cloudflare Stealth Engine)
# ----------------------------------------------------
class UniversalEngineV17:
    def __init__(self):
        self.user_agents = [
            ...
            if d.get("status") == "downloading":
                ...
                if total > 0 and (time.time() - last_update[0] > 2):
                    ...
                    asyncio.run_coroutine_threadsafe(
                        update_progress_ui(status_msg, "جاري التجاوز والكشط (v17)", downloaded, total, start_time, task_id),
                        loop
                    )
...
async def update_progress_ui(message: Message, action_title: str, current: int, total: int, start_time: float, task_id: str):
    ...
    text = (
        f"⚡ **[v17 Stealth Engine] {action_title}**\n\n"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v17 Engine**\n\n"
        "• تحديث الاستخراج المباشر للهيكلة الديناميكية\n"
        "أرسل رابط الميديا لبدء التحميل."
    )

async def process_task(client: Client, message: Message, task_id: str, url: str):
    status_msg = await message.reply_text("🔎 **جاري تحليل الرابط بـ v17 Stealth Engine...**")
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v17 Engine...")
    app.run()