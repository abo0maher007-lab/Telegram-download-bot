...
# [الحل 10]: استيراد curl_cffi / cloudscraper لمقدمة معالجة الجلسات والطلبات المباشرة
try:
    import curl_cffi
    HAS_CURL_CFFI = True
except ImportError:
...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v63 Anti-403 & Cloudflare Shield Engine
# ----------------------------------------------------
...
# [الحل 5]: توسيع وتروير رؤوس الطلبات وقوائم User-Agents الحديثة
USER_AGENTS_POOL = [
...
# ----------------------------------------------------
# [الحل 2]: تحديث مكتبة yt-dlp تلقائياً عند تشغيل البوت
# ----------------------------------------------------
def auto_update_ytdlp():
    try:
        logger.info("🔄 [v63 Update] جاري فحص وتحديث مكتبة yt-dlp برمجياً لضمان فك حماية Cloudflare...")
        subprocess.run([os.sys.executable, "-m", "pip", "install", "--upgrade", "yt-dlp", "curl_cffi"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        logger.info("✅ [v63 Update] تم تحديث مكتبة yt-dlp والملحقات بأحدث إصدار متاحة.")
    except Exception as e:
        logger.warning(f"⚠️ فشل تحديث yt-dlp تلقائياً: {e}")
...
# ----------------------------------------------------
# [الحل 4]: إدارة الكوكيز (Cookies Management & Auto Netscape Support)
# ----------------------------------------------------
...
GENERIC_COOKIES_PATH = "cookies.txt"
...
GENERIC_COOKIE_PATH = setup_cookies("COOKIES_BASE64", GENERIC_COOKIES_PATH)
...
# ----------------------------------------------------
# 📌 تنزيل ورفع فيديوهات Dailymotion (مع دعم حلول v63 الحماية)
# ----------------------------------------------------
...
async def download_dailymotion_video(event, url, quality_choice, status_msg, user_message: Optional[Message] = None):
    ...
    # [الحل 1 + 7 + 8]: خيارات لتجاوز Cloudflare و403
    ydl_opts = {
        'format': format_str,
        ...
        'impersonate': 'chrome',  # [الحل 1]
        'extractor_args': {      # [الحل 7]
            'generic': ['impersonate'],
            'dailymotion': {
                'app_id': 'dmfed',
    ...
    try:
        ...
        if str(e) == "CANCELLED":
            ...
            await status_msg.edit_text("⚠️ **تنبيه حماية (HTTP 403/429):** قامت المنصة بتحديد الطلبات، تم تفعيل التجاوز التلقائي، جرب مجدداً.", reply_markup=None)
        else:
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v63 Anti-403 & Cloudflare Bypass Engine
# ----------------------------------------------------
class UniversalEngineV63:
    ...
    # [الحل 10]: استخدام curl_cffi أو Cloudscraper لمعالجة جلب الروابط مباشرة وتمريرها
    def fetch_url_with_custom_session(self, url: str) -> Optional[str]:
        if HAS_CURL_CFFI:
            try:
                ...
                logger.warning(f"curl_cffi fetch warning: {e}")
        if HAS_CLOUDSCRAPER:
            try:
                ...
                logger.warning(f"cloudscraper fetch warning: {e}")
        return None

    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """دالة مخصصة لتحميل الفيديوهات المباشرة بتمرير البصمات وتفادي حظر HTTP 403"""
        out_dir = "downloads"
        os.makedirs(out_dir, exist_ok=True)
        ...
        # [الحل 1 + 5 + 6 + 7]: إعدادات المحاكاة وتدوير البصمات والمحاولة
        ydl_opts = {
            'outtmpl': file_path,
            'quiet': True,
            'no_warnings': True,
            ...
            'fragment_retries': 50,
            'impersonate': 'chrome', # [الحل 1]
            'http_headers': get_random_headers(), # [الحل 5]
            'extractor_args': { 'generic': ['impersonate'] }, # [الحل 7]
            'external_downloader_args': {
                'ffmpeg': ['-headers', f'User-Agent: {random.choice(self.user_agents)}']
            }
        }

        if GENERIC_COOKIE_PATH and os.path.exists(GENERIC_COOKIE_PATH):
            ydl_opts['cookiefile'] = GENERIC_COOKIE_PATH # [الحل 4]

        if HTTP_PROXY:
            ydl_opts['proxy'] = HTTP_PROXY # [الحل 6]
        ...
        def ytdl_hook(d):
            ...
            if d.get("status") in ["downloading", "finished"]:
                ...
                q = PROGRESS_QUEUES.get(task_id)
                if q:
                    loop.call_soon_threadsafe(q.put_nowait, ("تحميل مباشر عالي السرعة (محمي ضد 403)", downloaded, total, start_time, "bytes"))
        ...
        try:
            ...
            logger.warning(f"⚠️ Fast yt-dlp direct download failed, fallback to retry mechanism: {ex}")
            ...
            # [الحل 3 + 8]: آلية إعادة المحاولة التلقائية عند الفشل (Exponential Backoff & Delay)
            headers = get_random_headers()
            max_attempts = 5
            for attempt in range(max_attempts):
                try:
                    ...
                    if e.code in [403, 429] and attempt < max_attempts - 1:
                        sleep_time = (2 ** attempt) + random.uniform(1.0, 3.0) # [الحل 3]
                        logger.warning(f"⚠️ HTTP {e.code} detected. Retrying in {sleep_time:.1f}s with new UA...")
                        time.sleep(sleep_time) # [الحل 8]
                        headers = get_random_headers() # [الحل 5]
                        continue
                    raise e
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        if is_direct_link(url) and target_option != "mp3":
            try:
                ...
                logger.warning(f"⚠️ Direct download failed, falling back to yt-dlp anti-403 engine: {ex}")
        ...
        # [تطبيق كافة حلول v63 الحزمية 1, 3, 4, 5, 6, 7, 8, 9, 10]:
        ydl_opts = {
            ...
            'retries': 50,                                # [الحل 3]
            'fragment_retries': 50,                       # [الحل 3]
            'concurrent_fragment_downloads': 8,
            'sleep_interval': 2,                          # [الحل 8]
            'max_sleep_interval': 5,                      # [الحل 8]
            'sleep_interval_requests': 1,                 # [الحل 8]
            'skip_unavailable_fragments': True,
            'geo_bypass': True,
            'impersonate': 'chrome',                      # [الحل 1]
            'http_headers': get_random_headers(),         # [الحل 5]
            'extractor_args': {
                'generic': ['impersonate'],              # [الحل 7]
                'youtube': {'player_client': ['android', 'web']} # [الحل 7]
            },
            'legacyserverconnect': True,
        }

        # [الحل 4]: الكوكيز العام
        if GENERIC_COOKIE_PATH and os.path.exists(GENERIC_COOKIE_PATH):
            ydl_opts['cookiefile'] = GENERIC_COOKIE_PATH
        ...
        # [الحل 3 + 9]: معالجة استثناءات 403 بالتحديد وتطبيق Exponential Backoff & Fallback
        max_dl_retries = 4
        for dl_attempt in range(max_dl_retries):
            ...
            except yt_dlp.utils.DownloadError as err: # [الحل 9]
                err_msg = str(err).lower()
                logger.warning(f"⚠️ DownloadError encountered (Attempt {dl_attempt+1}/{max_dl_retries}): {err}")
                
                # التقاط خطأ 403 بالتحديد وتغيير خطة التنفيذ فورا
                if "403" in err_msg or "forbidden" in err_msg or "429" in err_msg:
                    if dl_attempt < max_dl_retries - 1:
                        backoff_delay = (2 ** dl_attempt) + random.uniform(1.0, 3.0) # [الحل 3]
                        logger.info(f"🛡️ [Anti-403 Triggered] Applying Exponential Backoff Delay: {backoff_delay:.2f}s...")
                        time.sleep(backoff_delay) # [الحل 8]
                        
                        # [الحل 5]: تغيير User-Agent والجلسة
                        new_ua = random.choice(FB_USER_AGENTS if "facebook" in url_lower else self.user_agents)
                        ydl_opts['user_agent'] = new_ua
                        ydl_opts['http_headers'] = get_random_headers(for_fb=("facebook" in url_lower))
                        
                        # [الحل 9 Fallback]: تبديل التنسيقات عند تكرار الفشل
                        if dl_attempt >= 1:
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v63 Engine - Cloudflare Shield & Fast Speed]**\n"
                    ...
                    if total > 0:
                        ...
                        bar = "🔄 `جاري تدفق البيانات والمعالجة بأمان...`\n"
                        eta_str = ""

                    text = (
                        f"⚡ **[v63 Engine - Anti-403 Protected]**\n"
                        f"📌 **العملية:** {action_title}\n\n"
                        f"{bar}"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        ...
        "🛡️ **تحديثات إصدار v63 (حماية وتجاوز Cloudflare & HTTP 403):**\n"
        "• 🎭 **تفعيل المحاكاة (impersonate):** دعم بصمة المتصفحات عبر `curl_cffi` لتفادي كشف البوتات.\n"
        "• 🔄 **تحديث تلقائي للمكتبة:** تحديث تلقائي برمجياً لـ yt-dlp عند تشغيل السيرفر.\n"
        "• 🔁 **إعادة المحاولة الذكية (Exponential Backoff):** معالجة أخطاء 403 مع زيادة فترة التأخير وتدوير العناوين.\n"
        "• 🍪 **إدارة الكوكيز المحسنة:** دعم ملف `cookies.txt` وإعداده تلقائياً بصيغة Netscape.\n"
        "• 🛡️ **تعديل extractor_args و User-Agents:** تخصيص أداء الاستخراج وخفض معدل الطلبات.\n"
        "• 🔗 **محرك الطلبات المخصص:** الدمج التلقائي لـ `cloudscraper` / `curl_cffi` لتوليد الاستجابات المحمية.\n\n"
        "✨ **مع الحفاظ التام على باقي المميزات:** (الرفع التلقائي للأدمن، الضغط بـ FFmpeg، أمر /trim، وإعدادات اللقطات)."
    )
...
if __name__ == "__main__":
    logger.info("🚀 جاري تشغيل بوت v63 Anti-403 & Cloudflare Shield Engine...")
    auto_disk_guard()
    app.run()
