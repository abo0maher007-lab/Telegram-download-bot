...
async def safe_edit_message(msg, text, buttons=None):
    try:
        ...
        # منع تجميد الواجهة عند الخروج والعودة بسبب حظر التعديل المؤقت من تلغرام
...
def generate_thumbnail(video_path, thumb_path):
    """استخراج صورة مصغرة للفيديو عند الثانية 3"""
    try:
        cmd = [
...
def extract_screenshots(video_path, num_shots=4):
    """استخراج لقطات شاشة متفرقة كمعاينة"""
    saved_files = []
    try:
...
async def download_file_direct(url, status_msg, cancel_event, task_id):
    ...
    if "mega.nz" in url:
        if not m:
            return False, "خدمة Mega غير مفعلة حالياً.", None
        ...
        try:
            ...
            if downloaded_path and os.path.exists(downloaded_path):
                return True, None, downloaded_path
            return False, "فشل تنزيل ملف Mega.", None
        except Exception as e:
            return False, f"خطأ Mega: {str(e)}", None

    # 3. التحميل المباشر مع دعم 429 Retry
            ...
            async with session.get(url, allow_redirects=True) as resp:
                ...
                with open(output_path, "wb") as f:
                    async for chunk in resp.content.iter_chunked(1024 * 1024):
                        ...
                        now = time.time()
                        # التحديث كل 4 ثوانٍ لضمان استقرار الشاشة وعدم التجمد عند الخروج
...
async def process_download_job(event, url, status_msg, task_id):
    ...
    try:
        ...
        # استخراج لقطات المعاينة للـ Album في حالة الفيديوهات
        if is_video:
...
async def start_handler(event):
    ...
    welcome_text = (
        ...
        "• ☁️ دعم كامل لروابط Mega.nz وGoogle Drive.\n"
