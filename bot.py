...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v61 Engine (Full Real Browser Headers Anti-403 & Anti-429 Engine)
# ----------------------------------------------------
...
def get_real_browser_headers() -> dict:
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v61 Engine (حل مشكلة Cloudflare Anti-Bot 403 & Anti-429)
# ----------------------------------------------------
class UniversalEngineV61:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if ("429" in str(e) or "403" in str(e)) and attempt < max_retries - 1:
                    ...
                    new_headers = get_real_browser_headers()
                    ydl_opts['headers'] = new_headers
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """دالة مخصصة لتحميل الفيديوهات المباشرة وتفادي أخطاء 403 Cloudflare و 429 بترويسات متصفح حقيقي"""
        out_dir = "downloads"
        os.makedirs(out_dir, exist_ok=True)
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if (e.code in [429, 403]) and attempt < max_attempts - 1:
                    ...
                    logger.warning(f"⚠️ HTTP {e.code} detected during direct download. Retrying with updated browser headers in {sleep_time:.1f}s...")
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        browser_headers = get_real_browser_headers()
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v61 Engine - Real Browser Protected & Compressor]**\n"
                    ...
                    text = (
                        f"⚡ **[v61 Engine - Real Browser Anti-403 & 429 Protected]**\n"
                        f"📌 **العملية:** {action_title}\n\n"
                        f"{bar}"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v61 Universal Downloader & Real Browser Anti-403 Protected Engine**\n\n"
        "✨ **تحديثات الإصدار v61 الجديدة:**\n"
        "• 🌐 **حماية حقيقية كاملة لترويسات المتصفح Real Browser Headers:** إضافة طقم ترويسات حقيقي متكامل لتخطي حظر 403 و 429 على المنصات والروابط المباشرة.\n"
        "• 🛡️ **تدوير الرؤوس التلقائي:** تدوير ترويسات طلب المتصفح تلقائياً عند كشف أي محاولة تحديد طلبات مؤقتة (429/403).\n"
...
async def process_url_trim_task(client: Client, task_id: str, url: str, start_str: str, end_str: str, status_msg: Message):
    ...
    try:
        ...
        if not success or not os.path.exists(trimmed_path):
            await status_msg.edit_text("❌ **فشل في قص المقطع. تحقق من صيغة وقيم الوقت المحددة.**")
            return
        ...
        for idx, part_file in enumerate(parts):
            ...
            caption = (
                f"✂️ **تم قص الفيديو بنجاح!**{part_suffix}\n\n"
                f"⏱️ **البداية:** `{start_str}` | ⏱️ **النهاية:** `{end_str}`\n"
                f"🛡️ **Engine:** `v61 Engine`"
            )
...
@app.on_callback_query(filters.regex(r"^(cancel_|cncl_)"))
async def cancel_callback_handler(client: Client, callback: CallbackQuery):
    task_id = callback.data.split("_", 1)[1]
    CANCELLED_TASKS.add(task_id)
    
    evt = ACTIVE_CANCEL_EVENTS.get(task_id)
    if evt:
        evt.set()
...
if __name__ == "__main__":
    logger.info("🚀 جاري تشغيل بوت v61 Universal Downloader...")
    app.run()
