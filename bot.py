...
# تنزيل الملفات باستخدام yt-dlp لتجاوز حظر 403 Forbidden
async def download_with_ytdlp(url, output_path, cancel_event):
    cmd = [
        "yt-dlp",
...
async def start_handler(event):
    await event.respond("🚀 **مرحباً بك في بوت التنزيل والرفع Pro v5.5**")
...
async def process_download_job(event, url):
    ...
    status_msg = await event.respond(
        "⏳ **جاري الفحص وبدء التنزيل عبر المحرك المتقدم...**",
        buttons=get_buttons(),
    )
    thumb_path = None
    ...
    if "play.google.com" in url:
        ...
        target_url = f"https://apkpure.com/store/apps/details?id={package_id}"
    ...
    try:
        await status_msg.edit(
            f"📥 **جاري التنزيل وتجاوز الحماية:**\n`{file_name}`",
            buttons=get_buttons(),
        )

        # تنفيذ التنزيل عبر yt-dlp للتغلب على 403
        success = await download_with_ytdlp(
            target_url, file_name, cancel_event
        )
        ...
        if not success or not os.path.exists(file_name):
            await status_msg.edit(
                "❌ تعذر تنزيل الملف. قد يكون الرابط محمياً بشكل صارم أو غير مباشر.",
                buttons=None,
            )
            return
        ...
        if file_size > MAX_FILE_SIZE:
            await status_msg.edit(
                "❌ يتجاوز حجم الملف الحد المسموح به (2GB).", buttons=None
            )
            return
