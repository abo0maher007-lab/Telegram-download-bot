...
# ----------------------------------------------------
# 📊 أدوات القياس والاستخراج
# ----------------------------------------------------

def get_video_duration(video_path: str) -> int:
    """استخراج مدة الفيديو الدقيقة بالثواني باستخدام ffprobe"""
    try:
        cmd = [
            "ffprobe", "-v", "error",
        ...
        logger.warning(f"تعذر استخراج المدة عبر ffprobe: {e}")
        return 0

def scrape_direct_video_link(url: str) -> str:
    """خطة التعزيز: تحليل وقشط كود HTML باستخدام BeautifulSoup4 لاستخراج الرابط المباشر"""
    headers = {
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # 1. البحث في وسوم <video> و <source>
...
# ----------------------------------------------------
# 🧠 محرك الاستخراج - 18 استراتيجية معززة
# ----------------------------------------------------
...
async def smart_extractor(url: str, status_msg: Message, task_id: str):
    ...
    strategies = [
        ...
        ({
            ...
            **thumb_opts
        }, "3/18: Googlebot Firewall Bypass"),
        ...
        ({
            ...
            **thumb_opts
        }, "6/18: Arab Streaming Server Bypass"),
        ...
        ({
            ...
            **thumb_opts
        }, "7/18: Geo-Bypass & Legacy Connect"),
        ...
        ({
            ...
            **thumb_opts
        }, "10/18: Fallback Direct MP4 Protocol"),
        ...
        ({
            ...
            **thumb_opts
        }, "11/18: Facebook External Scraper"),
        ...
        ({
            ...
            **thumb_opts
        }, "17/18: Deep Generic Raw Extract"),
    ...
    for idx, (st_opts, st_name) in enumerate(strategies, 1):
        if task_id in CANCELLED_TASKS:
            raise ProcessCancelledException("CANCELLED_BY_USER")
        try:
            await update_status_ui(status_msg, f"جاري تجربة المحاولة ({idx}/18)...", 0, 100, time.time(), task_id)
    ...
    # 🚀 خطة التعزيز باستخدام BeautifulSoup4
    try:
        await update_status_ui(status_msg, "⚡ جاري تنفيذ خطة التعزيز وقشط الرابط المباشر (BS4)...", 0, 100, time.time(), task_id)
        scraped_url = await loop.run_in_executor(None, scrape_direct_video_link, url)
        if scraped_url:
            opts = make_ytdl_opts(strategies[8][0], status_msg, loop, task_id, "تحميل الرابط المقشوط")
            file_path, title, duration = await loop.run_in_executor(None, run_yt, scraped_url)
            if file_path and os.path.exists(file_path):
                return file_path, title, duration, "⚡ BS4 HTML Scraper Plan"
    except Exception as e:
        logger.error(f"فشلت خطة التعزيز المباشرة: {e}")
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text("👋 أهلاً بك! أرسل رابط الفيديو وسيقوم البوت باستخراجه عبر 18 محاولة وخطة قشط تعزيزية متقدمة.")
...
async def process_download(client: Client, message: Message, task_id: str, url: str):
    ...
    try:
        ...
        if file_path and os.path.exists(file_path):
            ...
            await status_msg.edit_text("❌ **فشلت جميع المحاولات والخطط التعزيزية في استخراج الفيديو.**")
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل البوت مع دعم BeautifulSoup4...")
    app.run()