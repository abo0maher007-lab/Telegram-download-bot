...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v23.0 Hybrid Engine
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v23_0")
...
app = Client("UniversalDownloaderBot_v23_0", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
def download_mediafire_file(url: str, task_id: str) -> Dict[str, Any]:
    ...
    # البحث عن الرابط المباشر داخل الصفحة
...
class UniversalEngineV23:
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        # التوجيه لمحركات MediaFire أو Mega
    ...
    while task_id not in CANCELLED_TASKS:
        try:
            ...
            if now - last_update_time >= 2.5:
                ...
                text = (
                    f"⚡ **[v23.0 Universal Hybrid Engine]**\n"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v23.0 Universal Hybrid Engine**\n\n"
        "• دعم تحميل الملفات المباشرة من **MediaFire** و **Mega.nz**.\n"
        "• لوحة التحكم بالقرص المحدثة `/disk`.\n"
        "• إرسال وتصنيف المستندات والوسائط تلقائياً.\n\n"
        "أرسل أي رابط للبدء."
    )
...
async def handle_message(client: Client, message: Message):
    ...
    # إتاحة خيار تنزيل مباشر في حال كانت روابط MediaFire أو Mega
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v23.0 Hybrid Engine...")
    app.run()
