...
logger = logging.getLogger("DownloaderBot-v2")
...
app = Client("SmartDownloaderBot_v2", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
# ----------------------------------------------------
# 📊 أدوات القياس والاستخراج (v2 Enhanced)
# ----------------------------------------------------
...
def scrape_direct_video_link_v2(url: str) -> str:
    """محرك القشط المتقدم v2 لاستخراج الرابط المباشر من الـ HTML"""
    headers = {
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # 1. فحص وسوم الفيديوهات والمصادر
            ...
            # 2. فحص وسوم الميتا og:video و twitter:player
            meta_tags = [
            ...
            for tag, attr in meta_tags:
                found = soup.find(tag, attrs=attr)
                if found and found.get('content'):
                    return found['content']

            # 3. المطابقة السريعة عبر التعابير النمطية Regex كخطة احتياطية
            matches = re.findall(r'https?://[^\s\'"]+\.(?:mp4|m3u8|webm|mov)', resp.text)
            if matches:
                return matches[0]
    except Exception as e:
        logger.warning(f"v2 Scraper failed: {e}")
    return None
...
def get_cancel_keyboard(task_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[
        InlineKeyboardButton("❌ إلغاء العملية (v2)", callback_data=f"cancel_{task_id}")
    ]])
...
async def smart_extractor_v2(url: str, status_msg: Message, task_id: str):
    ...
    for idx, (st_opts, st_name) in enumerate(strategies, 1):
        if task_id in CANCELLED_TASKS:
            raise ProcessCancelledException("CANCELLED_BY_USER")
        try:
            await update_status_ui(status_msg, f"فحص الاستراتيجية ({idx}/18)...", 0, 100, time.time(), task_id)
            ...
            logger.warning(f"فشلت المحاولة {idx}: {e}")
            continue

    # 🚀 خطة التعزيز v2 المتقدمة
    try:
        await update_status_ui(status_msg, "⚡ تشغيل خطة BS4 + Meta Scraper (v2)...", 0, 100, time.time(), task_id)
        scraped_url = await loop.run_in_executor(None, scrape_direct_video_link_v2, url)
        if scraped_url:
            opts = make_ytdl_opts(strategies[8][0], status_msg, loop, task_id, "تحميل الرابط المقشوط v2")
            file_path, title, duration = await loop.run_in_executor(None, run_yt, scraped_url)
            if file_path and os.path.exists(file_path):
                return file_path, title, duration, "⚡ BS4 + Meta Scraper v2"
    except Exception as e:
        logger.error(f"فشلت خطة التعزيز v2: {e}")
...
async def cancel_handler(client: Client, callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text("❌ **تم إلغاء العملية وتنظيف الملفات المؤقتة.**")
    except Exception:
        pass

@app.on_message(filters.command("start") & filters.private)
async def start_cmd(client: Client, message: Message):
    await message.reply_text("🚀 **مرحباً بك في البوت (إصدار v2)**\n\nأرسل رابط الفيديو الآن لبدء الفحص والاستخراج عبر المحرك الجديد.")

async def process_download(client: Client, message: Message, task_id: str, url: str):
    status_msg = await message.reply_text(
        "⏳ **[v2] جاري بدء المحرك وفحص الرابط...**",
    ...
    try:
        file_path, title, duration, used_method = await smart_extractor_v2(url, status_msg, task_id)
        ...
        if file_path and os.path.exists(file_path):
            ...
            async def upload_progress(current, total):
                ...
                if time.time() - last_up_update[0] > 2:
                    ...
                    await update_status_ui(status_msg, "جاري رفع الفيديو إلى تلجرام", current, total, upload_start, task_id)
            ...
            await status_msg.edit_text("❌ **فشلت جميع محاولات الاستخراج والتعزيز v2.**")

    except (asyncio.CancelledError, ProcessCancelledException):
        logger.info(f"🛑 إلغاء المهمة بنجاح: {task_id}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    logger.info("🚀 تم تشغيل البوت بنجاح - الإصدار v2...")
    app.run()