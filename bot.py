...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v64 Engine (Proxy Rotation Edition)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 🌐 مدير البروكسيات المباشر والتنقل الآلي (Proxy Rotation Manager)
# ----------------------------------------------------
class ProxyManager:
    def __init__(self):
    ...
    def fetch_free_proxies(self):
        now = time.time()
        if now - self.last_fetched < 1800 and self.proxies:  # جلب كل 30 دقيقة
            return

        logger.info("🔄 [v64 Proxy Manager] جاري سحب قائمة بروكسيات مجانية جديدة من المصادر...")
        fetched_list = []
        for url in self.sources:
            try:
                req = urllib.request.Request(url, headers={'User-Agent': random.choice(USER_AGENTS_POOL)})
                with urllib.request.urlopen(req, timeout=8) as resp:
                    text = resp.read().decode('utf-8', errors='ignore')
                    matches = re.findall(r'\b(?:\d{1,3}\.){3}\d{1,3}:\d{2,5}\b', text)
                    fetched_list.extend(matches)
            except Exception as e:
                logger.warning(f"⚠️ فشل سحب البروكسيات من {url}: {e}")

        with self.lock:
            if fetched_list:
                # تصفية وتجهيز العناوين بصيغة http
                unique_proxies = list(set(fetched_list))
                random.shuffle(unique_proxies)
                self.proxies = [f"http://{p}" for p in unique_proxies]
                self.last_fetched = now
                logger.info(f"✅ [v64 Proxy Manager] تم تجهيز {len(self.proxies)} بروكسي مجاني للتدوير التلقائي.")
            else:
                logger.warning("⚠️ لم يتم العثور على بروكسيات مجانية شتغالة في هذه الدورة.")

    def get_working_proxy(self) -> Optional[str]:
        if HTTP_PROXY:  # الأولوية دائماً للبروكسي المخصص في البيئة إن وجد
            return HTTP_PROXY
    ...
    def mark_proxy_failed(self, proxy_url: str):
        if not proxy_url or proxy_url == HTTP_PROXY:
            return
        with self.lock:
            if proxy_url in self.proxies:
                self.proxies.remove(proxy_url)
                logger.info(f"🗑️ [v64 Proxy Rotation] تم استبعاد البروكسي المتعطل: {proxy_url}. المتبقي: {len(self.proxies)}")
...
def clean_facebook_url(url: str) -> str:
    """تنظيف وتنسيق روابط فيسبوك لحل مشاكل Cannot parse data في yt-dlp v64"""
    clean_url = url.strip()
...
# ----------------------------------------------------
# 📌 تنزيل ورفع فيديوهات Dailymotion مع تدوير البروكسي
# ----------------------------------------------------
...
async def download_dailymotion_video(event, url, quality_choice, status_msg, user_message: Optional[Message] = None):
    ...
    try:
        ...
        def run_dl():
            nonlocal post_caption_text, current_proxy
            for retry in range(3):
                try:
                    ...
                    current_proxy = proxy_manager.get_working_proxy()
                    logger.warning(f"🔄 [Dailymotion Proxy Rotation] التبديل إلى البروكسي الجديد: {current_proxy}")
                    if retry == 2:
                        raise ex_inner
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v64 Engine - مع التدوير والتغيير الذكي للبروكسيات والالتفاف على Cloudflare
# ----------------------------------------------------
class UniversalEngineV64:
    def __init__(self):
        self.user_agents = USER_AGENTS_POOL
        self.cookie_files = {
            'instagram': IG_COOKIE_PATH,
            'twitter': TW_COOKIE_PATH,
            'pornhub': PH_COOKIE_PATH,
            'dailymotion': DM_COOKIE_PATH,
            'facebook': FB_COOKIE_PATH
        }

    def is_dailymotion_link(self, url: str) -> bool:
        return is_dailymotion_url(url)

    def _get_base_ydl_opts(self, url: str, target_option: str, out_template: str, impersonate_target: str = "chrome", active_proxy: Optional[str] = None) -> dict:
        """بناء خيارات yt-dlp المتقدمة مع تعيين البروكسي والدوران التلقائي"""
        headers = get_random_headers("facebook.com" in url.lower() or "fb.watch" in url.lower())
        ...
        # استخدام البروكسي النشط من نظام Rotation
        chosen_proxy = active_proxy or proxy_manager.get_working_proxy()
        if chosen_proxy:
            ydl_opts['proxy'] = chosen_proxy

        # تعيين الكوكيز المتاحة وفق النطاق
        url_lower = url.lower()
        if "instagram.com" in url_lower and self.cookie_files['instagram'] and os.path.exists(self.cookie_files['instagram']):
            ydl_opts['cookiefile'] = self.cookie_files['instagram']
        elif ("twitter.com" in url_lower or "x.com" in url_lower) and self.cookie_files['twitter'] and os.path.exists(self.cookie_files['twitter']):
            ydl_opts['cookiefile'] = self.cookie_files['twitter']
        elif "pornhub.com" in url_lower and self.cookie_files['pornhub'] and os.path.exists(self.cookie_files['pornhub']):
            ydl_opts['cookiefile'] = self.cookie_files['pornhub']
        elif "dailymotion.com" in url_lower and self.cookie_files['dailymotion'] and os.path.exists(self.cookie_files['dailymotion']):
            ydl_opts['cookiefile'] = self.cookie_files['dailymotion']
        elif "facebook.com" in url_lower and self.cookie_files['facebook'] and os.path.exists(self.cookie_files['facebook']):
            ydl_opts['cookiefile'] = self.cookie_files['facebook']
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        ...
        for attempt, target in enumerate(impersonate_targets):
            current_proxy = proxy_manager.get_working_proxy()
            ydl_opts = self._get_base_ydl_opts(url, "info", "", impersonate_target=target, active_proxy=current_proxy)
            ydl_opts.update({
                'skip_download': True,
                'check_formats': False,
            })
            ...
            try:
                ...
                if current_proxy:
                    proxy_manager.mark_proxy_failed(current_proxy)
                err_str = str(e).lower()
                if ("403" in err_str or "cloudflare" in err_str or "proxy" in err_str) and attempt < len(impersonate_targets) - 1:
                    logger.warning(f"🔄 [Proxy Rotation] تعثر الفحص بـ ({current_proxy}). إعادة المحاولة مع تدوير بروكسي وهدف جديد...")
                    time.sleep(1 + random.uniform(0.5, 1.0))
                    continue
                if attempt == len(impersonate_targets) - 1:
                    raise e
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        current_proxy = proxy_manager.get_working_proxy()
        ydl_opts = self._get_base_ydl_opts(url, "direct", file_path, impersonate_target="chrome", active_proxy=current_proxy)
        ydl_opts.update({
        ...
        try:
            ...
            logger.warning(f"⚠️ Direct download fast mode failed, switching to urllib/rotation: {ex}")
            if current_proxy:
                proxy_manager.mark_proxy_failed(current_proxy)
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    rot_proxy = proxy_manager.get_working_proxy()
                    opener = urllib.request.build_opener()
                    if rot_proxy:
                        proxy_handler = urllib.request.ProxyHandler({'http': rot_proxy, 'https': rot_proxy})
                    ...
                    if e.code in [403, 429] and attempt < max_attempts - 1:
                        ...
                        logger.warning(f"⚠️ HTTP {e.code} during direct download. Retrying with proxy rotation in {sleep_time}s...")
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """دالة التحميل الاستثنائية بأسلوب Async/Clean مع استراتيجية التدوير المباشر للبروكسيات (Proxy Rotation)"""
        auto_disk_guard()
        url_lower = url.lower()
        ...
        # البدائل والوضعيات المختلفة للتنقل التلقائي مع تدوير البروكسيات عند الحظر
        fallback_strategies = [
            {"impersonate": "chrome", "use_cookies": True},
            {"impersonate": "chrome:120", "use_cookies": True},
            {"impersonate": "safari", "use_cookies": False},
            {"impersonate": "edge", "use_cookies": True}
        ]
        ...
        for attempt, strategy in enumerate(fallback_strategies):
            ...
            # جلب بروكسي جديد عند كل محاولة بديلة أو فشل
            current_proxy = proxy_manager.get_working_proxy()
            ydl_opts = self._get_base_ydl_opts(url, target_option, out_template, impersonate_target=strategy["impersonate"], active_proxy=current_proxy)
            ...
            try:
                logger.info(f"🔄 محاولة التحميل ({attempt+1}/{len(fallback_strategies)}) | Impersonate: '{strategy['impersonate']}' | البروكسي المستخدم: {current_proxy or 'Direct'}")
                ...
                if "403" in err_message or "cloudflare" in err_message or "challenge" in err_message or "proxy" in err_message:
                    logger.warning(f"🔄 [Rotation Event] فشل أو حظر على البروكسي/المستهدف الحالي في المحاولة {attempt+1}. تدوير البروكسي والتجربة بالبديل التالي...")
                    time.sleep(1.5 * (attempt + 1))
                    continue
                elif ("facebook.com" in url_lower or "fb.watch" in url_lower) and attempt == 0:
...
# ----------------------------------------------------
# 🛠️ لوحات الأزرار الجودة والضغط v64
# ----------------------------------------------------
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v64 Engine - Rotation System Active]**\n"
                    ...
                    text = (
                        f"⚡ **[v64 Engine - Direct Proxy Stream]**\n"
                        f"📌 **العملية:** {action_title}\n\n"
                        f"{bar}"
...
# ----------------------------------------------------
# 📥 استقبال المعالجات العامة والروابط مع لوحة الجودة المباشرة v64
# ----------------------------------------------------
...
async def process_compression_task(client: Client, task_id: str, target_msg: Message, quality: str, status_msg: Message):
    ...
    try:
        ...
        for idx, part_file in enumerate(parts):
            ...
            part_suffix = f" (الجزء {idx+1}/{len(parts)})" if len(parts) > 1 else ""
            caption = (
                f"🗜️ **تم ضغط الفيديو بنجاح!{part_suffix}**\n\n"
                f"📊 **الحجم الأصلي:** `{format_size(orig_size)}`\n"
                f"📉 **الحجم الجديد:** `{format_size(compressed_size)}`\n"
                f"🎉 **نسبة التوفير:** `{saved_percent:.1f}%` (`{format_size(saved_bytes)}`)\n"
                f"⚙️ **المحرك:** `v64 Engine (Proxy Rotation)`"
            )
...
# ----------------------------------------------------
# ⚙️ الأوامر الأساسية للبوت
# ----------------------------------------------------
@app.on_message(filters.command("start") & filters.private)
async def start_command(client: Client, message: Message):
    welcome_text = (
        "👋 **أهلاً بك في بوت التحميل الشامل المحسّن (v64 Engine)!**\n\n"
        "✨ **المميزات المتاحة:**\n"
        "• 📥 تحميل الفيديوهات من كافة المنصات (تيك توك، إنستغرام، فيسبوك، دايلي موشن، تويتر).\n"
        "• 🌐 **نظام Proxy Rotation المباشر لتدوير وتغيير البروكسيات تلقائياً.**\n"
        "• 🛡️ **تجاوز حظر Cloudflare 403 وحماية عالية من التقيد.**\n"
        "• 🗜️ ضغط وتحويل الفيديوهات وتقليل حجمها بـ FFmpeg.\n"
        "• 📦 دعم الروابط المباشرة و Mega & MediaFire.\n"
        "• 🇦🇪 ترجمة الوصف تلقائياً إلى العربية.\n\n"
        "📌 **أرسل رابط الفيديو الآن أو قم بتحويل فيديو لضغطه مباشرة!**"
    )
...
async def settings_command(client: Client, message: Message):
    ...
    await message.reply_text("⚙️ **إعدادات التفضيلات والتقاط اللقطات:**", reply_markup=kb, quote=True)

@app.on_callback_query(filters.regex(r"^cfg_"))
async def process_settings_callback(client: Client, callback: CallbackQuery):
...
# ----------------------------------------------------
# 🏁 تشغيل البوت مع تجهيز نظام البروكسيات تلقائياً
# ----------------------------------------------------
if __name__ == "__main__":
    logger.info("🚀 جاري تشغيل البوت مع محرك v64 (Proxy Rotation Engine)...")
