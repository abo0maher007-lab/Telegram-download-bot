...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v36.0 Engine (Dailymotion & Direct Quality Matrix)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v36.0 (Dailymotion & Direct Quality Support)
# ----------------------------------------------------
class UniversalEngineV36:
    ...
    def is_dailymotion(self, url: str) -> bool:
        """فحص وتحديد ما إذا كان الرابط يتبع لمنصة Dailymotion"""
        url_lower = url.lower()
        return "dailymotion.com" in url_lower or "dai.ly" in url_lower

    def extract_info_only(self, url: str) -> Dict[str, Any]:
        """استخراج معلومات المقطع والدقات بشكل نظيف مع دعم محسن لـ Dailymotion"""
...
# ----------------------------------------------------
# 🛠️ منشئ لوحة أزرار الجودة الفائق v36.0 (Explicit Quality Grid 1080/720/480/320)
# ----------------------------------------------------
def build_quality_keyboard(req_id: str, resolutions: List[int]) -> InlineKeyboardMarkup:
    """بناء لوحة أزرار الجودة بـ 1080, 720, 480, 320 وإدراج باقي الجودات ديناميكياً بدون تكرار"""
    buttons = []
    
    # قائمة الدقات المطلوبة صراحة بالترتيب
    target_qualities = [1080, 720, 480, 320]
    
    # تجميع جميع الدقات الفريدة المتاحة والمطلوبة
    combined_res = set(target_qualities + [r for r in resolutions if isinstance(r, int)])
    unique_res = sorted(list(combined_res), reverse=True)
    
    row = []
    for res in unique_res:
        row.append(InlineKeyboardButton(f"{res}", callback_data=f"q_{res}_{req_id}"))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:
    ...
    while task_id not in CANCELLED_TASKS:
        try:
            ...
            if now - last_update_time >= 2.5:
                ...
                text = (
                    f"⚡ **[v36.0 Engine - Quality & Dailymotion]**\n"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        ...
        "✨ **تحديثات v36.0:**\n"
        "• 🎬 **إظهار أزرار الجودة القياسية صراحة (1080، 720، 480، 320)**.\n"
        "• 🌐 **دعم كامل ومتقدم للتعرف والتحميل من روابط Dailymotion** (dailymotion.com & dai.ly).\n"
        "• 🛡️ الحفاظ التام على جميع المحركات السابقة واحتساب السجل التنفيذي.\n"
    )
...
async def handle_message(client: Client, message: Message):
    ...
    is_dm = engine.is_dailymotion(url)
    status_header = "🌐 **[Dailymotion Detected]**\n" if is_dm else ""
    loading_msg = await message.reply_text(f"{status_header}🔍 **جاري فحص الرابط واستخراج معلومات الفيديو...**", quote=True)
    loop = asyncio.get_event_loop()
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v36.0 (دعم Dailymotion وإظهار أزرار الجودة 1080/720/480/320)...")
    app.run()
