...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v35.0 Engine (Dynamic Quality Keyboard Fix)
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v35_0")
...
app = Client("UniversalDownloaderBot_v35_0", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
# ----------------------------------------------------
# 🖼️ أدوات الثمبنيل والمدة وأبعاد الفيديو v35.0
# ----------------------------------------------------
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v35.0 (Quality Buttons Fix)
# ----------------------------------------------------
class UniversalEngineV35:
    def __init__(self):
        self.user_agents = [
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        """استخراج معلومات المقطع والدقات بشكل نظيف بدون تكرار"""
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        def ytdl_hook(d):
            ...
            if d.get("status") in ["downloading", "finished"]:
                ...
                label = "استخراج الصوت MP3" if target_option == "mp3" else f"جاري التحميل المتوازي v35 ({target_option}p)"
...
engine = UniversalEngineV35()

# ----------------------------------------------------
# 🛠️ منشئ لوحة أزرار الجودة الفائق (Dynamic Quality Keyboard Builder)
# ----------------------------------------------------
def build_quality_keyboard(req_id: str, resolutions: List[int]) -> InlineKeyboardMarkup:
    """بناء لوحة أزرار الجودة بشكل منظّم ومنع التكرار نهائياً"""
    buttons = []
    
    # تنظيف القائمة وتطبيق فرز تنازلي فريد
    unique_res = sorted(list(set(resolutions)), reverse=True)
    
    # تحويل الدقات المتوفرة لأزرار مع تجمعيها زرين في كل صف
    row = []
    for res in unique_res:
        row.append(InlineKeyboardButton(f"🎬 {res}p", callback_data=f"q_{res}_{req_id}"))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:
    ...
    # إذا كانت القائمة فارغة (لم تُميز yt-dlp أبعاداً محددة)، نضيف خيارات افتراضية قياسية
    if not buttons:
        buttons = [
            [InlineKeyboardButton("🎬 1080p", callback_data=f"q_1080_{req_id}"), InlineKeyboardButton("🎬 720p", callback_data=f"q_720_{req_id}")],
            [InlineKeyboardButton("🎬 480p", callback_data=f"q_480_{req_id}"), InlineKeyboardButton("🎬 360p", callback_data=f"q_360_{req_id}")]
        ]

    # الأزرار الإضافية الدائمة
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v35.0 Universal Engine**\n\n"
        "✨ **تحديثات v35.0:**\n"
        "• 🛠️ **إصلاح مشكلة تكرار وخبطة أزرار خيارات الجودة** عند جلب معلومات الفيديو.\n"
        "• 🎯 ترتيب الجودات ديناميكياً وتنظيف الخيارات بشكل متناسق.\n"
        "• ⚡ أداء فائق وتجاوز لكافة قيود السيرفرات المتنوعة.\n"
    )
...
# ----------------------------------------------------
# 📩 معالجة الرسائل والروابط ومدخلات القص v35.0
# ----------------------------------------------------
...
async def handle_message(client: Client, message: Message):
    ...
    try:
        ...
        # 🔧 الإصلاح الجوهري للواجهة: بناء لوحة الأزرار الديناميكية بدون تكرار
        keyboard = build_quality_keyboard(req_id, res_list)
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v35.0 (معالجة وإصلاح كيبورد الجودة Dynamic Keyboard Engine)...")
    app.run()
