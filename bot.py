...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v59 Engine (Advanced Anti-429 & Direct Video Downloader)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 🌐 v59: توسيع حوض متصفحات الـ User-Agents للموبايل والويب
# ----------------------------------------------------
USER_AGENTS_POOL = [
...
# ----------------------------------------------------
# ⚙️ v59: محاكي متصفح متقدم (Advanced HTTP Headers System)
# ----------------------------------------------------
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    # v59: تفعيل خيارات كسر حماية TLS والـ Dynamic/Fragment Retries
    ydl_opts = {
        'format': format_str,
...
def download_mediafire_file(url: str, target_option: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
    ...
    # v59 Jittered Exponential Backoff & Advanced HTTP Headers System
    max_attempts = 5
    base_delay = 2
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v59 Engine (مع اصلاحات دالة التحميل المباشر وحماية Anti-429)
# ----------------------------------------------------
class UniversalEngineV59:
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        """استخراج معلومات الفيديو مع حماية بصمة المتصفح JA3 والتدوير وبنية التراجع الأسّي"""
        headers = get_random_headers()
        url_lower = url.lower()
        ...
        # v59: Jittered Exponential Backoff
        max_retries = 4
        base_delay = 2
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """v59: دالة مخصصة لتحميل الفيديوهات المباشرة، مع محاكي متصفح متقدم والتراجع الأسّي التفاعلي وخداع السيرفرات"""
        out_dir = "downloads"
        os.makedirs(out_dir, exist_ok=True)
                        ...
                        while True:
                            ...
                            if q:
                                loop.call_soon_threadsafe(
                                    q.put_nowait, 
                                    ("تحميل مباشر بـ (Advanced Browser Emulator)", downloaded, totalsize, start_time, "bytes")
                                )
                break
            except urllib.error.HTTPError as e:
                if e.code in [429, 403, 500, 502, 503, 504] and attempt < max_attempts - 1:
                    jitter = random.uniform(0.5, 2.5)
                    sleep_time = min(30, (base_delay ** attempt) + jitter)
                    logger.warning(f"⚠️ HTTP {e.code} detected on attempt {attempt + 1}. Retrying with jittered backoff in {sleep_time:.2f}s...")
                    time.sleep(sleep_time)
                    headers = get_random_headers()
                    continue
                raise e
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        # v59: التحقق مما إذا كان الرابط مباشراً لملف فيديو لتحميله بدالة التحميل المباشر المحسنة
        ...
        # v59: إعدادات yt-dlp المعززة لكسر بصمة المتصفح (JA3) والتجزئة التكيفية
        ydl_opts = {
        ...
        # v59: تنفيذ الاستخراج والتنزيل مع نظام التراجع الأسّي الجيتر (Jittered Exponential Backoff)
        max_dl_retries = 4
        base_delay = 2
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v59 Engine - Anti-429 & Compressor]**\n"
                    ...
                    text = (
                        f"⚡ **[v59 Engine - Advanced Anti-429 Protected]**\n"
                        f"📌 **العملية:** {action_title}\n\n"
                        f"{bar}"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v59 Universal Downloader & Advanced Anti-429 Protected Engine**\n\n"
        "✨ **أبرز التحديثات والحلول المضافة في هذا الإصدار (v59):**\n"
        "• 🌐 **توسيع حوض متصفحات الـ User-Agents:** دعم أحدث متصفحات الموبايل (iOS/Android) والويب لخداع السيرفرات كلياً.\n"
        "• ⚙️ **محاكي متصفح متقدم (Advanced HTTP Headers System):** تزويد السيرفر ببيانات حقيقية مثل `Sec-Ch-Ua` و `Accept-Encoding` لمنع كشف الـ Scripts.\n"
        "• 🔄 **التراجع الأسّي التفاعلي (Jittered Exponential Backoff):** حماية التنزيل وإتاحة المحاولة التلقائية الذكية عند استخراج معلومات الفيديو أو التحميل المباشر.\n"
        "• 🛡️ **كسر حماية TLS/JA3:** تعطيل فحص بصمة المتصفح (Browser Fingerprinting) لمنع الحظر المؤقت.\n"
        "• 🧩 **التحميل التكيفي المقسم (Dynamic & Fragment Retries):** تقسيم أجزاء التحميل وتخفيف العبء عن السيرفرات وإعادة المحاولة عند الفشل المؤقت.\n"
        "• ⚡ **دالة التحميل المباشر المحسّنة:** تدفق سلس للبيانات مع نظام معالجة الأخطاء الذكي.\n"
        "• 🛡️ **الحفاظ التام على جميع الخصائص والميزات السابقة دون أي تغيير.**\n"
    )
...
if __name__ == "__main__":
    logger.info("🚀 جاري تشغيل بوت v59 Universal Downloader...")
    app.run()
