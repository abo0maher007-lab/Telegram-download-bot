...
# استيراد محرك curl_cffi لمحاكاة بصمات المتصفحات وتجاوز الحماية (Cloudflare & 403)
try:
...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v63 Engine (Smart Extension Bypass Layer)
# ----------------------------------------------------
...
# قائمة الامتدادات المباشرة المدعومة للفيديو
DIRECT_VIDEO_EXTENSIONS = ('.mp4', '.mkv', '.avi', '.mov', '.flv', '.webm', '.m4v', '.3gp')

def has_direct_video_ext(url: str) -> bool:
    """فحص ذكي يكشف وجود امتداد فيديو مباشر داخل الرابط حتى لو أتبِع بمعاملات مثل ?token= أو شفرات إضافية"""
    parsed_path = urlparse(url).path.lower()
    return any(ext in parsed_path for ext in DIRECT_VIDEO_EXTENSIONS)
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    ydl_opts = {
        ...
        'quiet': True,
        'no_warnings': True,
        'verbose': True, # تفعيل وضع السجل المفصل للتحقق من نجاح بصمة المتصفح
        'impersonate': 'chrome', # تفعيل محرك البصمة الذكي curl_cffi لتجاوز 403 و Cloudflare
        'headers': headers,
...
def download_mediafire_file(url: str, target_option: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
    ...
    if HAS_CURL_CFFI:
        r = cffi_requests.get(direct_url, impersonate="chrome", stream=True)
        r.raise_for_status()
        ...
        with open(file_path, 'wb') as f:
            for chunk in r.iter_content(chunk_size=8192):
                ...
                if chunk:
                    ...
                    if q:
                        label = "تحميل الفيديو من MediaFire (curl_cffi)" if target_option == "vid" else "تحميل المستند من MediaFire (curl_cffi)"
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v63 Engine (مع Cloudflare Core Bypass Layer & Direct Chrome Impersonation)
# ----------------------------------------------------
class UniversalEngineV63:
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        ...
        ydl_opts = {
            ...
            'geo_bypass': True,
            'check_formats': False,
            'verbose': True, # تفعيل السجل المفصل للتحقق من نجاح البصمة
            'impersonate': 'chrome', # استخدام بصمة المتصفح الذكية لتجاوز القيود والحظر
            'sleep_interval': 3,
            'max_sleep_interval': 6,
            'sleep_interval_requests': 2,
            'http_headers': {
        ...
        for attempt in range(max_retries):
            try:
                ...
                if ("429" in str(e) or "403" in str(e)) and attempt < max_retries - 1:
                    ...
                    ydl_opts['user_agent'] = new_ua
                    ydl_opts['http_headers']['User-Agent'] = new_ua
                    continue
                if self.is_dailymotion_link(url):
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """دالة مخصصة لتحميل الفيديوهات المباشرة وتفادي خطأ Cloudflare & 403 / 429 عبر محرك curl_cffi وبصمة Chrome الذكية"""
        out_dir = "downloads"
        os.makedirs(out_dir, exist_ok=True)
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if HAS_CURL_CFFI:
                    # توجيه طلبات الروابط المباشرة صامتاً عبر curl_cffi محاكياً بصمة Chrome لتخطي Cloudflare وحظر 403
                    r = cffi_requests.get(url, impersonate="chrome", stream=True, timeout=30)
                    r.raise_for_status()
                            ...
                            if chunk:
                                ...
                                if q:
                                    loop.call_soon_threadsafe(
                                        q.put_nowait, 
                                        ("تحميل الفيديو (curl_cffi Chrome Impersonate)", downloaded, total_size, start_time, "bytes")
                                    )
                else:
                    headers = get_random_headers()
                    opener = urllib.request.build_opener()
                    ...
                    def progress_callback(blocknum, blocksize, totalsize):
                        ...
                        q = PROGRESS_QUEUES.get(task_id)
                        if q:
                            loop.call_soon_threadsafe(q.put_nowait, ("تحميل الفيديو المباشر", downloaded, totalsize, start_time, "bytes"))
                ...
                if attempt < max_attempts - 1:
                    ...
                    logger.warning(f"⚠️ خطأ أثناء التحميل المباشر ({e}). إعادة المحاولة بعد {sleep_time:.1f} ثانية...")
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        # 🚀 فحص ذكي: إذا كان الرابط ينتهي بأي امتداد فيديو أو يحتوي عليه في مساره وتم طلب تحميل فيديو (ليس MP3)
        if has_direct_video_ext(url) and target_option != "mp3":
            try:
        ...
        ydl_opts = {
            ...
            'quiet': True,
            'no_warnings': True,
            'verbose': True, # تفعيل وضع السجل المفصل للتأكد من تمرير البصمة الذكية
            'impersonate': 'chrome', # إعداد yt-dlp لاستخدام محرك curl_cffi وتجاوز 403 و Cloudflare تلقائياً
            'nocheckcertificate': True,
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v63 Engine - curl_cffi Impersonate]**\n"
                    ...
                    text = (
                        f"⚡ **[v63 Engine - curl_cffi Chrome Bypass]**\n"
                        f"📌 **العملية:** {action_title}\n\n"
                        f"{bar}"
    ...
    try:
        ...
        for idx, part_file in enumerate(parts):
            ...
            caption = (
                ...
                f"🛡️ **Engine:** `v63 Engine (curl_cffi)`"
            )
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v63 Universal Downloader (curl_cffi Smart Bypass Layer)**\n\n"
        "✨ **تحديثات الإصدار v63 الجديدة:**\n"
        "• ⚡ **[curl_cffi Chrome Impersonate]**: توجيه الطلبات عبر بصمة المتصفح الذكية `--impersonate chrome` لتجاوز حظر 403 وحماية Cloudflare تلقائياً.\n"
        "• 🛡️ **تخطي الحظر الذكي:** استخدام محرك محاكاة بصمات TLS و JA3 الحديثة في فحص واستخراج الروابط مع تفعيل وضع السجل المفصل (`--verbose`).\n"
        "• 🎯 **الروابط المباشرة:** إزالة أزرار اختيار الجودة في الروابط المباشرة وتوجيهها فوراً لمحرك `curl_cffi` لضمان سرعة التنزيل وتخطي الحماية.\n"
        "• 🗜️ **تتبع ضغط FFmpeg التفاعلي الحقيقي:** شريط تقدم مباشر أثناء عملية ضغط الفيديو يعرض النسبة المئوية، الوقت، والسرعة.\n"
        "• 🎵🎬 **ميزة TikTok SS:** تنزيل الفيديو والصوت بشكل منفرد عند إضافة `ss` لنهاية الرابط.\n"
        "• 🛡️ **الحفاظ التام على جميع المميزات السابقة:** ديليموشن، ميديافاير، ميجا ومختلف منصات التواصل الاجتماعي دون أي تغيير.\n"
    )
        ...
        if task_id not in CANCELLED_TASKS:
            ...
            try:
                if "429" in str(e) or "403" in str(e):
                    await status_msg.edit_text("⚠️ **تنبيه (HTTP Error):** كثرة طلبات معالجة أو حماية مؤقتة. يرجى الانتظار دقيقة قبل إعادة المحاولة.")
                else:
...
# ----------------------------------------------------
# 📩 معالجة الرسائل والروابط v63 (Cloudflare & curl_cffi Impersonation)
# ----------------------------------------------------
...
async def handle_message(client: Client, message: Message):
    ...
    # 🚀 التحقق من الروابط المباشرة: إزالة أزرار اختيار الجودة تماماً وتوجيهها فوراً لمحرك curl_cffi وتخطي حماية Cloudflare و 403
    if has_direct_video_ext(raw_url):
        req_id = f"dir_{message.from_user.id}_{int(time.time())}"
        PROGRESS_QUEUES[req_id] = asyncio.Queue()
        PENDING_URLS[req_id] = raw_url
        
        status_msg = await message.reply_text(
            "⚡ **[v63 Engine - Direct Path with curl_cffi]**\n"
            "🔍 تم كشف رابط فيديو مباشر! جاري توجيه الطلب عبر بصمة متصفح Chrome لتخطي حماية Cloudflare وحظر 403...",
            quote=True
        )
        
        task = asyncio.get_running_loop().create_task(process_task(client, req_id, raw_url, "best", status_msg))
        ACTIVE_TASKS[req_id] = task
        return

    # النمط العادي للروابط الأخرى (تيك توك، فيسبوك، إلخ) التي لا تحتوي على امتدادات فيديو مباشرة
    ...
    if "mega.nz" in url_lower or "mega.co.nz" in url_lower:
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("📁 تنزيل الملف المباشر السريع", callback_data=f"q_doc_{req_id}")]
        ])
        await message.reply_text("📁 **روابط ملفات ميجا التلقائية:**", reply_markup=keyboard, quote=True)
        return

    loading_msg = await message.reply_text("🔍 **جاري فحص الرابط واستخراج معلومات الفيديو والجودات المتاحة عبر بصمة Chrome...**", quote=True)
    loop = asyncio.get_running_loop()
    ...
    try:
        ...
        if "429" in str(e) or "403" in str(e):
            await loading_msg.edit_text("⚠️ **تنبيه:** قامت المنصة بتحديد الطلبات أو حظر الوصول مؤقتمًا (HTTP 403/429). سيتم استخدام محرك `curl_cffi` التلقائي للتحميل:", reply_markup=keyboard)
        else:
        ...
        if task_id not in CANCELLED_TASKS:
            ...
            try:
                if "403" in str(e) or "429" in str(e):
                    await init_status_msg.edit_text("⚠️ **خطأ حظر أو قيود من المنصة (HTTP 403/429):** يرجى المحاولة لاحقاً.")
                else:
...
if __name__ == "__main__":
    logger.info("🚀 جاري بدء تشغيل بوت تلجرام الإصدار v63 (curl_cffi Smart Impersonation)...")
    app.run()
