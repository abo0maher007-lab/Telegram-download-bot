...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v60 Engine (Generic Impersonator & Cloudflare Bypass)
# ----------------------------------------------------
...
USER_AGENTS_POOL = [
    ...
    # Mobile Android / iOS
    'Mozilla/5.0 (Linux; Android 14; SM-S918B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/129.0.6668.70 Mobile Safari/537.36',
    'Mozilla/5.0 (Linux; Android 13; Pixel 7 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/129.0.0.0 Mobile Safari/537.36',
...
# ----------------------------------------------------
# ⚙️ v60: محاكي متصفح متقدم وضبط رؤوس الاتصال الصامتة (HTTP Headers System)
# ----------------------------------------------------
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    # v60: تفعيل محاكي الهوية المتطور وتخطي حماية Cloudflare تلقائياً
    ydl_opts = {
        'format': format_str,
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v60 Engine (تحديث دالة التحميل المباشر وتضمين محاكي generic:impersonate)
# ----------------------------------------------------
class UniversalEngineV60:
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        """استخراج معلومات الفيديو مع محاكاة الهوية وتجاوز Cloudflare"""
        headers = get_random_headers()
        url_lower = url.lower()
        ...
        for attempt in range(max_retries):
            try:
                ...
                if ("429" in str(e) or "403" in str(e) or "HTTP Error" in str(e)) and attempt < max_retries - 1:
                    jitter = random.uniform(0.5, 2.5)
                    sleep_time = min(30, (base_delay ** attempt) + jitter)
                    logger.warning(f"⚠️ Exponent Backoff retry {attempt + 1}: sleeping {sleep_time:.2f}s due to error: {e}")
                    time.sleep(sleep_time)
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """v60: دالة مخصصة لتحميل الفيديوهات المباشرة بالكامل مع حماية الأخطاء الديناميكية وتجاوز Cloudflare تلقائياً"""
        out_dir = "downloads"
        os.makedirs(out_dir, exist_ok=True)
                        ...
                        while True:
                            ...
                            if q:
                                loop.call_soon_threadsafe(
                                    q.put_nowait, 
                                    ("تحميل مباشر بـ (v60 Generic Impersonator)", downloaded, totalsize, start_time, "bytes")
                                )
                break
            except urllib.error.HTTPError as e:
                # v60: آلية حماية معالج الأخطاء الديناميكي المباشر (403 أو تحدي حظر البوتات Cloudflare)
                if e.code in [403, 429, 500, 502, 503, 504] and attempt < max_attempts - 1:
                    logger.warning(f"⚠️ اكتشاف خطأ {e.code} أو تحدي Cloudflare. جاري تعديل الترميز ومحاكاة المتصفح تلقائياً (محاولة {attempt + 1})...")
                    headers = get_random_headers()
                    headers['Referer'] = f"{parsed.scheme}://{parsed.netloc}/"
                    headers['Sec-Fetch-Site'] = 'same-origin' if attempt % 2 == 0 else 'cross-site'
                    headers['Sec-Fetch-Mode'] = 'cors' if attempt % 2 == 1 else 'navigate'
                    headers['Sec-Fetch-Dest'] = 'video' if attempt % 2 == 1 else 'document'
                    jitter = random.uniform(0.5, 2.0)
                    time.sleep(min(15, (base_delay ** attempt) + jitter))
                    continue
                
                # عند استمرار الحظر، الانتقال السلس إلى محاكي generic:impersonate عبر yt-dlp دون إظهار خطأ للمستخدم
                logger.warning("⚠️ تعذر التحميل المباشر البسيط، جاري الانتقال إلى محاكي generic:impersonate للتخفي الكلي...")
                return self._download_direct_fallback_ytdlp(url, file_path, task_id, loop)
            except Exception as ex:
                if attempt < max_attempts - 1:
                    headers = get_random_headers()
                    headers['Referer'] = f"{parsed.scheme}://{parsed.netloc}/"
                    time.sleep(1.5)
                    continue
                logger.warning(f"⚠️ استثناء في التنزيل المباشر ({ex})، التراجع السلس لمحاكي الهوية الكلي...")
                return self._download_direct_fallback_ytdlp(url, file_path, task_id, loop)
    ...
    def _download_direct_fallback_ytdlp(self, url: str, out_path: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """v60: التراجع إلى yt-dlp مع تمرير generic:impersonate لكسر حظر Cloudflare و 403 بدون أخطاء للمستخدم"""
        headers = get_random_headers()
        ydl_opts = {
            'outtmpl': out_path,
            'quiet': True,
        ...
        thumb_path = generate_ffmpeg_thumbnail(filename, task_id)
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        # v60: التحقق من الرابط المباشر للبدء بدالة التحميل المباشرة المحدثة والمحمية بـ generic:impersonate
        ...
        if any(url_lower.endswith(ext) for ext in direct_extensions) and target_option != "mp3":
            try:
                ...
                logger.warning(f"⚠️ Direct download failed, falling back seamlessly: {ex}")
        ...
        # v60: تضمين معاملات محاكي generic:impersonate والتخفي التام
        ydl_opts = {
        ...
        if "facebook.com" in url_lower or "fb.watch" in url_lower or "fb.gg" in url_lower:
            ...
            ydl_opts['extractor_args']['facebook'] = {'skip': ['hls']}
        ...
        for dl_attempt in range(max_dl_retries):
            try:
                ...
                if ("429" in str(err) or "403" in str(err) or "HTTP Error" in str(err)) and dl_attempt < max_dl_retries - 1:
                    jitter = random.uniform(0.5, 2.5)
                    sleep_time = min(30, (base_delay ** dl_attempt) + jitter)
                    logger.warning(f"⚠️ Retrying indirect download attempt {dl_attempt + 1} with sleep {sleep_time:.2f}s due to: {err}")
                    time.sleep(sleep_time)
                    new_hdrs = get_random_headers()
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v60 Engine - Generic Impersonator]**\n"
                    ...
                    text = (
                        f"⚡ **[v60 Engine - Generic Impersonator Protected]**\n"
                        f"📌 **العملية:** {action_title}\n\n"
                        f"{bar}"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت Universal Downloader - v60 Engine (Generic Impersonator & Cloudflare Protection)**\n\n"
        "✨ **الحلول التقنية والأجهزة المضافة في هذا التحديث (v60):**\n"
        "• 🎭 **تضمين محاكي الهوية المتطور (`generic:impersonate`):** تمرير معاملات `extractor_args` المخصصة للتخفي الكلي ومحاكاة Chrome وتخطي جدران Cloudflare تلقائياً.\n"
        "• 🍪 **ضبط نظام الاتصال عبر الـ Cookies الصامتة ورؤوس المتصفح الحقيقية:** تمكين `Sec-Fetch-Site` و `Sec-Fetch-Mode` و `Accept-Language` لتخضي كشف البوتات.\n"
        "• 🛡️ **آلية حماية معالج الأخطاء الديناميكي المباشر:** عند اكتشاف خطأ 403 أو تحدي حظر، يتم تلقائياً تجربة ترميز طلب معزز دون إظهار أي خطأ للمستخدم.\n"
        "• 🔄 **تكامل معالجة التراجع الأسّي التفاعلي (Jittered Exponential Backoff):** استقرار تام أثناء فحص أو تنزيل الروابط المباشرة والمعقدة.\n"
        "• ⚡ **الحفاظ التام على جميع الخصائص والميزات السابقة دون أي تغيير.**\n"
    )
...
async def process_task(client: Client, task_id: str, url: str, option: str, init_status_msg: Message):
    ...
    try:
        ...
        if task_id not in CANCELLED_TASKS:
            ...
            try:
                if "429" in str(e):
                    await init_status_msg.edit_text("⚠️ **تنبيه (HTTP 429):** كثرة طلبات معالجة. يرجى الانتظار دقيقة قبل إعادة المحاولة.")
                else:
...
# ----------------------------------------------------
# 🚀 نقطة تشغيل البوت v60
# ----------------------------------------------------
if __name__ == "__main__":
    logger.info("🤖 جاري تشغيل بوت v60 Universal Downloader Engine...")
    app.run()
