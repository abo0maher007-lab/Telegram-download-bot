...
# ----------------------------------------------------
# 🧠 محرك الاستخراج - 8 استراتيجيات متقدمة
# ----------------------------------------------------
...
async def smart_extractor(url: str, status_msg: Message, task_id: str):
    ...
    # 🚀 قائمة الاستراتيجيات الـ 8 للمواقع الصعبة والمحمية
    strategies = [
        # 1. الاستراتيجية الأولى: جودة عالية قياسية
        ({
            ...
            **thumb_opts
        }, "1/8: Standard High-Quality"),

        # 2. الاستراتيجية الثانية: محاكاة هاتف آيفون Safari (لتجاوز قيود Desktop)
        ({
            "format": "best",
            ...
            **thumb_opts
        }, "2/8: Mobile Safari Spoofing"),

        # 3. الاستراتيجية الثالثة: محاكاة Googlebot (لتجاوز حظر Cloudflare وصفحات التسجيل)
        ({
            "format": "best",
            ...
            **thumb_opts
        }, "3/8: Googlebot Bypass"),

        # 4. الاستراتيجية الرابعة: استخراج البث المباشر والملفات المدمجة (HLS/DASH Prefer MP4)
        ({
            "format": "bv*+ba/b",
            ...
            **thumb_opts
        }, "4/8: HLS/DASH Stream Extractor"),

        # 5. الاستراتيجية الخامسة: التجاوز الجغرافي القسري واستخراج الصوت والصورة المنفصلة
        ({
            ...
            **thumb_opts
        }, "5/8: Legacy Network / Bypass"),

        # 6. الاستراتيجية السادسة: محاكاة شاشات Smart TV (لتجاوز حماية منصات المسلسلات)
        ({
            "format": "best",
        ...
        # 7. الاستراتيجية السابعة: Force Generic Extractor (للمواقع المباشرة والسيرفرات الخاصة)
        ({
            "format": "best/worst",
            "outtmpl": f"downloads/{task_id}_s7.%(ext)s",
            "quiet": True, "force_generic_extractor": True, "nocheckcertificate": True,
            **thumb_opts
        }, "7/8: Direct File / Force Generic"),

        # 8. الاستراتيجية الثامنة: الوضع القسري للروابط المباشرة وبث MP4 البديل
        ({
            "format": "mp4/best",
            ...
            **thumb_opts
        }, "8/8: Fallback Direct MP4")
    ]

    # تجربة كافة الاستراتيجيات بالترتيب
    for idx, (st_opts, st_name) in enumerate(strategies, 1):
        if task_id in CANCELLED_TASKS:
            raise ProcessCancelledException("CANCELLED_BY_USER")
        try:
            # تحديث الواجهة باسم المحاولة الحالية
            await update_status_ui(status_msg, f"جاري تجربة المحاولة ({idx}/8)...", 0, 100, time.time(), task_id)
            
            opts = make_ytdl_opts(st_opts, status_msg, loop, task_id, f"تحميل الفيديو ({idx}/8)")
            def run_yt():
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text("👋 أهلاً بك! أرسل رابط الفيديو وسيقوم البوت بتجربة 8 طرق استخراج مختلفة لضمان التنزيل.")
...
async def process_download(client: Client, message: Message, task_id: str, url: str):
    ...
    try:
        ...
        if file_path and os.path.exists(file_path):
            ...
            await status_msg.edit_text("❌ **فشلت جميع المحاولات الـ 8 في استخراج الفيديو.**")
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل البوت مع 8 طرق استخراج متقدمة...")
    app.run()
