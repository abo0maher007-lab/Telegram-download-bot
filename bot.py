...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v27.0 Hybrid Engine
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v27_0")
...
app = Client("UniversalDownloaderBot_v27_0", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)

ACTIVE_TASKS = {}
CANCELLED_TASKS = set()
PENDING_URLS = {}
PENDING_CONVERSIONS = {}  # لإدارة تحويل الفيديوهات المحولة
AWAITING_TRIM_INPUT = {}  # لإدارة أدخل بداية ونهاية القص
PROGRESS_QUEUES = {}
...
# ----------------------------------------------------
# 🖼️ أدوات الثمبنيل والمدة والقص v27
# ----------------------------------------------------
...
# ----------------------------------------------------
# 🌐 محرك MediaFire Direct Downloader v26
# ----------------------------------------------------
    ...
    while task_id not in CANCELLED_TASKS:
        try:
            ...
            if now - last_update_time >= 2.5:
                ...
                text = (
                    f"⚡ **[v27.0 Universal Hybrid Engine]**\n"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v27.0 Universal Hybrid Engine**\n\n"
        "• دعم **تحويل وقص الفيديوهات المحولة للبوت** (تحويل إلى فيديو / صوت MP3 / مستند).\n"
        "• قص الفيديو بدقة بتحديد وقت البداية والنهاية.\n"
        "• دعم ملفات تصل لـ **2 جيجابايت**.\n"
        "• التعرف الذكي على روابط **MediaFire**، **Mega.nz**، ومواقع التواصل الاجتماعي.\n"
        "• نظام التحكم بالقرص `/disk` وإلغاء العمليات الفوري.\n\n"
        "أرسل رابطاً أو قم بتحويل أي فيديو للبوت للبدء."
    )
...
# ----------------------------------------------------
# 📩 معالجة الفيديو الموجه والملفات (Video Forward Engine v27)
# ----------------------------------------------------
@app.on_message(filters.private & (filters.video | filters.document))
async def handle_video_or_document(client: Client, message: Message):
    # التحقق من أن المستند هو فيديو في حالة وصوله كمستند
    is_vid = False
    file_size = 0
    if message.video:
        is_vid = True
    ...
    conv_id = f"cv_{message.from_user.id}_{int(time.time())}"
    PENDING_CONVERSIONS[conv_id] = message

    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("🎬 إرسال كـ فيديو", callback_data=f"conv_vid_{conv_id}"),
            InlineKeyboardButton("🎵 تحويل إلى MP3", callback_data=f"conv_mp3_{conv_id}"),
        ],
        [
            InlineKeyboardButton("📁 إرسال كـ مستند", callback_data=f"conv_doc_{conv_id}"),
            InlineKeyboardButton("✂️ قص وتعديل الفيديو", callback_data=f"conv_trim_{conv_id}")
        ]
    ])
...
@app.on_callback_query(filters.regex(r"^conv_"))
async def conversion_callback_handler(client: Client, callback: CallbackQuery):
    try:
        parts = callback.data.split("_")
        mode = parts[1]
        conv_id = f"{parts[2]}_{parts[3]}"

        target_msg = PENDING_CONVERSIONS.get(conv_id)
        if not target_msg:
            await callback.answer("⚠️ انتهت صلاحية الطلب، يرجى إعادة إرسال/تحويل الفيديو.", show_alert=True)
            return

        if mode == "trim":
            await callback.answer()
            user_id = callback.from_user.id
            AWAITING_TRIM_INPUT[user_id] = {"conv_id": conv_id, "status_msg": callback.message}
            await callback.message.edit_text(
                "✂️ **إعداد قص الفيديو:**\n\n"
                "أرسل أوقات القص بالشكل التالي (البداية والنهاية):\n"
                "`MM:SS MM:SS` (مثال: `00:10 01:30` لقص الفيديو من الثانية 10 حتى الدقيقة 1والثانية 30)\n"
                "أو أرسل `HH:MM:SS HH:MM:SS` للمقاطع الطويلة."
            )
            return

        PENDING_CONVERSIONS.pop(conv_id, None)
        await callback.answer()
...
async def process_video_conversion(client: Client, task_id: str, target_msg: Message, mode: str, status_msg: Message, trim_times: Optional[tuple] = None):
    ...
    try:
        ...
        # تنفيذ عمليات التعديل والقص والتحويل
        if trim_times:
...
async def handle_message(client: Client, message: Message):
    ...
    if user_id in AWAITING_TRIM_INPUT:
        ...
        if len(times) == 2:
            ...
            target_msg = PENDING_CONVERSIONS.pop(conv_id, None)
            if target_msg:
                task_id = conv_id
                PROGRESS_QUEUES[task_id] = asyncio.Queue()
    ...
    # معالجة رابط MediaFire بدقة وتفقد نوع الملف
    if "mediafire.com" in url.lower():
        try:
...
async def cancel_handler(client: Client, callback: CallbackQuery):
    try:
        ...
        # إلغاء مهمة asyncio النشطة فوراً
...
async def process_task(client: Client, task_id: str, url: str, option: str, init_status_msg: Message):
    ...
    try:
        ...
        # التحقق الفوري فور انتهاء التنزيل وقبل الرفع
        if task_id in CANCELLED_TASKS:
            raise ProcessCancelledException("CANCELLED")
        ...
        if os.path.exists(file_path):
            ...
            def upload_progress(current, total):
                # قطع عملية الرفع تلقائياً إذا تم إلغاؤها أثناء إرسال الملف
                if task_id in CANCELLED_TASKS:
                    raise ProcessCancelledException("CANCELLED")
                q = PROGRESS_QUEUES.get(task_id)
                if q:
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v27.0 Hybrid Engine...")
    app.run()
