...
# مخصصة لموقع APKMirror لاستخراج الرابط الفعلي للتنزيل
async def resolve_apkmirror_link(session, url):
    try:
        # Step 1: Request original page
        ...
        # Check if we need to follow the "Download APK" button to step 2 page
        ...
        # Step 2: Extract real download link from "here" anchor
...
async def download_file_direct(url, output_path, cancel_event, status_msg):
    ...
    async with aiohttp.ClientSession(
        ...
        # تحويل الرابط إذا كان من APKMirror
...
async def start_handler(event):
    await event.respond("🚀 **مرحباً بك في بوت التنزيل والرفع Pro v5.6**")
...
async def process_download_job(event, url):
    ...
    if "play.google.com" in url:
        ...
        if not package_id:
            await status_msg.edit("❌ تعذر معرفة ID التطبيق.", buttons=None)
            return
        ...
        # تحويل روابط Google Play تلقائياً للبحث والتحميل من APKMirror
        url = f"https://www.apkmirror.com/?post_type=app_release&searchtype=apk&s={package_id}"
    ...
    try:
        ...
        if not success:
            await status_msg.edit(f"❌ فشل التنزيل:\n`{err}`", buttons=None)
            return
