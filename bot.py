...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v14 Engine
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v14")
...
app = Client("UniversalDownloaderBot_v14", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
def setup_cookies():
    """فك تشفير كوكيز Pornhub من متغيرات البيئة بـ Base64 وحفظها مؤقتاً بتنسيق Netscape"""
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v14 Core Engine
# ----------------------------------------------------
class UniversalEngineV14:
    def __init__(self):
        self.user_agents = [
    ...
    def download_indirect_media(self, url: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
            ...
            if d.get("status") == "downloading":
                ...
                if total > 0 and (time.time() - last_update[0] > 2):
                    ...
                    asyncio.run_coroutine_threadsafe(
                        update_progress_ui(status_msg, "جاري التجاوز والكشط", downloaded, total, start_time, task_id),
                        loop
                    )

        user_agent = random.choice(self.user_agents)

        # خيارات v14 المتقدمة لتجاوز حظر 403 Forbidden
        ydl_opts = {
            ...
            'impersonate': 'chrome', # محاكاة البصمة البرمجية للمتصفحات لتجاوز Cloudflare 403
        }
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # محاولة البحث عن الصورة المصغرة
            thumb_path = None
...
async def update_progress_ui(message: Message, action_title: str, current: int, total: int, start_time: float, task_id: str):
    ...
    text = (
        f"⚡ **[v14 Bypass Engine] {action_title}**\n\n"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت التحميل والكشط v14 Engine**\n\n"
        "• تجاوز حظر 403 Forbidden تلقائياً\n"
        "• دعم البصمة البرمجية وتوليد Thumbnails\n"
        "أرسل رابط الميديا لبدء المعالجة فوراً."
    )

async def process_task(client: Client, message: Message, task_id: str, url: str):
    status_msg = await message.reply_text("🔎 **جاري تحليل الرابط وتجاوز الحماية بآلية v14...**")
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v14 Engine...")
    app.run()