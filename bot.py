...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v64 Engine (Anti-Cloudflare & Proxy Rotation)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 🌐 نظام إدارة وسحب البروكسيات المجانية وتدويرها تلقائياً
# ----------------------------------------------------
PROXIES_CACHE = []
LAST_PROXY_FETCH = 0

def fetch_free_proxies() -> List[str]:
    """سحب قائمة بروكسيات مجانية حية ومحدثة من مصادر خارجية"""
    global PROXIES_CACHE, LAST_PROXY_FETCH
    now = time.time()
    # تحديث الذاكرة كل 15 دقيقة
    if PROXIES_CACHE and (now - LAST_PROXY_FETCH < 900):
        return PROXIES_CACHE

    logger.info("🔄 جاري سحب قائمة بروكسيات مجانية محدثة...")
    sources = [
    ...
    if proxies:
        PROXIES_CACHE = list(proxies)
        LAST_PROXY_FETCH = now
        logger.info(f"✅ تم الحصول على {len(PROXIES_CACHE)} بروكسي بنجاح.")
    return PROXIES_CACHE

def get_next_proxy() -> Optional[str]:
    """إعادة بروكسي عشوائي من القائمة أو الممرر من البيئة"""
    if HTTP_PROXY:
        return HTTP_PROXY
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v64 Engine (تجاوز Cloudflare + تدوير البروكسي)
# ----------------------------------------------------
class UniversalEngineV63:
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        ...
        ydl_opts = {
            ...
            'http_headers': get_random_headers(for_fb=is_fb),
            'geo_bypass': True,
            'check_formats': False,
            'sleep_interval': 3,
            'max_sleep_interval': 6,
        ...
        if is_fb:
            ydl_opts['extractor_args'].update({'facebook': {'skip': ['dash', 'hls']}})
    ...
    # =========================================================================
    # 🔄 دالة التحميل المحدثة كلياً - التجاوز التراتبي لـ Cloudflare والبروكسيات
    # =========================================================================
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        # 1. إعدادات yt-dlp الأساسية (YDL Options) لحماية التصفح ومحاكاة متصفح حقيقي
        ydl_opts = {
            ...
            'skip_unavailable_fragments': True,
            'geo_bypass': True,
            # محاكاة بصمة curl_cffi لتجاوز حظر Cloudflare
            'impersonate': 'chrome',
            'extractor_args': {
                'generic': ['impersonate'],
                'youtube': ['player_client=android,web']
            },
            # إضافة رؤوس طلبات HTTP كاملة لمطابقة المتصفح الحقيقي
            'http_headers': get_random_headers(for_fb=("facebook.com" in url_lower or "fb.watch" in url_lower)),
            'legacyserverconnect': True,
        }

        # تخصيص الإعدادات بحسب المنصة
        if self.is_dailymotion_link(url):
        ...
        # 2 & 3. خوارزمية Retry & Fallback + Proxy Rotation لتجاوز الحظر تلقائياً وتراتبيًا
        fallback_impersonates = ["chrome", "chrome:120", "safari"]
        attempts = 0
        max_attempts = 6

        while attempts < max_attempts:
            try:
                logger.info(f"🔄 محاولة التحميل رقم [{attempts + 1}/{max_attempts}] بإعدادات (Impersonate: {ydl_opts.get('impersonate')})")
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    info = ydl.extract_info(url, download=True)
                ...
                err_str = str(err)
                logger.warning(f"⚠️ خطأ أثناء التنزيل بالمحاولة {attempts + 1}: {err_str}")
                attempts += 1

                # التقاط خطأ 403 أو حظر Cloudflare أو تقييد المعدل 429
                if any(k in err_str.lower() for k in ["403", "cloudflare", "challenge", "429", "bot"]):
                    # أ) التبديل التراتبي لإعدادات impersonate و user-agent
                    new_imp = fallback_impersonates[(attempts) % len(fallback_impersonates)]
                    ydl_opts['impersonate'] = new_imp
                    ydl_opts['http_headers'] = get_random_headers(for_fb=("facebook.com" in url_lower))
                    
                    # ب) تدوير وتفعيل بروكسي جديد تلقائياً عند استمرار الفشل
                    rotated_proxy = get_next_proxy()
                    if rotated_proxy:
                        logger.info(f"🌐 تفعيل بروكسي جديد لتجاوز Cloudflare: {rotated_proxy}")
                        ydl_opts['proxy'] = rotated_proxy

                    # ج) تبسيط خيارات صيغة التحميل لتسهيل العملية
                    if ("facebook.com" in url_lower or "fb.watch" in url_lower) and attempts >= 2:
                        ydl_opts['format'] = 'best' if not is_audio else 'bestaudio/best'

                    time.sleep(2)
                    continue
                else:
                    # استثناء خفيف لتجربة بروكسي آخر بدلاً من رفع استثناء مباشر يظهر للمستخدم
        ...
        raise Exception("فشل البوت في تجاوز حماية المنصة بعد عدة محاولات متكررة واستخدام البروكسيات.")
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v64 Engine - Cloudflare Bypass Protected]**\n"
...
# ----------------------------------------------------
# 📥 استقبال المعالجات العامة والروابط مع لوحة الجودة المباشرة
# ----------------------------------------------------
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v64 Universal Downloader & Direct Stream Engine**\n\n"
        "✨ **تحديثات الإصدار v64 الجديدة:**\n"
        "• 🛡️ **تجاوز نهائي لحظر Cloudflare:** محاكاة المتصفحات باستخدام `curl_cffi` وإعدادات `impersonate` المتطورة.\n"
        "• 🔄 **تدوير البروكسيات المجانية الحية:** سحب قائمة بروكسيات وتجربتها تلقائياً عند فشل الطلب لمنع حجب الخدمة.\n"
        "• ⚡ **لوحة الجودات المباشرة:** إظهار لوحة اختيار الجودة فوراً لكافة الروابط دون انتظار التحليل.\n"
        "• 🔵 **دعم شامل للمنصات:** فيسبوك، انستغرام، تيك توك، تويتر، يوتيوب، وديليموشن.\n"
        "• 🛡️ **الحفاظ التام على كل الخصائص:** (إعادة التوجيه للأدمن، خيارات اللقطات، أمر /trim، وضغط FFmpeg).\n"
    )
...
if __name__ == "__main__":
    logger.info("🚀 جاري بدء تشغيل بوت Universal Downloader Bot (v64 Engine with Anti-Cloudflare & Proxy Rotation)...")
    app.run()
