...
# ----------------------------------------------------
# 🧠 المحرك الشامل v58 Engine (مع دالة التحميل المباشر وتفادي الحظر Anti-429 & Cloudflare Bypasser)
# ----------------------------------------------------
class UniversalEngineV58:
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        ...
        # إعادة المحاولة بتأخير عشوائي عند الخطأ 429 أو Cloudflare
        max_retries = 3
        for attempt in range(max_retries):
            try:
                ...
                if ("429" in str(e) or "403" in str(e)) and attempt < max_retries - 1:
                    ...
                    continue
                if self.is_dailymotion_link(url):
                    ydl_opts['extractor_args'].pop('dailymotion', None)
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """دالة مخصصة لتحميل الفيديوهات المباشرة وتفادي خطأ HTTP 429 & Cloudflare"""
        out_dir = "downloads"
        os.makedirs(out_dir, exist_ok=True)
        ...
        # دعم الإعادة والتحمل عند مواجهة HTTP Error 429 أو 403
        max_attempts = 4
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        # التحقق مما إذا كان المسار ينتهي بملف فيديو حتى مع وجود معاملات query strings
        direct_extensions = ('.mp4', '.mkv', '.avi', '.mov', '.flv', '.webm', '.m4v', '.3gp')
        parsed_path = urlparse(url_lower).path
        if any(parsed_path.endswith(ext) for ext in direct_extensions) and target_option != "mp3":
            try:
                return self.download_direct_url(url, task_id, loop)
            except Exception as ex:
                logger.warning(f"⚠️ Direct download failed, falling back to yt-dlp with impersonate: {ex}")
        ...
        # تنفيذ معالجة الاستخراج مع نظام إعادة التجريد عند مواجهة خطأ 429 أو 403
        max_dl_retries = 3
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        ...
        "• 🛡️ **حل مشكلة HTTP Error 429 & 403:** إضافة تدوير مخصص للرؤوس (Header Rotation) وانتحال متصفح تلقائي لتجاوز حماية Cloudflare.\n"
...
async def handle_message(client: Client, message: Message):
    ...
    try:
        ...
        if "429" in str(e) or "403" in str(e):
            await loading_msg.edit_text("⚠️ **تنبيه:** قامت المنصة بتحديد الطلبات مؤقتاً أو تفعيل حماية Cloudflare. يمكنك اختيار الجودة للبدء بنمط الحماية المباشر:", reply_markup=keyboard)
        else:
...
async def process_task(client: Client, task_id: str, url: str, option: str, init_status_msg: Message):
    ...
    try:
        ...
        if task_id not in CANCELLED_TASKS:
            ...
            try:
                if "429" in str(e) or "403" in str(e):
                    await init_status_msg.edit_text("⚠️ **تنبيه (HTTP Error):** قامت المنصة بتحديد الطلبات مؤقتاً أو تفعيل حماية Cloudflare. جرب مجدداً بعد دقيقة.")
                else:
