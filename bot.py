...
# --- إضافة استيراد curl_cffi للتمويه المحترف ---
try:
...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v63 Engine (Impersonate & Curl-CFFI)
# ----------------------------------------------------
...
# إعداد البروكسي الخاص بـ Oxylabs لتخطي الحظر
OXYLABS_PROXY = "http://abomaher_H8S06:AboMaher2027A2027=@pr.oxylabs.io:7777"
HTTP_PROXY = os.environ.get("HTTP_PROXY", OXYLABS_PROXY) 
...
# قائمة خيارات impersonate المتاحة
IMPERSONATE_CHOICES = ["chrome120", "chrome110", "safari15_5", "chrome119", "edge101"]
...
async def download_dailymotion_video(event, url, quality_choice, status_msg, user_message: Optional[Message] = None):
    ...
    ydl_opts = {
        'format': format_str,
        'proxy': HTTP_PROXY,
        'impersonate': random.choice(IMPERSONATE_CHOICES),  # خيار التمويه التلقائي
...
# ----------------------------------------------------
# 🌐 محرك MediaFire & Mega Direct Downloader (محدث بـ curl_cffi)
# ----------------------------------------------------
def inspect_mediafire_link(url: str) -> Dict[str, Any]:
    if HAS_CURL_CFFI:
        html = curl_requests.get(url, impersonate="chrome120", proxies={"http": HTTP_PROXY, "https": HTTP_PROXY}).text
    else:
        ua = random.choice(USER_AGENTS_POOL)
        req = urllib.request.Request(url, headers={'User-Agent': ua})
...
# ----------------------------------------------------
# 🧠 المحرك الشامل (v63 Engine مع التمويه)
# ----------------------------------------------------
class UniversalEngineV63:
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """دالة مخصصة لتحميل الفيديوهات المباشرة بتمويه كأنها قادمة من متصفح حقيقي وتفادي الحظر"""
        out_dir = "downloads"
        os.makedirs(out_dir, exist_ok=True)
        ...
        # إعداد yt_dlp مع خيار التمويه المباشر
        ydl_opts = {
            'outtmpl': file_path,
            'proxy': HTTP_PROXY,
            'impersonate': random.choice(IMPERSONATE_CHOICES),  # تمويه المتصفح بـ Chrome/Safari
            'quiet': True,
            'no_warnings': True,
            'nocheckcertificate': True,
        ...
        def ytdl_hook(d):
            ...
            if d.get("status") in ["downloading", "finished"]:
                ...
                q = PROGRESS_QUEUES.get(task_id)
                if q:
                    loop.call_soon_threadsafe(q.put_nowait, ("تحميل مباشر عالي السرعة (تمويه)", downloaded, total, start_time, "bytes"))
        ...
        try:
            ...
            logger.warning(f"⚠️ Fast yt-dlp direct download failed, fallback to curl_cffi/urllib: {ex}")
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if HAS_CURL_CFFI:
                        with curl_requests.get(url, impersonate="chrome120", stream=True, proxies={"http": HTTP_PROXY, "https": HTTP_PROXY}) as resp:
                            ...
                            with open(file_path, 'wb') as f:
                                for chunk in resp.iter_content(chunk_size=65536):
                                    ...
                                    if chunk:
                                        ...
                                        q = PROGRESS_QUEUES.get(task_id)
                                        if q:
                                            loop.call_soon_threadsafe(q.put_nowait, ("تحميل مباشر عبر Curl-CFFI", dl_bytes, total_len, start_time, "bytes"))
                        break
                    else:
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        ydl_opts = {
            ...
            'proxy': HTTP_PROXY,
            'impersonate': random.choice(IMPERSONATE_CHOICES),  # تمويه الطلب بمتصفح حقيقي لجميع المواقع
        ...
        for dl_attempt in range(max_dl_retries):
            try:
                ...
                if "429" in str(err) and dl_attempt < max_dl_retries - 1:
                    ...
                    ydl_opts['impersonate'] = random.choice(IMPERSONATE_CHOICES)
                    continue
                raise err
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v63 Engine - Impersonate Protected]**\n"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        ...
        "• 🕵️ **تقنية التمويه (Impersonate):** تمويه الطلبات كأنها صادرة من متصفح Chrome/Safari حقيقي لتجاوز الحظر.\n"
        "• ⚡ **دعم مكتبة `curl-cffi`:** استخدام TLS/JA3 Fingerprint spoofing لمنع اكتشاف أدوات التنزيل.\n"
        "• 🌐 **تعديل وتكامل الرؤوس:** إرسال كامل رؤوس `Sec-Ch-Ua` للحماية من حظر الـ User-Agent السطحي.\n"
        "• 🛡️ **حماية ضد خطأ HTTP 429:** تدوير تلقائي للمتصفحات والمحاكاة عند وجود ضغط على السيرفرات.\n"
    )
...
if __name__ == "__main__":
    logger.info("🚀 جاري تشغيل بوت v63 Universal Downloader Engine مع تقنية Impersonate...")
    auto_disk_guard()
    app.run()
