...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v65 Engine (Fast Quality & Proxy Rotation Edition)
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v65")
...
app = Client("UniversalDownloaderBot_v65", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
class ProxyManager:
    ...
    def fetch_free_proxies(self):
        ...
        logger.info("🔄 [Proxy Manager] جاري سحب قائمة بروكسيات مجانية جديدة...")
        fetched_list = []
        ...
        with self.lock:
            if fetched_list:
                ...
                logger.info(f"✅ [Proxy Manager] تم تجهيز {len(self.proxies)} بروكسي مجاني.")
            else:
                logger.warning("⚠️ لم يتم العثور على بروكسيات مجانية شغالة في هذه الدورة.")
    ...
    def mark_proxy_failed(self, proxy_url: str):
        ...
        with self.lock:
            if proxy_url in self.proxies:
                ...
                logger.info(f"🗑️ [Proxy Rotation] استبعاد البروكسي: {proxy_url}. المتبقي: {len(self.proxies)}")
...
def auto_disk_guard():
    ...
    if info["free_gb"] < 0.5 or info["downloads_mb"] > 2000:
        logger.warning("🧹 تنظيف آلي: تنظيف مجلد التنزيل المؤقت...")
...
# ----------------------------------------------------
# 🖼️ أدوات المعالجة والتحويل والترجمة
# ----------------------------------------------------
...
def compress_video_ffmpeg(input_path: str, target_quality: str, output_path: str, task_id: Optional[str] = None, loop: Optional[asyncio.AbstractEventLoop] = None) -> bool:
    try:
        ...
        logger.error(f"❌ استثناء غير متوقع أثناء الضغط: {e}")
        return False

# ----------------------------------------------------
# 🌐 محرك الترجمة العربية
# ----------------------------------------------------
...
# ----------------------------------------------------
# 🧠 المحرك الشامل المحدث v65 (معالجة سرعة الجودة العالية)
# ----------------------------------------------------
class UniversalEngineV65:
    def __init__(self):
    ...
    def _get_base_ydl_opts(self, url: str, target_option: str, out_template: str, impersonate_target: str = "chrome", active_proxy: Optional[str] = None) -> dict:
        ...
        ydl_opts = {
            ...
            'geo_bypass': True,
            'socket_timeout': 15,            # مهلة سريعة للأنبوب
            'retries': 10,
            'fragment_retries': 10,
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        if is_direct_link(url) and target_option != "mp3":
            try:
                ...
                logger.warning(f"⚠️ Direct download failed: {ex}")
        ...
        if target_option == "mp3":
            ...
            # 🔥 الإصلاح الرئيسي: صيغة سريعة جداً وتجنب ثقل خيار best المسبب للتأخير والخطأ 🔥
            if target_option in ["best", "bestp"]:
                format_selector = 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best'
            else:
                format_selector = f'bestvideo[height<={target_option}][ext=mp4]+bestaudio[ext=m4a]/best[height<={target_option}][ext=mp4]/best'
        ...
        for attempt, strategy in enumerate(fallback_strategies):
            ...
            if "facebook.com" in url_lower or "fb.watch" in url_lower or "fb.gg" in url_lower:
                ...
                ydl_opts['http_headers'] = get_random_headers(for_fb=True)
...
engine = UniversalEngineV65()

# ----------------------------------------------------
# 🛠️ لوحات الأزرار للجودة والضغط
# ----------------------------------------------------
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        text = (
                            f"⚙️ **[v65 Engine - Optimized Stream]**\n"
                            f"📌 **العملية:** {action_title}\n\n"
                            f"{bar}"
                            f"⏱️ **المنقضي:** `{format_time(current)}` / `{format_time(total)}`\n"
                            f"🚀 **سرعة الضغط:** `{speed:.2f}x` | ⏳ **المتبقي:** `{format_time(eta)}`\n\n"
                            f"📤 **الخطوة التالية:** سيتم إعادة رفع الفيديو تلقائياً."
                        )
                    else:
    ...
    try:
        ...
        for idx, part_file in enumerate(parts):
            ...
            caption = (
                ...
                f"⚙️ **المحرك:** `v65 Engine (Fast Quality)`"
            )
...
async def start_command(client: Client, message: Message):
    welcome_text = (
        "👋 **أهلاً بك في بوت التحميل الشامل المحسّن (v65 Engine)!**\n\n"
        "✨ **المميزات المتاحة:**\n"
        "• 📥 تحميل الفيديوهات من كافة المنصات السرعة القصوى لزر أفضل جودة.\n"
...
if __name__ == "__main__":
    logger.info("🚀 جاري تشغيل البوت مع محرك v65 المحسّن...")
