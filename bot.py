...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v60 Engine (Real Browser Injection & Ultra Anti-403/429)
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v60")
...
app = Client("UniversalDownloaderBot_v60", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
# ----------------------------------------------------
# 🌐 ترويسات متصفح حقيقي متكاملة (Real Browser Headers Engine)
# ----------------------------------------------------
BROWSER_PROFILES = [
    {
    ...
    {
        "user_agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; edg/128.0.0.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36 Edg/128.0.0.0",
        "sec_ch_ua": '"Microsoft Edge";v="128", "Chromium";v="128", "Not=A?Brand";v="24"',
        "platform": '"Windows"',
        "mobile": "?0"
    },
    {
        "user_agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:129.0) Gecko/20100101 Firefox/129.0",
        "sec_ch_ua": None,  # Firefox does not use sec-ch-ua by default
        "platform": '"Windows"',
        "mobile": "?0"
    }
]
...
# ----------------------------------------------------
# 🖼️ أدوات الثمبنيل والمدة وأبعاد الفيديو والترجمة والضغط v60
# ----------------------------------------------------
...
def compress_video_ffmpeg(input_path: str, target_quality: str, output_path: str, task_id: Optional[str] = None, loop: Optional[asyncio.AbstractEventLoop] = None) -> bool:
    try:
        ...
        logger.info(f"⚙️ جاري تنفيذ أمر الضغط بـ FFmpeg v60: {' '.join(cmd)}")
...
def is_direct_download_url(url: str) -> bool:
    url_lower = url.lower()
    if any(x in url_lower for x in ["mediafire.com", "mega.nz", "mega.co.nz"]):
        return True
    direct_extensions = ('.mp4', '.mkv', '.avi', '.mov', '.flv', '.webm', '.m4v', '.3gp', '.mp3', '.zip', '.rar', '.pdf')
    return any(url_lower.endswith(ext) or ext + "?" in url_lower for ext in direct_extensions)
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    headers = get_random_headers('https://www.dailymotion.com/')
    ...
    try:
        ...
        elif "429" in str(e) or "403" in str(e):
            await status_msg.edit_text("⚠️ **تنبيه (HTTP Error 403/429):** قامت المنصة بتحديد عدد الطلبات مؤقتاً. تم التراجع بأمان، جرب مجدداً بعد دقيقة.", reply_markup=None)
        else:
...
def inspect_mediafire_link(url: str) -> Dict[str, Any]:
    headers = get_random_headers('https://www.mediafire.com/')
    req = urllib.request.Request(url, headers=headers)
...
def download_mediafire_file(url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
    ...
    def download_hook(blocknum, blocksize, totalsize):
        ...
        if q:
            label = "تحميل الملف المباشر من MediaFire"
    ...
    return {
        "file_path": file_path,
        "title": file_name,
        "duration": get_media_duration(file_path) if info["is_video"] else 0,
        "thumb_path": generate_ffmpeg_thumbnail(file_path, task_id) if info["is_video"] else None,
        "description": "",
        "is_audio": False,
        "is_document": not info["is_video"]
    }
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v60 Engine (Real Browser Injection & Anti 403/429)
# ----------------------------------------------------
class UniversalEngineV60:
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        ...
        if self.is_dailymotion_link(url):
            ydl_opts.update({
                'http_headers': get_random_headers('https://www.dailymotion.com/'),
                'extractor_args': {
                    'dailymotion': {
                        'app_id': 'dmfed',
                        'geo_verification_network': 'http'
        ...
        for attempt in range(max_retries):
            try:
                ...
                if ("429" in err_str or "403" in err_str) and attempt < max_retries - 1:
                    ...
                    ydl_opts['http_headers'] = get_random_headers(referer)
                    continue
                if self.is_dailymotion_link(url):
                    ydl_opts['extractor_args'] = {}
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        if not filename or '.' not in filename:
            filename = f"direct_file_{task_id}.mp4"
        ...
        def progress_callback(blocknum, blocksize, totalsize):
            ...
            q = PROGRESS_QUEUES.get(task_id)
            if q:
                loop.call_soon_threadsafe(q.put_nowait, ("تنزيل مباشر للفيلم/الملف", downloaded, totalsize, start_time, "bytes"))
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if (e.code == 429 or e.code == 403) and attempt < max_attempts - 1:
    ...
    def _sync_fallback_stream_download(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> str:
        """بديل للتدفق المباشر بشكل متزامن أمن ينفذ داخل الـ Executor بأسلوب صحيح يمنع الكراش"""
        out_dir = "downloads"
        os.makedirs(out_dir, exist_ok=True)
        file_path = os.path.join(out_dir, f"{task_id}_stream_fallback.mp4")
        ...
        with urllib.request.urlopen(req) as response:
            ...
            with open(file_path, 'wb') as f:
                while True:
                    ...
                    q = PROGRESS_QUEUES.get(task_id)
                    if q:
                        loop.call_soon_threadsafe(q.put_nowait, ("تنزيل بالنمط الآمن البديل (Stream Fallback)", downloaded, total_size, start_time, "bytes"))
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        if "mediafire.com" in url_lower:
            return download_mediafire_file(url, task_id, loop)
        if "mega.nz" in url_lower or "mega.co.nz" in url_lower:
            return download_mega_file(url, task_id)

        if is_direct_download_url(url) and target_option != "mp3":
            try:
        ...
        if self.is_dailymotion_link(url):
            ...
            ydl_opts['http_headers'] = get_random_headers('https://www.dailymotion.com/')
            ydl_opts['extractor_args'] = {
                'dailymotion': {
                    'app_id': 'dmfed',
                    'geo_verification_network': 'http'
        ...
        if "facebook.com" in url_lower or "fb.watch" in url_lower or "fb.gg" in url_lower:
            ...
            ydl_opts.update({
                ...
                'http_headers': get_random_headers('https://www.facebook.com/')
            })

        if "tiktok.com" in url_lower:
            ydl_opts['extractor_args'] = {'tiktok': {'app_version': '1.0.0'}}
            ydl_opts['http_headers'] = get_random_headers('https://www.tiktok.com/')
        
        if "instagram.com" in url_lower:
            ydl_opts['http_headers'] = get_random_headers('https://www.instagram.com/')
            if IG_COOKIE_PATH and os.path.exists(IG_COOKIE_PATH):
                ydl_opts['cookiefile'] = IG_COOKIE_PATH
        elif ("twitter.com" in url_lower or "x.com" in url_lower):
            ydl_opts['http_headers'] = get_random_headers('https://x.com/')
            if TW_COOKIE_PATH and os.path.exists(TW_COOKIE_PATH):
                ydl_opts['cookiefile'] = TW_COOKIE_PATH
        elif "pornhub.com" in url_lower and PH_COOKIE_PATH and os.path.exists(PH_COOKIE_PATH):
            ydl_opts['cookiefile'] = PH_COOKIE_PATH
        ...
        for dl_attempt in range(max_dl_retries):
            try:
                ...
                last_exception = err
                err_str = str(err)
                if ("429" in err_str or "403" in err_str) and dl_attempt < max_dl_retries - 1:
                    time.sleep(3 * (dl_attempt + 1) + random.uniform(0.5, 1.5))
                    ydl_opts['http_headers'] = get_random_headers(referer)
                    continue

        try:
            logger.warning("⚠️ Trying Flexible yt-dlp Fallback Strategy...")
            fallback_opts = {
                'format': 'best',
                'outtmpl': out_template,
                'quiet': True,
                'no_warnings': True,
                'nocheckcertificate': True,
                'http_headers': get_random_headers(referer)
            }
            if HTTP_PROXY: fallback_opts['proxy'] = HTTP_PROXY

            with yt_dlp.YoutubeDL(fallback_opts) as ydl_flex:
                info = ydl_flex.extract_info(url, download=True)
                filename = ydl_flex.prepare_filename(info)
                return {
                    "file_path": filename,
                    "title": str(info.get('title', 'Media File')),
            ...
            logger.warning(f"⚠️ Flexible Fallback failed: {flex_err}")

        try:
            logger.warning("🚀 Activating Ultimate Strategy 3: Sync Stream Fallback...")
            fallback_path = self._sync_fallback_stream_download(url, task_id, loop)
            
            if os.path.exists(fallback_path) and os.path.getsize(fallback_path) > 0:
                duration = get_media_duration(fallback_path)
                thumb = generate_ffmpeg_thumbnail(fallback_path, task_id)
                return {
                    "file_path": fallback_path,
                    "title": f"Media_{task_id}",
                    "duration": duration,
                    "thumb_path": thumb,
                    "description": "",
                    "is_audio": False,
            ...
            logger.error(f"❌ All Download Strategies Exhausted. Final Error: {async_err}")

        if last_exception:
            raise last_exception
        raise Exception("فشل تحميل الفيديو بجميع الطرق والاستراتيجيات المتاحة.")

engine = UniversalEngineV60()

# ----------------------------------------------------
# 🛠️ لوحات الأزرار الجودة والضغط v60
# ----------------------------------------------------
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v60 Engine - Real Browser Injection]**\n"
                    ...
                    text = (
                        f"⚡ **[v60 Engine - Real Browser Injection]**\n"
                        f"📌 **العملية:** {action_title}\n\n"
                        f"{bar}"
...
# ----------------------------------------------------
# 🗜️ معالجة الفيديوهات المحولة والمباشرة للضغط (v60 Engine)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 📡 الأحداث والأوامر v60
# ----------------------------------------------------
@app.on_message(filters.command(["start", "."]) | filters.regex(r"^/$") & filters.private)
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v60 Universal Downloader - Multi-Tier Download Engine**\n\n"
        "✨ **تحديثات الإصدار v60 الجديدة:**\n"
        "• ⚡ **التحميل المباشر التلقائي:** إلغاء لوحة الخيارات للروابط المباشرة و MediaFire و Mega والتحميل فجأة بشكل مباشر.\n"
        "• 🛡️ **حقن ترويسات متصفح حقيقي (Real Browser Headers):** دعم كامل لترويسات Client Hints (sec-ch-ua, Sec-Fetch) لتجاوز أخطاء 403 Forbidden و 429 Too Many Requests.\n"
        "• 🗜️ **شريط التقدم التفاعلي الحقيقي:** متابعة حية لعمليات الرفع، التنزيل، وضغط FFmpeg بنفس أسلوب الأداء الرائد.\n"
        "• 📺 **ترميز `yuv420p` المعياري:** تشغيل كلي لجميع المقاطع المضغوطة على المشغلات والهواتف الذكية.\n"
        "• 🎵🎬 **ميزة TikTok SS:** تنزيل الفيديو والصوت بشكل منفرد عند إضافة `ss` لنهاية الرابط.\n"
        "• ✂️ **أمر القص Trim:** اقتطاع أي مقطع حسب الرغبة باستخدام `/trim`.\n"
    )
...
async def process_url_trim_task(client: Client, task_id: str, url: str, start_str: str, end_str: str, status_msg: Message):
    ...
    try:
        ...
        for idx, part_file in enumerate(parts):
            ...
            q = PROGRESS_QUEUES.get(task_id)
            if q:
                loop.call_soon_threadsafe(q.put_nowait, (f"رفع الجزء ({idx+1}/{len(parts)})", 0, part_size, upload_start, "bytes"))
            ...
            def upload_progress(current, total):
                ...
                if now - last_edit[0] >= 1.5 or current == total:
                    q_inner = PROGRESS_QUEUES.get(task_id)
                    if q_inner:
                        loop.call_soon_threadsafe(q_inner.put_nowait, (f"رفع الجزء ({idx+1}/{len(parts)})", current, total, upload_start, "bytes"))
                    last_edit[0] = now
        ...
        if task_id not in CANCELLED_TASKS:
            ...
            try:
                if "429" in str(e) or "403" in str(e):
                    await status_msg.edit_text("⚠️ **تنبيه (HTTP 403/429):** كثرة طلبات معالجة. يرجى الانتظار دقيقة قبل إعادة المحاولة.")
                else:
...
async def handle_message(client: Client, message: Message):
    ...
    # 🚀 التعديل الجوهري v60: إذا كان الرابط مباشراً أو MediaFire أو Mega تبدأ عملية التحميل مباشرة دون إظهار خيارات الجودة
    if is_direct_download_url(url):
        status_msg = await message.reply_text("📥 **رابط تحميل مباشر! جاري بدء التحميل الفوري...**", quote=True)
        PROGRESS_QUEUES[req_id] = asyncio.Queue()
        task = asyncio.get_running_loop().create_task(process_task(client, req_id, url, "best", status_msg))
        ACTIVE_TASKS[req_id] = task
        return
    ...
    try:
        ...
        if "429" in str(e) or "403" in str(e):
            await loading_msg.edit_text("⚠️ **تنبيه:** قامت المنصة بتحديد الطلبات مؤقتاً (HTTP 403/429). يمكنك اختيار الجودة للبدء بنمط الحماية المباشر:", reply_markup=keyboard)
        else:
...
async def process_task(client: Client, task_id: str, url: str, option: str, init_status_msg: Message):
    ...
    try:
        ...
        for idx, part_file in enumerate(parts):
            ...
            q = PROGRESS_QUEUES.get(task_id)
            if q:
                loop.call_soon_threadsafe(q.put_nowait, (label, 0, part_size, upload_start, "bytes"))
            ...
            def upload_progress(current, total):
                ...
                if now - last_edit[0] >= 1.5 or current == total:
                    q_inner = PROGRESS_QUEUES.get(task_id)
                    if q_inner:
                        loop.call_soon_threadsafe(q_inner.put_nowait, (label, current, total, upload_start, "bytes"))
                    last_edit[0] = now
            ...
            if is_document:
                ...
                part_duration = await loop.run_in_executor(None, get_media_duration, part_file)
                width, height = await loop.run_in_executor(None, get_video_dimensions, part_file)
                raw_thumb = await loop.run_in_executor(None, get_valid_thumbnail, part_file, task_id, file_info.get("thumb_path"), f"_{idx}")
                thumb_path = sanitize_thumb(raw_thumb)
        ...
        should_snapshot = (url_type == "direct" and user_cfg["snapshots_direct"]) or (url_type == "social" and user_cfg["snapshots_social"])

        if not is_audio and not is_document and should_snapshot:
        ...
        if task_id not in CANCELLED_TASKS:
            ...
            try:
                if "429" in str(e) or "403" in str(e):
                    await init_status_msg.edit_text("⚠️ **تنبيه (HTTP 403/429):** قامت المنصة بتحديد الطلبات مؤقتاً. تم التراجع بأمان، أعد المحاولة بعد دقيقة.")
                else:
                    await init_status_msg.edit_text(f"❌ **حدث خطأ أثناء تنزيل أو معالجة الرابط:**\n`{str(e)[:150]}`")
            except Exception:
                pass
    finally:
        worker_task.cancel()
...
if __name__ == "__main__":
    logger.info("🚀 جاري تشغيل بوت Universal Downloader v60 Engine...")
    app.run()