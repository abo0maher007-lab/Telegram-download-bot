...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v47.2 Engine (Direct Link Fix)
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v47.2")
...
app = Client("UniversalDownloaderBot_v47_2", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
def is_direct_media_url(url: str) -> bool:
    ...
    exts = ['.mp4', '.mkv', '.webm', '.avi', '.mov', '.flv', '.mp3', '.m4a', '.aac']
...
def generate_ffmpeg_thumbnail(video_path: str, task_id: str, suffix: str = "") -> Optional[str]:
    ...
    try:
        ...
        logger.warning(f"⚠️ تعذر إنتاج thumbnail: {e}")
    return None
...
async def get_video_metadata_and_thumb(file_path: str) -> Tuple[int, int, int, Optional[str]]:
    ...
    thumb = generate_ffmpeg_thumbnail(file_path, f"meta_{int(time.time())}")
...
async def translate_to_arabic(text: str) -> str:
    ...
    if is_arabic_text(text): return ""
    clean_text = text.strip()
    if len(clean_text) > 800: clean_text = clean_text[:800] + "..."

    def _do_translate():
        if HAS_TRANSLATOR:
            try: return GoogleTranslator(source='auto', target='ar').translate(clean_text)
            except Exception: pass
        try:
...
def determine_url_type(url: str) -> str:
    url_lower = url.lower()
    if is_dailymotion_url(url): return "dailymotion"
    social_domains = ["instagram.com", "tiktok.com", "twitter.com", "x.com", "facebook.com", "fb.watch", "fb.gg", "youtube.com", "youtu.be"]
    if any(domain in url_lower for domain in social_domains): return "social"
    return "direct"
...
async def extract_9_frames(file_path: str, duration: int, chat_id: int) -> List[str]:
    ...
    for i in range(1, 10):
        ...
        try:
            subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
            if os.path.exists(out_f): frames.append(out_f)
        except Exception: pass
    return frames

def trim_video_ffmpeg(input_path: str, start_str: str, end_str: str, output_path: str) -> bool:
    try:
        cmd = ["ffmpeg", "-y", "-ss", start_str, "-to", end_str, "-i", input_path, "-c:v", "copy", "-c:a", "copy", "-movflags", "+faststart", output_path]
        subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
...
def split_video_file(file_path: str, task_id: str, target_size_bytes: int = 1900 * 1024 * 1024) -> List[str]:
    ...
    if file_size <= target_size_bytes: return [file_path]
    total_duration = get_media_duration(file_path)
    if total_duration <= 0: return [file_path]
    ...
    for i in range(num_parts):
        ...
        cmd = ["ffmpeg", "-y", "-ss", str(start_sec), "-i", file_path, "-t", str(segment_duration), "-c:v", "copy", "-c:a", "copy", "-movflags", "+faststart", part_out]
        try:
            subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    async def safe_edit_text(text: str, reply_markup):
        try: await status_msg.edit_text(text, reply_markup=reply_markup)
        except Exception: pass
    ...
    if quality_choice == 'mp3':
        ...
        format_str = f"best[height<={quality_choice}]/best" if quality_choice in ['1080', '720', '480', '320'] else "best"

    headers = BROWSER_HEADERS.copy()
    headers.update({'Referer': 'https://www.dailymotion.com/', 'Origin': 'https://www.dailymotion.com'})
    ...
    if DM_COOKIE_PATH and os.path.exists(DM_COOKIE_PATH): ydl_opts['cookiefile'] = DM_COOKIE_PATH
    ...
    try:
        ...
        file_path = f"{base_file}.mp3" if is_audio_mode and os.path.exists(f"{base_file}.mp3") else await convert_to_mp4(downloaded_file)
        ...
        for idx, part_file in enumerate(parts):
            ...
            async def upload_progress_callback(current, total):
                ...
                if now - last_upload_update[0] >= 2.0 or current == total:
                    ...
                    try: await status_msg.edit_text(text, reply_markup=cancel_btn)
                    except Exception: pass

            duration, width, height, thumb_path = await get_video_metadata_and_thumb(part_file)
            caption_out = f"🎬 **Dailymotion Media [{quality_choice}]**"
            if len(parts) > 1: caption_out += f"\n📦 **الجزء ({idx+1}/{len(parts)})**"
            if post_caption_text and idx == 0:
                clean_desc = post_caption_text.strip()
                if len(clean_desc) > 400: clean_desc = clean_desc[:400] + "..."
                caption_out += f"\n\n📝 **النص الأصلي المنشور:**\n{clean_desc}"
                if translated_arabic: caption_out += f"\n\n🇦🇪 **الترجمة العربية:**\n{translated_arabic}"

            if is_audio_mode:
                audio_kwargs = {"chat_id": chat_id, "audio": part_file, "caption": caption_out, "title": os.path.basename(part_file), "progress": upload_progress_callback}
                if duration > 0: audio_kwargs["duration"] = int(duration)
                await app.send_audio(**audio_kwargs)
            else:
                video_kwargs = {"chat_id": chat_id, "video": part_file, "caption": caption_out, "supports_streaming": True, "progress": upload_progress_callback}
                if width > 0: video_kwargs["width"] = width
                if height > 0: video_kwargs["height"] = height
                if duration > 0: video_kwargs["duration"] = int(duration)
                if thumb_path and os.path.exists(thumb_path): video_kwargs["thumb"] = thumb_path
                await app.send_video(**video_kwargs)
        ...
        if not is_audio_mode and user_cfg["snapshots_dailymotion"]:
            ...
            if total_duration > 0:
                ...
                if frames:
                    media_group = [InputMediaPhoto(media=fr) for fr in frames]
        ...
        if str(e) == "CANCELLED": await status_msg.edit_text("🛑 **تم إلغاء العملية بناءً على طلبك.**", reply_markup=None)
        else:
...
# ----------------------------------------------------
# 🌐 محرك MediaFire & Direct File Stream Downloader
# ----------------------------------------------------
def download_direct_file_stream(url: str, target_option: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
    out_dir = "downloads"
    os.makedirs(out_dir, exist_ok=True)
    
    parsed_path = urlparse(url).path
    raw_name = os.path.basename(parsed_path) or f"video_{task_id}.mp4"
    if not os.path.splitext(raw_name)[1]:
        raw_name += ".mp4"
        
    file_path = os.path.join(out_dir, f"{task_id}_{raw_name}")
    start_time = time.time()
    ...
    with urllib.request.urlopen(req, timeout=30) as response, open(file_path, 'wb') as out_file:
        ...
        while True:
            ...
            if q:
                label = "تنزيل رابط مباشر"
                loop.call_soon_threadsafe(q.put_nowait, (label, downloaded, total_size, start_time))

    is_doc = (target_option == "doc")
    return {
        "file_path": file_path,
        "title": raw_name,
        "duration": 0,
...
def inspect_mediafire_link(url: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "file_name": file_name,
        "is_video": ext in ['.mp4', '.mkv', '.avi', '.mov', '.flv', '.webm', '.m4v', '.3gp'],
        "ext": ext
    }
...
def download_mediafire_file(url: str, target_option: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
    ...
    return download_direct_file_stream(info["direct_url"], target_option, task_id, loop)
...
def download_mega_file(url: str, task_id: str) -> Dict[str, Any]:
    ...
    out_dir = "downloads"
    os.makedirs(out_dir, exist_ok=True)
    if task_id in CANCELLED_TASKS: raise ProcessCancelledException("CANCELLED")
    ...
    return {"file_path": new_path, "title": filename, "duration": 0, "thumb_path": None, "description": "", "is_audio": False, "is_document": True}

# ----------------------------------------------------
# 🧠 المحرك الشامل v47.2
# ----------------------------------------------------
class UniversalEngineV47_2:
    def __init__(self):
        self.user_agents = [
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        if is_direct_media_url(url):
            ...
            return {"title": filename, "duration": 0, "uploader": "رابط مباشر", "resolutions": [720]}
        ...
        if is_dailymotion_url(url):
            ydl_opts.update({
                'http_headers': {'User-Agent': ua, 'Referer': 'https://www.dailymotion.com/', 'Origin': 'https://www.dailymotion.com'},
                'extractor_args': {'dailymotion': {'app_id': 'dmfed', 'geo_verification_network': 'http'}}
            })
            if DM_COOKIE_PATH and os.path.exists(DM_COOKIE_PATH): ydl_opts['cookiefile'] = DM_COOKIE_PATH

        if "instagram.com" in url_lower and IG_COOKIE_PATH and os.path.exists(IG_COOKIE_PATH): ydl_opts['cookiefile'] = IG_COOKIE_PATH
        elif ("twitter.com" in url_lower or "x.com" in url_lower) and TW_COOKIE_PATH and os.path.exists(TW_COOKIE_PATH): ydl_opts['cookiefile'] = TW_COOKIE_PATH
        elif "pornhub.com" in url_lower and PH_COOKIE_PATH and os.path.exists(PH_COOKIE_PATH): ydl_opts['cookiefile'] = PH_COOKIE_PATH
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for f in formats:
                h = f.get('height')
                if h and isinstance(h, int) and h >= 144: resolutions_set.add(h)
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        if is_direct_media_url(url):
            return download_direct_file_stream(url, target_option, task_id, loop)
        ...
        def ytdl_hook(d):
            ...
            if d.get("status") in ["downloading", "finished"]:
                ...
                q = PROGRESS_QUEUES.get(task_id)
                if q: loop.call_soon_threadsafe(q.put_nowait, (label, downloaded, total, start_time))
        ...
        if target_option == "mp3":
            ...
            is_audio = False
            postprocessors = [{'key': 'FFmpegThumbnailsConvertor', 'format': 'jpg'}]
            format_selector = 'best[ext=mp4]/best' if target_option == "best" else f'best[height<={target_option}][ext=mp4]/best[height<={target_option}]/best'
        ...
        ydl_opts = {
            ...
            'socket_timeout': 20,
            'geo_bypass': True,
            'http_headers': {'User-Agent': user_agent, 'Accept-Language': 'en-US,en;q=0.5'},
        }

        if is_dailymotion_url(url):
            ydl_opts['format'] = 'best' if target_option == "best" else f'best[height<={target_option}]/best'
            ydl_opts['http_headers'].update({'Referer': 'https://www.dailymotion.com/', 'Origin': 'https://www.dailymotion.com'})
            ydl_opts['extractor_args'] = {'dailymotion': {'app_id': 'dmfed', 'geo_verification_network': 'http'}}
            if DM_COOKIE_PATH and os.path.exists(DM_COOKIE_PATH): ydl_opts['cookiefile'] = DM_COOKIE_PATH
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if not os.path.exists(final_file_path):
                if os.path.exists(filename): final_file_path = filename
                else:
                    matched = glob.glob(f"{base}.*")
                    if matched: final_file_path = matched[0]
...
engine = UniversalEngineV47_2()

def build_quality_keyboard(req_id: str, resolutions: List[int] = None) -> InlineKeyboardMarkup:
    buttons = [
        [InlineKeyboardButton("🎬 1080p (Full HD)", callback_data=f"q_1080_{req_id}"), InlineKeyboardButton("🎬 720p (HD)", callback_data=f"q_720_{req_id}")],
        [InlineKeyboardButton("🎬 480p (SD)", callback_data=f"q_480_{req_id}"), InlineKeyboardButton("🎬 320p (Low)", callback_data=f"q_320_{req_id}")],
        ...
        try:
            ...
            if now - last_update_time >= 1.5:
                ...
                if total > 0:
                    ...
                    bar = "🔄 `جاري تدفق البيانات من السيرفر...`\n"
                    eta_str = ""

                text = (
                    f"⚡ **[v47.2 Engine - Direct Stream Fix]**\n"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v47.2 Universal Downloader Engine**\n\n"
        "✨ **التحديثات الجديد:**\n"
        "• ⚡ **دعم وتنزيل الروابط المباشرة (.mp4) فوراً دون تعليق السيرفر.**\n"
        "• ⏱️ **إضافة مهلة زمنية (Timeout) لتجنب تجمد عملية التنزيل.**\n"
        "• 🛠️ **إصلاح عرض واسم التنزيل عند اختيار أفضل جودة.**\n"
    )
...
async def settings_cmd(client: Client, message: Message):
    ...
    await message.reply_text("⚙️ **إعدادات التحكم باللقطات (Snapshots):**", reply_markup=kb, quote=True)
...
async def settings_callback_handler(client: Client, callback: CallbackQuery):
    ...
    if callback.data == "cfg_toggle_direct": cfg["snapshots_direct"] = not cfg["snapshots_direct"]
    elif callback.data == "cfg_toggle_dm": cfg["snapshots_dailymotion"] = not cfg["snapshots_dailymotion"]
    elif callback.data == "cfg_toggle_social": cfg["snapshots_social"] = not cfg["snapshots_social"]
...
async def handle_message(client: Client, message: Message):
    ...
    if "mediafire.com" in url.lower():
        try:
            ...
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("🎬 إرسال كـ فيديو", callback_data=f"q_vid_{req_id}"), InlineKeyboardButton("📁 إرسال كـ مستند", callback_data=f"q_doc_{req_id}")]
            ]) if mf_info["is_video"] else InlineKeyboardMarkup([[InlineKeyboardButton("📁 تنزيل الملف (مستند)", callback_data=f"q_doc_{req_id}")]])
            await message.reply_text(f"📁 **MediaFire File:** `{filename}`", reply_markup=keyboard, quote=True)
            return
        except Exception as e: logger.error(f"Error inspecting mediafire: {e}")

    if "mega.nz" in url.lower() or "mega.co.nz" in url.lower():
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("📁 تنزيل الملف المباشر", callback_data=f"q_doc_{req_id}")]])
        await message.reply_text("📁 **روابط ملفات ميجا التلقائية:**", reply_markup=keyboard, quote=True)
        return

    loading_msg = await message.reply_text("🔍 **جاري فحص الرابط واستخراج المعلومات...**", quote=True)
    loop = asyncio.get_running_loop()
...
async def option_callback_handler(client: Client, callback: CallbackQuery):
    try:
        ...
        quality_label = "أفضل جودة" if option == "best" else f"{option}p"
        msg_text = "📁 **جاري جلب وتحميل الملف...**" if option in ["doc", "vid"] else ("🎵 **جاري استخراج الصوت MP3...**" if option == "mp3" else f"🔎 **جاري التنزيل المتوازي ({quality_label})...**")
...
async def cancel_handler(client: Client, callback: CallbackQuery):
    ...
    except Exception as e: logger.error(f"Cancel Handler Error: {e}")
...
async def process_task(client: Client, task_id: str, url: str, option: str, init_status_msg: Message):
    ...
    try:
        ...
        for idx, part_file in enumerate(parts):
            ...
            base_caption = f"🎬 **{file_info['title']}**{part_suffix}\n🛡️ **Engine:** `v47.2 Engine`"
            if raw_desc and idx == 0:
                clean_raw = raw_desc.strip()
                if len(clean_raw) > 400: clean_raw = clean_raw[:400] + "..."
                base_caption += f"\n\n📝 **النص الأصلي المنشور:**\n{clean_raw}"
                if translated_arabic: base_caption += f"\n\n🇦🇪 **الترجمة العربية:**\n{translated_arabic}"

            if is_document:
                await client.send_document(chat_id=init_status_msg.chat.id, document=part_file, caption=base_caption, progress=upload_progress)
            elif is_audio:
                if part_duration <= 0: part_duration = get_media_duration(part_file)
                audio_kwargs = {"chat_id": init_status_msg.chat.id, "audio": part_file, "title": str(file_info['title']), "caption": f"🎵 **{file_info['title']}**{part_suffix}\n🎼 **الصيغة:** `MP3 320kbps`\n🛡️ **Engine:** `v47.2 Engine`", "progress": upload_progress}
                if part_duration > 0: audio_kwargs["duration"] = int(part_duration)
                await client.send_audio(**audio_kwargs)
            else:
                if part_duration <= 0: part_duration = get_media_duration(part_file)
                part_thumb = get_valid_thumbnail(part_file, task_id, file_info.get("thumb_path"), f"_{idx}")
                vid_kwargs = {"chat_id": init_status_msg.chat.id, "video": part_file, "supports_streaming": True, "caption": base_caption, "progress": upload_progress}
                if width > 0: vid_kwargs["width"] = width
                if height > 0: vid_kwargs["height"] = height
                if part_duration > 0: vid_kwargs["duration"] = int(part_duration)
                if part_thumb and os.path.exists(part_thumb): vid_kwargs["thumb"] = part_thumb
        ...
        if url_type == "direct" and user_cfg["snapshots_direct"]: should_extract = True
        elif url_type == "dailymotion" and user_cfg["snapshots_dailymotion"]: should_extract = True
        elif url_type == "social" and user_cfg["snapshots_social"]: should_extract = True
        ...
        if task_id not in CANCELLED_TASKS:
            ...
            try: await init_status_msg.edit_text(f"❌ **حدث خطأ أثناء معالجة الرابط:**\n`{str(e)[:150]}`")
            except Exception: pass
    finally:
        worker_task.cancel()
        cleanup_files(task_id)
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v47.2 (Direct Stream Engine)...")
    app.run()