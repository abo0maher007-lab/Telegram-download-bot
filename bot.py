...
# تكوين عميل Telethon بمهلة زمنية وإعادات محاولة عالية لتجنب انقطاع رفع الملفات الكبيرة
bot = TelegramClient(
    "url_uploader_bot",
    API_ID,
    API_HASH,
    timeout=600,  # 10 دقائق لمهلة الاستجابة الفردية
    connection_retries=15,  # زيادة محاولات إعادة الاتصال
    retry_delay=5,
)
...
async def download_file_direct(url, output_path, cancel_event, status_msg):
    # تمديد الـ Timeout للتنزيل المباشر إلى 6 ساعات مع ضبط وقت قراءة المقبس
...
async def start_handler(event):
    await event.respond("🚀 **مرحباً بك في بوت التنزيل والرفع Pro v5.8**")
...
async def process_download_job(event, url):
    ...
    try:
        ...
        # رفع الملف لـ Telegram
        await bot.send_file(
            event.chat_id,
            file_name,
            caption=f"📱 **اسم الملف:** `{file_name}`\n⚡ **تم الرفع بنجاح v5.8 Pro**",
