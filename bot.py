...
# ----------------------------------------------------
# 3. خطاف yt-dlp السريع دون تعليق الـ 99%
# ----------------------------------------------------
...
async def download_yt_dlp_best(url: str, output_path: str, bot, chat_id, message_id, task_id) -> dict:
    ...
    ydl_opts = {
        ...
        'outtmpl': output_path,
        'writethumbnail': False,  # إلغاء معالجة الصورة المصغرة لتفادي تعليق 99%
        'noplaylist': True,
        'quiet': True,
        'no_warnings': True,
        'geo_bypass': True,
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        "مرحباً بك في بوت التنزيل والرفع المباشر v9.0 🚀\n\n"
        "✨ **المميزات:**\n"
        "• **تم حل مشكلة التجمد عند 99% نهائياً.**\n"
        "• **عداد رفع وتنزيل حي مباشر.**\n"
...
async def process_download(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str, is_trim=False, start_sec=0, duration_sec=0):
    ...
    try:
        ...
        if not downloaded_file or not os.path.exists(downloaded_file):
            await status_msg.edit_text("❌ تعذر تحميل هذا المقطع. تأكد من صحة الرابط.")
            return
...
def main():
    ...
    logging.info("==========================================")
    logging.info("تم التشغيل بنجاح مع إصلاح تعليق الـ 99%!")
    logging.info("==========================================")
