...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v24.0 Mega Engine Fix
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v24_0")
...
app = Client("UniversalDownloaderBot_v24_0", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
# ----------------------------------------------------
# ☁️ محرك Mega.nz المحدث المباشر (تجاوز خطأ asyncio.coroutine)
# ----------------------------------------------------
def download_mega_file(url: str, task_id: str) -> Dict[str, Any]:
    out_dir = "downloads"
    os.makedirs(out_dir, exist_ok=True)
    
    # استخدام megatools كبديل لنواة العزل لتفادي أخطاء بايثون 3.11+ asyncio
    try:
        cmd = ["megatools", "dl", "--path", out_dir, url]
        result = subprocess.run(cmd, capture_output=True, text=True, check=True)
    except (subprocess.CalledProcessError, FileNotFoundError):
        # استخدام المكتبة بأسلوب آمن ومعزول
        try:
            ...
            raise Exception(f"فشل تنزيل ملف Mega: {str(e)}")

    # تحديد الملف المناسب في حال تم استخدام megatools
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v24.0 Mega Fix Engine**\n\n"
        "• تم إصلاح خطأ `asyncio.coroutine` للتحميل من Mega نهائياً.\n"
        "• إمكانية التحكم بقرص التحميلات عبر `/disk`.\n\n"
        ...
        if os.path.exists(file_path):
            ...
            if is_document:
                await client.send_document(
                    ...
                    document=file_path,
                    caption=f"📁 **{file_info['title']}**\n🛡️ **Engine:** `v24.0 Mega Fix Edition`",
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v24.0 Mega Fix Engine...")
    app.run()
