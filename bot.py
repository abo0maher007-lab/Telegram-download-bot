...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v63 Engine Fixed
# ----------------------------------------------------
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v63 Engine (معالجة إصلاح البروكسي ويوتيوب)
# ----------------------------------------------------
class UniversalEngineV63:
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        ...
        ydl_opts = {
            ...
            'skip_download': True,
            'user_agent': ua,
            'headers': get_random_headers(for_fb=is_fb),
            'impersonate': 'chrome',
            'geo_bypass': True,
            'check_formats': False,
            'sleep_interval': 2,
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    ydl_opts['headers'] = get_random_headers(for_fb=is_fb)
                    continue
                raise e
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        ydl_opts = {
            ...
            'external_downloader_args': {
                'ffmpeg': ['-headers', f'User-Agent: {headers["User-Agent"]}']
            }
        }
        ...
        try:
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    opener.addheaders = [(k, v) for k, v in current_headers.items()]
                    urllib.request.install_opener(opener)
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        if target_option == "mp3":
            ...
            if target_option == "best":
                format_selector = 'best[ext=mp4]/bestvideo[ext=mp4]+bestaudio[ext=m4a]/best'
            else:
                format_selector = f'best[height<={target_option}][ext=mp4]/bestvideo[height<={target_option}]+bestaudio/best[height<={target_option}]/best'
        ...
        # إصلاح إرسال البروكسي إلى yt-dlp بالشكل الصحيح
        if HTTP_PROXY:
            ydl_opts['proxy'] = HTTP_PROXY
        ...
        if is_fb:
            ...
            ydl_opts.update({
                ...
                'check_formats': False,
                'headers': get_random_headers(for_fb=True)
            })
            ydl_opts['extractor_args']['facebook'] = {'skip': ['dash', 'hls']}
            if FB_COOKIE_PATH and os.path.exists(FB_COOKIE_PATH):
                ydl_opts['cookiefile'] = FB_COOKIE_PATH
        ...
        for dl_attempt in range(max_dl_retries):
            try:
                ...
                if dl_attempt < max_dl_retries - 1:
                    time.sleep(2 * (dl_attempt + 1) + random.uniform(0.5, 1.5))
                    ydl_opts['headers'] = get_random_headers(for_fb=is_fb)
                    # إزالة القيود الصارمة في المحاولة الثانية للتنزيل المباشر
...
if __name__ == "__main__":
    logger.info("🚀 جاري بدء تشغيل بوت Universal Downloader Bot (v63 Engine)...")
    app.run()