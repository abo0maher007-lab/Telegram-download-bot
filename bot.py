...
def render_status_text(action_title: str, percent: float, current: float, total: float, speed: float, eta: float) -> str:
...
# ----------------------------------------------------
# 3. خطاف yt-dlp وحساب المسار المباشر
# ----------------------------------------------------
    ...
    def hook(d):
        ...
        if d['status'] == 'downloading':
            ...
            if now - last_update_time[0] >= 1.5:
                ...
                async def safe_edit():
                    try:
                        await bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=text, parse_mode="Markdown", reply_markup=keyboard)
                    except Exception:
                        pass
            ...
            text = "⚡ **اكتمل التنزيل! جاري تجهيز الملف للرفع...**"
            async def safe_finish():
                try:
                    await bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=text, parse_mode="Markdown")
                except Exception:
                    pass
            asyncio.run_coroutine_threadsafe(safe_finish(), loop)

    return hook

async def download_yt_dlp_best(url: str, output_path: str, bot, chat_id, message_id, task_id) -> tuple:
    ...
    info, actual_file = await loop.run_in_executor(None, _extract)
    return info, actual_file

# ----------------------------------------------------
# 4. محاكي تقدم الرفع لتلغرام (Upload Progress)
# ----------------------------------------------------
class ProgressFileReader:
    def __init__(self, filename, bot, chat_id, message_id, task_id, title_prefix="جاري الرفع إلى تلغرام..."):
    ...
    def read(self, size=-1):
        if CANCEL_REQUESTS.get(self._task_id):
            raise Exception("CANCELLED_BY_USER")
        ...
        now = time.time()
        if now - self._last_update_time >= 1.5 or self._read_so_far == self._total_size:
            self._last_update_time = now
            elapsed = now - self._start_time
            speed = self._read_so_far / elapsed if elapsed > 0 else 0
            eta = (self._total_size - self._read_so_far) / speed if speed > 0 else 0
            percent = (self._read_so_far / self._total_size * 100) if self._total_size > 0 else 0

            text = render_status_text(self._title_prefix, percent, self._read_so_far, self._total_size, speed, eta)
            keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("❌ إلغاء العملية", callback_data=f"cancel_{self._task_id}")]])

            async def safe_edit_upload():
                try:
            ...
            asyncio.run_coroutine_threadsafe(safe_edit_upload(), self._loop)
...
# ----------------------------------------------------
# 5. أدوات FFmpeg لمعالجة وسائط الميديا
# ----------------------------------------------------
...
# ----------------------------------------------------
# 6. المعالجة والتشغيل الرئيسي
# ----------------------------------------------------
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    db_add_user(update.effective_user.id)
    welcome_text = (
        "مرحباً بك في بوت التنزيل والرفع المباشر v10.0 🚀\n\n"
        "✨ **المميزات:**\n"
        "• **تم إصلاح التعليق بعد انتهاء التحميل والانتقال المباشر للرفع.**\n"
        "• **عداد حقيقي للرفع والتنزيل.**\n"
...
async def process_download(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str, is_trim=False, start_sec=0, duration_sec=0):
    ...
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("❌ إلغاء العملية", callback_data=f"cancel_{task_id}")]])
    status_msg = await update.message.reply_text("⏳ **جاري فتح الاتصال وبدء التحميل المباشر...**", parse_mode="Markdown", reply_markup=keyboard)
    ...
    try:
        ...
        info, downloaded_file = await download_yt_dlp_best(url, output_template, context.bot, update.effective_chat.id, status_msg.message_id, task_id)
        title = info.get('title', 'مقطع فيديو')

        if not downloaded_file or not os.path.exists(downloaded_file):
            await status_msg.edit_text("❌ تعذر العثور على الملف المحمل.")
            return
        ...
        for idx, part_path in enumerate(parts, 1):
            ...
            upload_title = f"⬆️ جاري الرفع ({idx}/{total_parts})..." if total_parts > 1 else "⬆️ جاري الرفع إلى تلغرام..."
            
            reader = ProgressFileReader(
                part_path, context.bot, update.effective_chat.id,
                status_msg.message_id, task_id, title_prefix=upload_title
            )
        ...
        if CANCEL_REQUESTS.get(task_id) or "CANCELLED_BY_USER" in str(e):
            ...
            await status_msg.edit_text(f"❌ **حدث خطأ أثناء الرفع أو المعالجة:**\n`{str(e)}`", parse_mode="Markdown")
    finally:
...
def main():
    ...
    logging.info("==========================================")
    logging.info("تم التشغيل بنجاح مع إصلاح الانتهاء المباشر للتحميل!")
    logging.info("==========================================")
