...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v59 Engine (Multi-Tier Fallback & Ultra Anti-403/429 Engine)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v59 Engine (Multi-Tier Fallback & Ultra Anti-403/429 Engine)
# ----------------------------------------------------
class UniversalEngineV59:
    ...
    def extract_info_only(self, url: str) -> Dict[str, Any]:
        ...
        # إعادة المحاولة بتأخير عشوائي وترويسات متصفح عند الخطأ 403 أو 429
        max_retries = 3
    ...
    def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """دالة مخصصة لتحميل الفيديوهات المباشرة وترويسات متصفح وتفادي خطأ HTTP 403/429"""
        out_dir = "downloads"
        os.makedirs(out_dir, exist_ok=True)
        ...
        # دعم الإعادة والتحمل عند مواجهة HTTP Error 403 / 429
        max_attempts = 4
    ...
    async def _async_fallback_stream_download(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> str:
        """استراتيجية بديلة للتدفق المباشر عبر Aiohttp كحل طوارئ عند تفاقم الحظر 403/429"""
        out_dir = "downloads"
        os.makedirs(out_dir, exist_ok=True)
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            async with session.get(url, headers=headers, proxy=HTTP_PROXY) as response:
                if response.status not in [200, 206]:
                    raise Exception(f"HTTP Error {response.status} in Async Stream Fallback")
                ...
                with open(file_path, 'wb') as f:
                    async for chunk in response.content.iter_chunked(64 * 1024):
                        ...
                        if chunk:
                            ...
                            q = PROGRESS_QUEUES.get(task_id)
                            if q:
                                loop.call_soon_threadsafe(q.put_nowait, ("تنزيل بالنمط الآمن البديل (Aiohttp Stream)", downloaded, total_size, start_time, "bytes"))
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        """دالة تحميل v59 فائقة القوة والاستقرار - معززة بـ 3 استراتيجيات تنزيل متدرجة للتغلب على كافة مشاكل التحميل"""
        auto_disk_guard()
        url_lower = url.lower()
        ...
        # التحقق مما إذا كان الرابط مباشراً لملف فيديو لتحميله بدالة التحميل المباشر
        ...
        # --- الاستراتيجية الأولى + الثانية (yt-dlp Primary & Flexible Fallback) ---
        max_dl_retries = 3
        last_exception = None
        ...
        # إعادة المحاولة بأسلوب fallback مرن ومرخص بدون تخصيص شديد للشكل
        try:
        ...
        # --- الاستراتيجية الثالثة: Aiohttp Async Stream Fallback (الحل الأخير لتجاوز الحجب 403/429) ---
        try:
            logger.warning("🚀 Activating Ultimate Strategy 3: Aiohttp Async Stream Fallback...")
            future = asyncio.run_coroutine_threadsafe(self._async_fallback_stream_download(url, task_id, loop), loop)
            fallback_path = future.result(timeout=300)
            ...
            if now - last_update_time >= 2.0 or current == 0 or current == total:
                ...
                if mode == "time":
                    if total > 0:
                        ...
                        speed_factor = speed
                        text = (
                            f"⚙️ **[v59 Engine - Multi-Tier Ultra Engine]**\n"
                    ...
                    text = (
                        f"⚡ **[v59 Engine - Multi-Tier Ultra Engine]**\n"
                        f"📌 **العملية:** {action_title}\n\n"
                        f"{bar}"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت v59 Universal Downloader - Multi-Tier Download Engine**\n\n"
        "✨ **تحديثات الإصدار v59 الجديدة:**\n"
        "• ⚡ **دالة التحميل الخارقة (Ultra Multi-Tier Downloader):** تدعيم دالة التحميل بـ 3 استراتيجيات متدرجة (Muti-Extractor fallback & Aiohttp Stream) لضمان تجاوز أخطاء 403 Forbidden و 429 Too Many Requests نهائياً.\n"
        "• 🛡️ **نظام الحماية من الحظر الشبكي:** تبديل تلقائي للترويسات، العناوين ومحاكاة المتصفحات الحقيقية أثناء التنزيل.\n"
        "• 🗜️ **تتبع ضغط FFmpeg التفاعلي الحقيقي:** شريط تقدم مباشر يعرض النسبة المئوية، الوقت المنقضي، السرعة الحقيقية ونسبة الحفظ.\n"
        ...
        "• 🛡️ **الحفاظ التام على جميع المميزات السابقة:** ديليموشن، ميديافاير، ميجا ومختلف المنصات دون أي تغيير.\n"
    )
...
async def process_task(client: Client, task_id: str, url: str, option: str, init_status_msg: Message):
    ...
    try:
        ...
        if task_id not in CANCELLED_TASKS:
            ...
            try:
                if "429" in str(e) or "403" in str(e):
                    await init_status_msg.edit_text("⚠️ **تنبيه (HTTP Error 403/429):** قامت المنصة بتحديد الطلبات مؤقتاً. جرب مجدداً بعد دقيقة.")
                else:
...
if __name__ == "__main__":
    logger.info("🚀 جاري تشغيل بوت v59 (Multi-Tier Download Engine & Ultra Protected)...")
    app.run()
