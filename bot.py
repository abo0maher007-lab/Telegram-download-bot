...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v15 Engine
# ----------------------------------------------------
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [%(levelname)s] - %(message)s")
logger = logging.getLogger("UniversalBot_v15")
...
app = Client("UniversalDownloaderBot_v15", api_id=int(API_ID), api_hash=API_HASH, bot_token=BOT_TOKEN)
...
def setup_cookies():
    """فك تشفير كوكيز Pornhub وحفظها بتنسيق Netscape القياسي"""
...
# ----------------------------------------------------
# 🧠 المحرك الشامل v15 Ultra Anti-Bypass Engine
# ----------------------------------------------------
class UniversalEngineV15:
    def __init__(self):
        self.user_agents = [
            ...
            if d.get("status") == "downloading":
                ...
                if total > 0 and (time.time() - last_update[0] > 2):
                    ...
                    asyncio.run_coroutine_threadsafe(
                        update_progress_ui(status_msg, "جاري الكشط بالتجاوز المتقدم v15", downloaded, total, start_time, task_id),
                        loop
                    )

        user_agent = random.choice(self.user_agents)

        # إعدادات v15 المتقدمة جداً لتجاوز Cloudflare و Anti-Bot 403
        ydl_opts = {
            ...
            'impersonate': 'chrome120', # محاكاة متصفح كروم محدد بدقة للتجاوز
        }
...
async def update_progress_ui(message: Message, action_title: str, current: int, total: int, start_time: float, task_id: str):
    ...
    text = (
        f"⚡ **[v15 Ultra Engine] {action_title}**\n\n"
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        "🚀 **أهلاً بك في بوت التحميل v15 Engine**\n\n"
        "• تحديث كامل لنظام محاكاة متصفحات Chrome 133\n"
        "• حل نهائي لرفض 403 وحجب الخوادم السحابية\n"
        "أرسل رابط الميديا لبدء التحميل مباشرة."
    )

async def process_task(client: Client, message: Message, task_id: str, url: str):
    status_msg = await message.reply_text("🔎 **جاري فك التشفير وتجاوز الحماية بـ v15...**")
...
if __name__ == "__main__":
    ...
    logger.info("🚀 جاري تشغيل بوت v15 Engine...")
    app.run()