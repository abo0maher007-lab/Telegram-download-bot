...
# ----------------------------------------------------
# 🚂 إعداد التسجيل والمحيط - v21.0 AZNude & Facebook Fix Engine
# ----------------------------------------------------
...
class UniversalEngineV21:
    ...
    def download_indirect_media(self, url: str, target_option: str, task_id: str, status_msg: Message, loop: asyncio.AbstractEventLoop) -> Dict[str, Any]:
        ...
        # ----------------------------------------------------
        # 🔵 الإصلاح المباشر لمشكلة Cannot parse data في فيسبوك
        # ----------------------------------------------------
        if "facebook.com" in url.lower() or "fb.watch" in url.lower():
            ydl_opts.update({
                'impersonate': 'chrome',  # تفعيل محاكاة متصفح كروم لتخطي حماية فيسبوك
                'check_formats': False,
            })
...
async def start_cmd(client: Client, message: Message):
    await message.reply_text(
        ...
        "• تم إصلاح خطأ استخراج بيانات فيسبوك (Cannot parse data).\n"
...
async def option_callback_handler(client: Client, callback: CallbackQuery):
    try:
        ...
        msg_text = "🎵 **جاري استخراج الصوت MP3...**" if option == "mp3" else f"🔎 **جاري كسر حماية الرابط والتنزيل ({option}p)...**"
