...
# ----------------------------------------------------
# 3. خطاف yt-dlp للتقدم واستخراج الجودات
# ----------------------------------------------------
...
# ----------------------------------------------------
# 4. معالجة الفيديو، الصورة المصغرة واقتطاع بيانات الميديا
# ----------------------------------------------------
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        "مرحباً بك في بوت التحميل الشامل الذكي v4.0 🚀\n\n"
        "✨ **المميزات:**\n"
        "• **تشغيل مباشر:** دعم خيار البث وزر التشغيل الفوري داخل تلغرام.\n"
        "• **صورة مصغرة تلقائية:** توليد غلاف المعاينة لكل الفيديوهات.\n"
        "• **دقة متعدده:** اختيار الجودة من أضعف دقة إلى 1080p.\n"
        "• **شريط تقدم متطور** يعرض السرعة والحجم والوقت المتبقي.\n"
...
async def quality_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    url_key = data_parts[2]

    url_info = PENDING_URLS.get(url_key)
    if not url_info:
        await query.edit_message_text("❌ انتهت صلاحية هذا الطلب. يرجى إرسال الرابط من جديد.")
        return

    url = url_info['url']
    is_trim = url_info.get('is_trim', False)
    start_sec = url_info.get('start_sec', 0)
    duration_sec = url_info.get('duration_sec', 0)
...
async def process_download(query, context: ContextTypes.DEFAULT_TYPE, url: str, format_spec: str, is_trim=False, start_sec=0, duration_sec=0):
    ...
    try:
        ...
        # تجهيز الصورة المصغرة ومعلومات الأبعاد والمدة
...
async def handle_url_qualities(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str, is_trim=False, start_sec=0, duration_sec=0):
    ...
    try:
        ...
        url_key = f"{update.effective_user.id}_{int(time.time())}"
        PENDING_URLS[url_key] = {
            'url': url,
            'is_trim': is_trim,
            'start_sec': start_sec,
        ...
        row = []
        for q in qualities:
            row.append(InlineKeyboardButton(f"🎬 {q}p", callback_data=f"q_{q}_{url_key}"))
            if len(row) == 2:
                buttons.append(row)
                row = []
        if row:
        ...
        buttons.append([
            InlineKeyboardButton("📱 أضعف جودة", callback_data=f"q_worst_{url_key}"),
            InlineKeyboardButton("🌟 أعلى جودة (1080p)", callback_data=f"q_best_{url_key}")
        ])
